...
3. A model fits the card (`fitsGraphicsMemory`) when `estimateGraphicsNeedMib(m) ≤ budgetMib`,
   ...
   **2026-09-08: the host-mapped weights come out of the BASE too, not only the share.** #321 took
   ...
   run **316–664 MiB per `llama_context`** across the five models in the 32 captured logs, 2.9 to
   19.8 GB on disk (**corrected 2026-09-08, #329**: the earlier "416–569 MiB across models from
   ...
   that same Q5 under a `--ubatch-size 512` probe. So the share reads 149 MiB on the E2B and 2,730
...
support (316–664 MiB per `llama_context` across the five models in the 32 captured logs, 2.9 to
