...
3. **The vault key is derived DIRECTLY from the password.** `VaultDescriptor`
   ...
   **no wrapped data key**. The DB file (`hilbertraum.sqlite.enc`) and every per-document file
...
5. **The sidecar pattern is fully reusable:** `LlamaServer` (spawn → free loopback port →
   ...
   `.hilbertraum-runtime.json` markers + `fetch-runtime.{ps1,sh}` + `assertCommercialDrive`
...
- **`services/transcriber/`:** `Transcriber` interface +
  ...
  **stderr only** (stdout carries the transcript — content must never ride an error
  ...
  `HILBERTRAUM_WHISPER_BIN` dev override, `resolveWhisperCliPath` mirrors the llama resolver.
...
- **Tests (51 new; 910/910 green):** `unit/audio-parser.test.ts` (packing/labels/
  ...
  `tests/manual/whisper-smoke.test.ts` (`HILBERTRAUM_WHISPER_SMOKE` + `HILBERTRAUM_WHISPER_AUDIO`,
...
- **Tests (+22 net):** `unit/wav.test.ts` (header/round-trip/clamp/empty),
  ...
  `tests/manual/dictation-smoke.test.ts` (`HILBERTRAUM_DICTATION_SMOKE` + `HILBERTRAUM_WHISPER_AUDIO`)
...
- **Verification:** +38 CI tests (detection fixtures, fake-engine task integration
  ...
  `HILBERTRAUM_OCR_SMOKE` manual harness PASSED on the real assets (confidence 95, zero remote
...
`HILBERTRAUM_WHISPER_SMOKE`, `HILBERTRAUM_OCR_SMOKE`, plus R-T1/R-T2 probes. Every phase ends on the
