...
| W5 | One trigger vocabulary + matcher + scoring + 8-skill corpus | P1 | — | L | [x] — NEW `services/skills/vocabulary.ts` single-sources each app skill's bilingual trigger terms (`{term,lang,match:word\|phrase\|stem,use:suggest\|route\|both}`); `match` drives ROUTING only (word-boundary via `wordIncludes` for single tokens, substring for phrases/German compound stems), while the SUGGESTION scorer word/phrase-infers from the manifest string itself (so an offer stays precision-first even on a `both`-stem German noun). All 5 routing gates (`isAnalysisShaped` bank+invoice, the 4 whole-doc shape gates, `isRedactionShaped`) now call `routeMatch(skillId,q)`; the 8 SKILL.md `triggers.keywords` regenerated to EXACTLY the `suggest\|both` terms + a bidirectional parity test. Scoring (`selector.ts`): `countKeywordHits` (word-boundary + longest-match dedupe, exported + reused by the eval harness so faithfulness is structural), keyword contribution capped `min(hits,2)×2`, and a keyword-REQUIRED gate in `selectByThreshold` (a lone doc signal never fires). Corpus expanded to 82 items across all 8 skills (German paraphrases + 8 cross-skill confusion pairs); asserted bars: keyword-required precision **98.3 %** (≥0.80 floor) + fired-wrong **0** on confusion pairs, threshold-3 auto-fire **100 %/0-wrong** preserved. Reproductions killed: `Netflix`→`net`, `Summarize this meeting` (offers+routes), `in 10 minutes`→Meeting, plus German-plural/compound route cases (`stimmen die Summen?`, `rechnungsposten`). Residuals documented (known-limitations): word-anchored offer under-fires German closed compounds (routing broader), the `meeting`-word offer ceiling, redaction `datenschutz`/`dsgvo`/`gdpr` suggest-only (§4.4/U4). +55 net-new tests; full suite green (2932) + typecheck. Branched off `fix/skills-w4`. `fix/skills-w5` |
| U1 | Honest completeness (droppedRowCount + badges + fence trim) | P2 | R2 | M | [x] — additive `dropped_row_count` on `bank_statements`+`invoices` (nullable; pre-U1 rows omit the gate) counts money-bearing lines the parser REJECTED (`hasMoneyToken`; a leading BOOKING DATE required on the bank path so an FX-reference/Valuta continuation or a money-less header is excluded — the geometry multi-baseline false-positive); >0 ⇒ the answer swaps the "whole statement/invoice" headline for `countPartial` ("N read; M with figures I couldn't parse"), and a D56-`contradicted` statement uses `countContradicted` (kills the self-contradicting count line over the no-total body). Currency-adjacent bare-integer read extracted to the shared `lastCurrencyAdjacentInteger` (money.ts) + reused by invoice `totalsMoney` AND extended to bank `lastMoneyOnLine` (a round `Opening balance 914 $` now feeds §3.5/D56). Both extractor versions→8. "Every match found…" extract badge softened to "Read across…" (ux-10); empty-extraction copy blames the reader + names the next step (OCR / not-machine-readable, en+de du-form, ux-11). All 8 SKILL.md bodies reordered honesty/safety-FIRST (survive fence trimming); `buildSkillFence` `trimmed`/`omitted` now LOGGED (`logSkillFenceReduction`, ids/counts only) at all 3 budget call sites instead of discarded. money.ts touched for the shared helpers (outside the listed files — noted). +23 net-new tests (droppedRowCount bank+invoice, currency-adjacent balance ±sign, countPartial/countContradicted analysis, fence-log, SKILL.md rules-lead ×8, badge soften); full suite green (2957) + typecheck. Residuals: round-integer-only lines uncounted (MONEY_RE definition); fence flag logged not badged (renderer out of scope). Branched off `fix/skills-w5`. `fix/skills-u1` |
| U2 | PII detectors: redaction gaps + share-safe pre-pass + dry-run | P2 | — | M | [x] — Luhn-checked `[CARD]` detector (13–19 digits; compact/spaced/dashed; masked after IBAN, before date/phone); 0-leading phone guard (separator-less ≥9-digit run = reference number, left unmasked — fixes the share-flow invoice corruption); either-order + 2-digit-year date masking (US `12/31/2026` + `01.02.26` now mask; BL-N6 leak closed; over-masking is the redaction posture); read-only `scanRedactionCandidates` (reuses `redactText`'s pipeline → counts identical to a real run) used TWICE — (a) the redaction handler's informational dry-run (a PII-topic ask with NO action verb over a single doc → per-category COUNTS, never a value; action asks keep the button deflection; `applies()` OR-s an `isInformationalPiiQuestion` predicate so declined German "personenbezogenen" that the route vocab misses still fires — no W5/U4 vocab churn), and (b) `scanWholeDocumentForPii` (all chunks, de-overlapped) injected into the share-safe grounded prompt via `buildShareSafeScanBlock` — counts summary + a verdict gate that FORBIDS "Likely low risk after review" when the read was truncated (handler flag `injectPiiScan` → `wholeDocumentPiiScan` opt → `buildGroundedPrompt` analysis block; SKILL.md §1 wording + prompt rule both gate). `card` added to counts contract/schema (contained — only `totalRedactions` consumed downstream); NO extractor-version bump (redaction is read-side, no persisted extract). +19 net-new tests (card Luhn/grouping, 0-leading reference guard, either-order/2-digit dates, scan parity, dry-run en+de/action/multi-doc, scan-block gate ×truncation, whole-doc scan across chunks); full suite green (2972) + typecheck. Residual: share-safe over a tree-rescued (over-budget deep-index) doc doesn't carry the pre-scan block (edge case; its own coverage stamp still marks truncation). Branched off `fix/skills-u1`. `fix/skills-u2` |
| U3 | Skill-selection UX: per-turn apply + undo + relay pinning | P2 | — | M | [x] — a composer pick now applies PER-TURN (session override, not the persisted `active_skill_id`); new `turnSkillArgFor` sends the pick verbatim (id / explicit null=no-skill-no-autofire / undefined=resolve saved default+maybe autofire), and `stream()` dropped the fresh-send `null→undefined` collapse so the chip (`skillFor`) and the answer can't disagree. Persistent composer chip with × (`clearSkill`=pick None + clear DB default) + an explicit "keep for this conversation" checkbox (`onKeepForConversation`, the ONLY writer of `active_skill_id`; un-keeping pins a session override so chip+send stay consistent); pre-existing sticky defaults keep working + read as kept. "Answer without this skill" undo extended to EVERY skill-stamped last turn (not only auto-fired). ux-6: the routed-run relay (Summarize/Categorize→transcript answer) pins `askDocuments` to the run's resolved document (`runTargetId`, module-store `getActiveSkillRunDocumentId` remount fallback → new `pinnedDocumentId` 5th arg; re-validated main-side, out-of-scope ignored, `hasExplicitDocSelection` set so filename auto-scope defers) so a multi-doc scope can't scatter it; the two routed buttons + the post-extract categorize follow-up are hidden in plain-chat mode (`visibleRunnableTools` + `SkillRunBar offerRoutedFollowups`). Attach-flow carry made per-turn too. NO routing-engine change. +17 net-new tests (incl. 4 ChatScreen-mount: pick never persisted / keep is the only persister / None clears a saved default / chip × clears both) + 2 pre-existing expectations updated to the fixed behavior; full suite green (2989) + typecheck. Adversarial 3-lens diff review caught + fixed 2 real bugs: routed-pin resolved AFTER `acknowledgeSkillRun()` nulled the store (dead remount fallback → resolve before acknowledge), and `selectSkill` left a saved default lurking so None couldn't clear it + the keep-checkbox shadowed it (→ `selectSkill` clears the default, keep is the SINGLE writer). Branched off `fix/skills-u2`. `fix/skills-u3` |
| U4 | Auto-fire reach: narrowed signals + skill opt-ins | P2 | W5 | S | [x] — auto-fire doc signals NARROWED to explicitly-scoped docs (`inScopeDocSignals(…, {explicitDocumentsOnly})`: whole-corpus Library/collection contributes nothing; suggest.ts keeps full scope — §4.4); redaction manifest↔handler ALIGNED (dropped `datenschutz`/`dsgvo`/`gdpr` — handler routes on neither `routeMatch` nor `PII_TOPIC_RE`; kept `sensitive data`/`sensible daten`); bank-statement/invoice/meeting-protocol opted into `triggers.autoFire` (setting stays default-OFF). Eval: harness models the narrowing (`toAutoFireContext` + `narrowsDocSignals` on the threshold-3 policy; monotonicity test pinned to full signals); corpus +8 whole-corpus/opt-in items (90 total). Measured — **auto-fire threshold-3: fired-wrong 0 / precision 100 % (55 fired-correct, incl. whole-corpus shapes); suggestion keyword-required: precision 98.4 %**. +13 net-new tests (DB narrowing incl. `inScopeDocSignals` full-vs-narrowed proof, real-manifest opt-in ×3, whole-corpus vs attached contrast, dsgvo-drop, eval U4 gate block); full suite green (3004) + typecheck. Branched off `fix/skills-u3`. `fix/skills-u4` |
| U5 | Copy & i18n sweep + export-dialog metadata | P2 | — | S | [x] — de assistant-voice swept to du (bank/invoice analysis + routing/redaction answers) + a lint guard (formal `Sie`/`Ihr` in `skills.*Analysis.*`/routing keys, pronoun-`Sie` tolerant); `needsExtraction` names the domain-correct extract button (`{button}`, quoted plain text); the "tools run only when you start them" claim corrected to honest "read-only may auto-run / writes+exports confirm" in the settings note + BOTH SKILL.md + user-guide; per-export save-dialog metadata (`SaveFileDialogMeta` content-free keys bound in the `tool-runs.ts` dispatch, resolved in `saveTextFile`) — JSON→`.json`, XML→`.xml`, redaction→"Save redacted copy" `.txt`, CSV default (kills the one-CSV-dialog-for-everything §6.2 drift; run.ts/invoice-run.ts seams untouched — the dispatch wraps `saveTextFile`); invoice last-chunks citation stopgap (head + `TAIL_CITATIONS` tail so the totals page is cited on long invoices); ephemeral `kind:'analysis'` "reading the document…" notice on exhaustive-handler start (compaction channel reuse, cleared on first token, never buffered). Supporting edits beyond the file list (noted): `shared/ipc.ts` (+`kind`), `chat-stream.ts` (`SendCompaction` kind), `ChatScreen.tsx`/`Transcript.tsx` (`compacting`→`progressNotice`), `run.ts` untouched. +10 net-new tests (du guard ×3, needsExtraction button, dialog metadata ×4, citation stopgap, analysis notice); full suite green (3014) + typecheck. Adversarial 3-lens diff review. `fix/skills-u5` |
| A1 | Parameterized run seam (fold invoice-run into run) | P3 | R3 | L | [x] — strictly behavior-preserving (green-to-green, 3014 unchanged). `invoice-run.ts`'s ~500-line layer-for-layer copy folded into ONE generic engine in `run.ts` (`runDomainExtractionInner`/`prepareDomainRun`/`domainPersistFailure`/`runDomainFileExport`) driven by a per-domain `DomainRunConfig`; `run.ts`=`BANK_RUN_CONFIG`+adapters, `invoice-run.ts`=`INVOICE_RUN_CONFIG`+adapters+the ONE authoritative exported `loadInvoice`. R3 staleness now lives in the SINGLE shared prepare path. Analysis plumbing (`singleInScopeDocument` ×3, `shouldFallThroughOnEmpty`, `computeCoverage`, `fmt`, citation query+`[Sn]` projection) lifted to NEW `analysis/common.ts`; per-domain citation SELECTION (bank page-narrow+head / invoice head+tail) kept, calling the shared helpers. Adversarial divergence audit (7-agent workflow) found NO STOP-and-report correctness bug; the one incidental difference (downstream ctx reader: bank lazy chunk-reader vs invoice eager segment-read, inert for structured-input tools) is PRESERVED via `buildDownstreamReader` config + flagged as a follow-up, not fixed. Adversarial diff review confirmed key-for-key faithful result shapes. Branched off `fix/skills-u5`. `fix/skills-a1` |
| A2 | Self-describing tool registry + per-doc run controller | P3 | U5 | L | [x] — NEW `shared/skill-tools.ts` single-sources one `SkillToolDescriptor` per wired tool (`labelKey, seamKind, confirm, resultShape, doneKey/reconcileKeys/redactionKeys, dialog`), pure-data + importable from BOTH processes; `WIRED_TOOL_NAMES`, the `buildToolRunner` guard, the renderer label/done maps (`SkillRunBar`) and every export's save-dialog now DERIVE from it (killed the parallel hardcoded lists + the two locale-adjacent copy maps). `ToolRunOutcome.transactionCount`→generic `count` (additive; `transactionCount` mirrored on `SkillRunState` one release; renderer reads `count ?? transactionCount`). `SkillRunController` scoped **per-document** (`Map<documentId,ActiveRun>`) — "a skill is already working" fires only for the same document, so unrelated chats run concurrently (the doc-lock/PC-1 is the real same-doc serializer). `resolveEffectiveTools(declared, declared)`→`resolveWiredTools(declared)` (grant leg deleted — no grant UI ever shipped; documented in security-model.md + architecture.md §7-A2). Behavior-preserving except the per-doc concurrency + dialog metadata; +9 net-new tests (descriptor↔registry parity ×5, per-doc concurrency + `count` alias, dispatch parity); full suite green (3023) + typecheck. Branched off `fix/skills-a1`. `fix/skills-a2` |
| A3 | Manifest analysis mode + whole-doc gate inversion | P3 | W1 W2 W5 | L | [x] — additive manifest `analysis: whole-doc\|compare\|none` (default none, lenient, instruction-only — a tool skill's declaration is noted+ignored, SEC-1 unchanged) honored for instruction skills of ANY source via `manifestAnalysisHandler` (a user instruction skill reaches the SAME engine; the bundled 5 declare it in SKILL.md, pinned to their registered handler mode by a consistency test). Gate INVERTED (`registerRagIpc` + `whole-doc-skills.ts`): with an analysis-mode skill over a matching fully-chunked scope the whole-doc/compare engine is the DEFAULT — per-skill `routeMatch` retired as the deciding gate. Keywords now skill-agnostic: `isSmallTalk` opts out chatter, `isNeedleShaped` sends a lookup to top-k ONLY when the whole-doc read would truncate AND no tree exists (W1's exact budget calculus is the input; deliverable asks never downgrade). security-model.md restates analysis = engine choice ≠ capability. Adversarial 6-lens diff review (each finding independently verified) → 3 confirmed, all fixed: MEDIUM needle-downgrade fall-through hit the coverage-extract listing and dropped the W2 scope notice + skill stamp (→ that branch now leads the notice + stamps the skill), MEDIUM `isNeedleShaped` false-fired on `what is the {takeaway/bottom line/Fazit/Kernaussage…}` synthesis asks (→ DELIVERABLE veto extended with the synthesis nouns EN+DE), LOW the security-model.md test citation named a user-skill routing test that didn't exist yet (→ added the end-to-end `rag-whole-doc-skill` user `analysis:whole-doc` routing test + corrected the citation). +33 net-new tests; full suite green (3056) + typecheck. Branched off `fix/skills-a2`. `fix/skills-a3` |
| T1 | Eval & fixture infrastructure + real-model smoke | P1½ | R1–R2 | M | [x] — NEW `tests/fixtures/real-layouts/corpus.ts` single-homes the incident-class fixtures (constructed AT/DE/CH statements+invoices, never real data, special chars as `\u` escapes): NBSP/narrow-NBSP, U+2212/en-dash, Summe/Endbetrag/Rechnungssumme labels, SEPA rows, dd.mm.yy+cross-year, wrapped descriptions — run through the REAL production extractors (currency-vote+order+anchor+balances, mirror of the tool call) with the parsed figures asserted (incl. the R2 CRITICAL: `Steuerberatung Jänner`→line item while `Steuer 20%`→taxTotal). NEW `tests/integration/extractor-realworld.test.ts` = the figure assertions + an output-SNAPSHOT guard (`extractor-output.snapshot.json`, per-fixture hash of the full output keyed by `*_EXTRACTOR_VERSION`): any output change FAILS the default suite unless the version was bumped AND the snapshot regenerated (`UPDATE_EXTRACTOR_SNAPSHOT=1 …`) — proven to fire (mutation-verified) with a distinct no-bump vs bumped-regenerate message; plus a corpus-coverage assertion (every incident class represented). NEW env-gated opt-in smoke `tests/e2e-model/skills-smoke.test.ts` (`SKILLS_SMOKE_MODEL=<gguf>`, `describe.runIf` → COLLECTED but SKIPPED in CI): one bank + one invoice (third-mode grounded-data over the corpus, real model narrates the verified extract + deterministic echo) + one German whole-doc minutes turn; asserts structure/figures (mode engaged, postscript rides under the answer, capped+not-truncated coverage, tail items), never prose. Nothing in `npm test` needs a model/network. Surfaced (documented, NOT fixed — no extractor change in a test-infra phase): `USt` ∈ TOTALS_FILLER but ∉ TAX_LABELS (standalone `USt … EUR` reads as a phantom item; spelled-out labels parse). Docs: model-benchmarks.md §10 + known-limitations.md. Full suite green (3068 pass / 44 skip incl. the 3 smoke) + typecheck. Branched off `fix/skills-a3`. `fix/skills-t1` |
