...
- **`services/doctasks.ts` — the shared task engine.** A job state machine on the Phase-4/18
  ...
  `compare`); only `summary` is implemented — Phases 34/35 plug into the same engine. Deps
  are injected (`getDb`, `getRuntime`, `isChatStreaming`, `getContextTokens`, `audit`), so
...
- **No `beginDocumentWork()` lease, deliberately:** a summary only READS chunk rows and
  WRITES one DB column — it never touches `.enc` document sidecars, so it does not need
  the Phase-32 vault lease. Instead, `registerDocsIpc` refuses re-index/delete of a
  document an active task targets (`isDocumentBusy`) so a task can't persist a result
  derived from replaced chunks. Future kinds that materialize documents (translation/
  compare, D27/D28) WILL take the lease.
- **IPC + UI:** `doctasks:start/get/cancel` (+ preload mirrors). The renderer watcher
  (`renderer/lib/doctasks.ts`) lives at module level so a running task's busy/progress
  state survives screen navigation; the Documents screen polls it (`useSyncExternalStore`),
  shows the per-row "Summarizing… (n/m)" busy state + Cancel, and on completion opens the
  preview, where the summary renders as a collapsible section with a
  "Generated by <model> · <date>" attribution line and a Regenerate action.
