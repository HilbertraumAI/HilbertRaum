...
### §50 Full audit (2026-07-16) — remediation ledger + close-out

The first full audit of the repo **as a public project**, at baseline `4e02a48` (v0.1.50, the
PR #56 newest-Qwen promotion merge; suite 4,217/49 — see the baseline correction below). Method:
a known-open register built first (BUILD_STATE §5/§6/§8, known-limitations, the §46–§49
residual rows, build-log riders, live `gh issue`/`pr` lists — finders marked matches instead of
re-reporting); **ten dimension finders** (12-finding quality cap each) over security · backend
pipeline · ingestion/sidecars · IPC/preload · renderer · reliability · performance · test suite
· docs · build/packaging; **every finding independently adversarially verified** (3 refuted and
dropped; every survivor carries verifier-corrected `file:line` + a verified blast radius).
Verdict: **0 Critical, 0 High — the first full audit round of this repo in which no
Critical/High finding survived verification**; 41 verified findings **F-01…F-14 (Medium) ·
F-15…F-41 (Low)**. Remediated across a **ten-phase wave on stacked branches**
`fix/audit-2026-07-16-p1…p9` (owner decision D-E; the first audit wave under the public-repo
no-direct-master-push rule — §46–§49 committed straight to `master`, this wave merges ONCE via
a wave-close PR with green `ci-success`), P1 `e8e1313` → P9 `886be68`; Phase 10 = this
close-out. Phases ran as fresh-context sub-agents of orchestrator sessions; Phases 4–6 (the
behavior-changing ones) each passed an **independent review before acceptance** (outcomes
below). Gate progression **4,217/49 → 4,271/49** (+54: 4,217 P1 → 4,219 P2 → 4,221 P3 → 4,233
P4 → 4,247 P5 → 4,265 P6 incl. its review test → 4,267 P7 → 4,270 P8 → 4,271 P9), typecheck
clean throughout, build green on every phase that touched `apps/desktop/src`. **Baseline
correction (Phase 0):** the plan's "4,216/49" figure predated PR #56's +1 promotion-facts pin —
the true master-HEAD gate was **4,217/49**, and every docs-only phase matched that number.

**Unlike §46–§49, the working papers survive in git history.** Both were committed on the wave
branch at kickoff (`8bf0fc7`) precisely so the close-out deletion loses nothing: the frozen
...
① **D-A** — add the UTF-8 BOM to `.csv` exports (Excel is the primary consumer of a
transactions CSV; the no-BOM posture was only pinned, never a recorded decision) → executed
P6/F-10. ② **D-B** — relabel/drop the cached drive-READ figure; `driveWriteMbps` stays the
headline; no cold-read build; already-persisted inflated values must render sanely → P8/F-35.
③ **D-C** — approve the async image-history port (the §35 PERF-1 divergence decision predates
the async twins; it gets a dated supersede annotation, not a rewrite) → P8/F-12. ④ **D-D** —
the CSP `'unsafe-inline'` investigation pre-authorized both ways (drop if KaTeX/streamdown
render styled without it, else keep + document) → P8/F-39 (verdict: KEEP). ⑤ **D-E** — stacked
branches + ONE wave-close PR; full local gates per phase compensate for the deferred CI signal
→ the wave's execution model.

Per-finding disposition (fixed → phase@commit; ⟨D-x⟩ = executed owner decision; review
fine-print folded into its rows):

| # | Sev | Phase | Disposition (decision / mechanism) |
|---|---|---|---|
| **F-01** | MED | P4 `96cb06e` | **fixed** — the structured-extract scan cache was content-keyed only, so a chat-model swap never refreshed extractions even on an explicit re-run (quality bugs got attributed to the new model): the markerExists cache-hit lookup gains `AND model_id = ?` (the current pass's model). `contentHashOf` deliberately untouched — the `analysis-extract-hash.test.ts` byte-identity pin stays green and persisted rows stay addressable; rows are replaced (delete-then-insert), never mixed; a same-model re-run stays 0-call (the #50 economy holds). Red-then-green on record (re-demonstrated after a test-stub correction, ledgered honestly). |
| **F-02** | MED | P4 `d57cfd9` | **fixed** — an in-band SSE error frame mid-stream was silently discarded: a failed generation persisted as a clean, complete answer, and a silently truncated compaction summary WAS checkpointed (corrupting later turns' context) — the one finding cutting against the product's honesty engineering. `parseSseLine` now recognizes BOTH failure carriers (a `data:` JSON with a top-level `error` object AND the bare `error: {…}` SSE field line — the TA-4 M3 carrier, completion.ts parity) and `readChatSSE` REJECTS with a typed content-free `ChatStreamError` (incl. the flushed-tail no-trailing-newline path); frame shapes pinned with b9849 provenance + the TS-3(a) re-verify rider. **Consumer semantics decided + test-pinned per consumer:** main chat turn PROPAGATES (partial never silently persisted; regenerate's F2 guard restores the prior reply; the `truncated` badge deliberately NOT used — its copy claims a context-limit stop); a mid-way-failed compaction summary writes NO checkpoint (turn answers on the L1 fallback); both grounded RAG paths propagate under the same `withChatStream` wrapper; new friendly `main.chat.streamError` EN+DE (structural reason to the local log only); skills categorizer/enricher/locate flows now fail-the-turn instead of completing degraded on a mid-generation server failure (deliberate, ledgered). Scoped out, recorded: a graceful close WITHOUT `[DONE]` and without an error frame keeps today's clean-end semantics (a terminal-frame requirement would change abort/close semantics for every consumer — its own future finding if wanted). Real-server error-frame smoke owed → consolidated checklist item (i), below. |
| **F-03** | MED | P3 `16b6438` | **fixed** — `prepare-drive.sh --with-assets` aborted on stock macOS bash 3.2 (empty-array expansion under `set -u`, the exact M23 class already fixed in the sibling scripts): `COMMON_MODEL_ARGS` expansion at both call sites → the repo's own M23 idiom `${arr[@]+"${arr[@]}"}`; file swept — it was the only empty-capable array. Genuine bash 3.2 was not available on the dev box (GNU bash 5.3 tolerates the bare expansion) — the fix is argued by idiom correctness + the fetch-models.sh:161 precedent, stated honestly. |
| **F-04** | MED | P3 | **fixed** — `fetch-models.sh` aborted the whole batch on the first download failure (`set -e` + `return 1`): both `handle_file` call sites now failure-tolerant (`… \|\| true`; `had_failure` still aggregates and drives exit 1) — parity with fetch-models.ps1 (continue + summary + exit 1). Teeth shown against the REAL script: 3 forced-bad-URL manifests — pre-fix aborts after the first, fixed attempts all 3, prints `Planned 3 \| fetched 0 \| skipped 0`, exits 1. |
| **F-05** | MED | P3 | **fixed** — every DIY `--with-assets` drive shipped WITHOUT the OCR language files, with no in-app path to fetch them (the root cause behind open issue #59): the `ocr` family fetch added to the `--with-assets` block in BOTH prepare-drive siblings (unconditional like llama.cpp; deu+eng traineddata ≈4 MB — inside the ~10.4 GB headline's rounding, figure deliberately unchanged); new direct sh↔ps1 parity net in `prepare-drive-default-set.test.ts` (no canonical TS plan exists for the with-assets fetch-call set — verified); packaging.md + README now enumerate the OCR files; dry-run parity confirmed on both shells. Existing DIY drives top up via `fetch-runtime --family ocr`. The in-app warning-copy half of #59 = Q-1 (resolved P7, below); the both-halves #59 comment posts after merge. |
| **F-06** | MED | P2 `293a0e4` | **fixed (per §D default)** — the qwen3.5-9b-q8 manifest was internally incoherent (`recommended_context_tokens: 98304` vs `recommended_min_ram_gb: 14` — the RAM start gate passed machines the resulting `--ctx-size` cannot fit): ctx → **8192**, the catalog convention (capable owners re-raise via the in-app Settings override); the manifest's sizing comment records why (96k f16 KV ≈12 GB + Q8 weights ≈8.9 GB ≈ 22 GB > 14). New invariant over EVERY chat manifest: `recommended_context_tokens ≤ recommended_min_ram_gb × 2048` (red-verified against the pre-fix value). The rank-0 posture and the separate G3 license-provenance review untouched (not conflated). |
| **F-07** | MED | P1 `e8e1313` | **fixed** — docs/benchmark.md still documented the pre-promotion recommendation table as the live mapping → the four §6.4 tiers (≤12 → qwen3.5-4b-ud-q4kxl, 16–20 → qwen3.5-9b-ud-q4kxl, 24 → qwen3.6-27b-q4, ≥32 → qwen3.6-27b-q5); the bundled-default note (qwen3-4b) kept separate; the rank-rationale link re-pointed §6.2 → §6.2–§6.4. |
| **F-08** | MED | P1 | **fixed** — the §6.4 promotion record and both promoted manifests credited Ministral with qwen3-8b's i9 tester row (.3262/.9765), inverting the committed 9B-vs-Ministral F1 relationship — and the pending BUILD_STATE §5-item-8 owner ratification would have been made against bad data. Corrected from the committed CSVs (Ministral's real row: F1 .3111 / EM .9529, i7-1185G7 Phase-29 rescored run, cross-run caveat); the 9B standing restated — it EDGES Ministral on F1/EM within cross-run tolerance and is ranked under it only on the hallucination-trap axis; the "~1 point EM" claim → the real 2.36-pt tier spread; the 4B EM tied at .9647. Correcting the incumbent baseline *strengthens* the promotion. |
| **F-09** | MED | P1 | **fixed** — model-policy's "Qwen3.5 Unsloth wave" section and model-benchmarks §9 still asserted present-tense that every Qwen3.5 manifest is rank 0 / never auto-recommended / smoke-unconfirmed: §6.3-style dated supersede annotations added (4B/9B promoted to rank 3 on 2026-07-12; 27B/35B-A3B remain rank 0; the 4B smoke satisfied 2026-07-12, the 9B/27B §9.1 smokes still open). Annotated, never rewritten. |
| **F-10** | LOW | P6 `9fb73cb` ⟨D-A⟩ | **fixed (owner-decided posture change)** — skill/table CSV exports carried no UTF-8 BOM (German umlauts → mojibake in Excel, the primary consumer): `bomFor` covers `.csv`; `registerSkillsIpc.saveTextFile` prepends it (`tableToCsv` stays pure). BOTH known no-BOM pins **plus a THIRD audit-unlisted one** (`skills-tool-run-ipc.test.ts`'s anchored `/^date,…/` header regex — §N-a deviation) flipped WITH the fix in the same commit (the owner-decided posture change, not assertion weakening); BOM'd-CSV re-import round-trip proven through the real CsvParser (papaparse strips the input BOM). Deliberate side effect, taken knowingly: `redacted.txt`/`edited.txt` (same boundary) now get the P4-mandated `.txt` BOM; JSON/XML/log stay BOM-free. known-limitations gains the trade-off bullet; this doc's result-tables + P4 records dated-annotated in-commit. |
| **F-11** | MED | P6 `facacad` | **fixed** — the docx-rewrite text-layer regex mis-parsed a self-closing `<w:t …/>` node (read as an OPENER; the lazy body swallowed all markup to the next `</w:t>`, and a span overlapping the pseudo-node re-emitted that markup xmlEscape'd as visible text — DOCX corruption on the redaction/edit same-format export): an explicit self-closing alternative BEFORE the paired-tag one, skipped as an EMPTY node (D77 byte-identity preserved); red first on both symptoms; the bare `<w:t/>` skip pinned as a regression case. |
| **F-12** | MED | P8 `88dd8f5` ⟨D-C⟩ | **fixed (owner-approved port)** — image-history store/open/delete ran up to ~60 MiB of SYNCHRONOUS fs+crypto+shred on the Electron main thread per image, the store firing inside the `done` emitter (protected until now by the §35 PERF-1 divergence decision, which predates the async twins): ported to `encryptFileAsync`/`decryptFileAsync` + `fs.promises` + a new `shredFileAsync` twin in `workspace-vault.ts`; all three history functions async; `registerImagesIpc.ensureSession` awaited BEFORE `base.done` so sessionId still rides the done event; REL-5 delete ordering (row-first, shred-after) preserved; the sync twins retained for crash-lock/export/DB-lifecycle. The §35 PERF-1 sync carve-out retired via a dated supersede annotation. **Measured** (dev SSD, 16 MiB encrypted store ×5, `monitorEventLoopDelay`): main-thread stall max 94.8 → 24.1 ms, mean 22.4 → 3.37 ms. The USB-stick + encrypted-workspace target run was not reproducible on the dev box — recorded honestly, no USB numbers fabricated (the stall scales with size/throughput, so the win is proportionally larger there). NF-1 rode this fix (below). |
| **F-13** | MED | P5 `eb50209` | **fixed** — resuming a COMPLETE `.part` sent an unsatisfiable `Range: bytes=<fullSize>-` → HTTP 416 failed every subsequent attempt with no in-app recovery (a completed-but-unrenamed `.part` bricked that download permanently): a complete `.part` is now verified IN PLACE — a pre-download short-circuit when `size_bytes` is known (`resumeFrom >= sizeBytes`, fetch never called) or a caught typed `RangeNotSatisfiableError` when unknown; a hash match (or placeholder) renames into place, a mismatch discards the `.part` and restarts ONE clean download (`allowResume=false`, the ResumeOffsetMismatch treatment); shared `finishVerifiedFile` keeps the fresh-download and complete-part paths identical. **Review fix-up `f59359c` (should-fix):** the 416-CATCH-path settle had no exception protection — a throw inside it (AV-interfered rename) escaped as an unhandled rejection and stranded the job at non-terminal 'verifying' forever; now any settle throw lands the job 'failed' with `friendlyDownloadError` + the audit hook (red-verified via an injectable verifyImpl). model-policy + data-contracts resume sentences gained the verify-in-place clause (review nit). |
| **F-14** | MED | P5 `7e55c6f` | **fixed** — a vision sidecar mid-session crash left a dead server handle (the TA-6 M1 crash-recovery fix was applied to TranslationRuntime but never ported): every later image question failed `runtimeFailed` for the full 120 s idle window, each retry re-arming the timer against the corpse. M1 ported verbatim — an identity-compared `onUnexpectedExit` (`if (this.server === server) this.server = null`) so the next `analyze()` cold-starts; NO device-fallback twin (vision has no GPU/CPU ladder); translation's two M1 test cases mirrored, the crash→cold-start case red first. |
| **F-15** | LOW | P6 `5677f90`+`18796f9` | **fixed** — citation snippets were cut by UTF-16 code units and could end in a lone surrogate (the exact RAG-2 class, fixed elsewhere but missed at two persisting sites): new LEAF module `services/text.ts` (`codePointSlice` + `truncateByCodePoints`) — leaf because rag imports coverage, so coverage importing rag's `truncateSnippet` would cycle; both persisting sites (`coverage.ts` documentLeafProvenance, `common.ts` chunksToCitations) cut at 280 CODE POINTS; cosmetic `compare.ts` oneLine converted; rag's `truncateSnippet` delegates byte-identically (the RAG-2 pin untouched-green). The P-6 SQL `substr(text,1,281)` sentinel still fires — SQLite substr counts code points and the JS guard now does too, proven against REAL `node:sqlite`. `18796f9` = optional-`Citation.snippet` typecheck narrowing in the new tests (assertions unchanged). |
| **F-16** | LOW | P2 | **fixed** — the Qwen3.6 27B pair recorded `size_on_disk_gb` in GiB while the rest of the catalog uses decimal GB (mixed units feeding the picker tiebreak and the Models-screen display): 15.7 → **16.8** / 18.2 → **19.5** (= `size_bytes/1e9`); the README + model-policy display cells and both manifests' GiB-labelled comments moved in the same commit. New invariant: `\|size_on_disk_gb − size_bytes/1e9\| < 0.15` for every manifest with a real download block (red-verified: 1.117 pre-fix). The recommendation mapping re-verified unchanged (`benchmark.test.ts` byte-identical; the size tiebreak is not decisive for either 27B). |
| **F-17** | LOW | P1 | **fixed** — stale `license_review.notes` on the promoted Qwen3.5 4B/9B manifests still stated the promotion preconditions were unmet: a dated `UPDATE 2026-07-12:` line appended to both, recording the smoke + rank-3 promotion (original text preserved — audit trail; the 27B "Supersedes…" style). YAML values untouched. |
| **F-18** | LOW | P3 | **fixed** — setup-dev.ps1's `--use-system-ca` probe redirected native stderr under `$ErrorActionPreference='Stop'` (PS 5.1 wraps redirected native stderr in ErrorRecords → on a Node without the flag the probe THREW instead of falling back): replaced with the redirect-free introspection `node -p "process.allowedNodeEnvironmentFlags.has('--use-system-ca')"`. Teeth under PS 5.1 + EAP Stop: the new probe works on real node AND a flag-less stub (fallback branch, no crash); the old shape crashes on the same stub. |
| **F-19** | LOW | P3 | **fixed** — fetch-runtime.sh derived the archive filename with `basename(URL)`, keeping any `?query`/`#fragment` (latent: the extraction dispatch keys on the name, so an HF-style `?download=true` would have mis-routed a `.tar.gz` to unzip): strips both after basename, converging with fetch-runtime.ps1 (`[uri].AbsolutePath`) and assets.ts. |
| **F-20** | LOW | P1 | **fixed** — data-contracts.md still said "19 model manifests (14 chat)" (24 total / 19 chat at HEAD — a regression of the §48 DOC-3 fix): both counts replaced with a pointer to the authoritative model-policy catalog (the structural 6-role-dir enumeration kept) so the number cannot rot a third time. |
| **F-21** | LOW | P1 | **fixed** — model-policy said the Context-size card offers "presets 4k–32k"; the shipped picker goes to 128k (issue #43) → "presets 4k–128k". |
| **F-22** | LOW | P6 `c5297f9` | **fixed** — the Markdown parser (and the txt parser) did not strip a leading UTF-8 BOM, so the first heading of a BOM'd `.md` file lost section detection — including files the app ITSELF exported (its `.md` exports prepend a BOM via `bomFor`): one `replace(/^\uFEFF/, '')` after readFile in both parsers; round-trip proven with the real `bomFor` prefix (section labels `['My Chat', 'Sub']`, was `[null, …]` — the watched red). |
| **F-23** | LOW | P6 `750860d` | **fixed** — `wordDiff` on two empty/whitespace-only texts returned null ("too different") via an out-of-bounds Int32Array read in the Myers d=0 iteration: early-return of an identical/empty DiffResult when BOTH token arrays are empty (chosen over resizing the array — it also states the contract); `('','')` and `('   ','\n')` red first; empty-vs-nonempty pure insert/delete pinned. Latent-only today — both compare entry points gate on non-empty text (verified reachability). |
| **F-24** | LOW | P6 `e3a2ef2` | **fixed** — chunker character slicing could split an astral (surrogate-pair) character at window boundaries, landing lone surrogates at the edges of STORED chunk text for glued emoji/astral runs (U+FFFD in SQLite/FTS/embeddings): `atomize`'s over-long-word slice loop now RETRACTS the cut one code unit when it lands mid-pair (EXTENDS only in the degenerate sliceChars=1 case where retracting would empty the piece — retract-not-extend is deliberate: a shorter piece can only cost fewer tokens, preserving the never-over-budget guarantee). Boundary-only: BMP text hits byte-identical cut positions — chunk-count no-churn pinned in-file with pre-fix-computed counts; no migration (existing corpora re-chunk only on re-index, documented). **Review addition `748fc06`:** the extend branch had no driving test (needs a cap/overlap-coprime config) — added `windowByTokens('😀'×30, 7, 3)`; TEETH by mutation: flipping the branch to a plain retract HANGS the slice loop (the honest red — fork killed at ~38 s), restored byte-identical. The SIBLING approx-token UNDER-COUNT for astral runs stays accepted (known-limitations L10, narrowed in-commit). |
| **F-25** | LOW | P7 `bbefe6e` | **fixed** — the translate-view lock/quit purge (`TranslateJobService.stop()`) was a THIRD stream terminal the FA-1/F-4 detach missed: it aborts + clears the job map but emits neither trDone nor trError, and a lock does NOT destroy the window — so each lock-during-in-flight text translation leaked one `destroyed` once-listener + one detachers entry (MaxListenersExceededWarning at ~11). New `onStop(listener)` purge-observer hook on the service; `registerTranslateIpc` subscribes and runs every outstanding detach. Chose the purge hook over routing through the error terminal: a single observed terminal beats reconstructing per-job emitters, and lock/quit already call `stop()` — no new call site to forget (the exact bug class F-25 is). Red first (listener count stayed 1 after stop()) → green (0). The F-4 disposition record in this doc annotated. |
| **F-26** | LOW | P7 `ad1a176` | **fixed** — `visionSession.analyze()` kept the pre-TA-3 busy guard (`activeJobId` only, which isn't set until AFTER the create round-trip): a second analyze in that window slipped through, main busy-rejected it, and its busy branch's `set({analyzing:false})` clobbered the still-live first job's flag (re-enabling the composer/drop-zone mid-stream, where a dropped image cancels the live analyze). Translate's L6a guard ported verbatim: `activeJobId \|\| analyzing`; the mirrored store test red first (second returned 'started', flag went false) → green ('busy', flag stays true). |
| **F-27** | LOW | P1 | **fixed** — the preload startDocTask contract comment still said translation languages come "from the curated 10" → the closed 51-code WMT24++ set (`TRANSLATION_LANGUAGE_CODES`, issue #31). |
| **F-28** | LOW | P7 `4a29e6f` | **fixed** — preload/shared contract drift: `listDocuments`'s `smart` filter typed as a 3-member subset of `SmartListView` → retyped to the imported shared type (main's `DocumentListFilter` already accepts the full ten-member set — the bridge no longer forbids values main supports; this doc's stated contract already said `SmartListView`, so code aligned to docs); dead `ChatOptions.useDocuments` deleted (zero references repo-wide, never sent on the wire). Type-only; `npm run typecheck` is the gate. |
| **F-29** | LOW | P8 `314ce66` | **fixed** — suggestSkills re-materialized the in-scope `indexed` title+MIME set (unindexed `created_at` sort) on every debounced composer pause: a resident cache in `scope-signals` (WeakMap-per-Db, single entry) keyed by the resolved scope fingerprint + a cheap corpus signature — `(COUNT, MAX(rowid))` over `indexed` documents **and** over `document_collections` + `includeArchived`. The membership-table half is a ledgered deviation from the plan's documents-only key: the "whole-corpus" scope resolves to the built-in Library COLLECTION, so a membership move must invalidate too. Hits serve byte-identical signals; auto-fire (`explicitDocumentsOnly`) reads live; `documentsInScope` semantics untouched (the audit's ~13-call-site caution honoured — the cache sits strictly above it). Probe: 5 pauses → 1 materialization, +1 on import. |
| **F-30** | LOW | P8 `11a556c` | **fixed** — `getAppStatus` synchronously re-read + re-parsed policy.json + drive.json on every call (TranslateScreen polls it every 4 s for a whole run): `loadPolicy` caches the merged policy per config dir keyed `isDev \| fileSig(policy.json) \| fileSig(drive.json)` where `fileSig = mtimeMs:size` (or `absent`) — one `stat` per call replaces two `readFileSync+JSON.parse`; re-parse only on a signature change, so the download gate / developerLeniency still see a live policy tightening; `buildPolicyStatus` signature unchanged. Probe: 3 polls → 1 parse, +1 on a content edit. Test-seam note (ledgered for later sessions): a same-mtime+same-size in-place edit is a cache hit — use `__resetPolicyCache` or change the file size. |
| **F-31** | LOW | P8 `150fd0c` | **fixed** — bulk import refreshed the full documents list on EVERY file completion, amplifying the registered PF-5 load-all during exactly the sessions that grow a library: `DocumentsScreen.watchJob` AND `watchReindex` completion refreshes route through `makeRefreshCoalescer` (leading edge + trailing throttle `REFRESH_THROTTLE_MS = 1500`, piggybacked on the 400 ms poll ticks — no nested timer); the terminal `job.done` refresh stays immediate. The FE-7 exact-call-count pins are PRESERVED (they exercise one transition); a new test proves 3 rapid completions in one window → 1 refresh (old code: +3). The FE-7 note in this doc amended. |
...
| **F-39** | LOW | P8 `306f7bb` ⟨D-D⟩ | **investigated → VERDICT: KEEP + document** (deliberately NOT a code change) — production CSP retains `style-src 'unsafe-inline'`: rendering KaTeX proves valid math emits many per-expression inline `style="height:…;vertical-align:…"` ATTRIBUTES (`x^2 + y^2` → 11, `\sum_{i=0}^{n} i` → 7), and inline style attributes have no nonce/hash escape hatch (CSP nonces cover only `<style>`/`<link>`; `'unsafe-hashes'` cannot hash dynamic values) — dropping it would block sizing/alignment on ALL rendered math. Residual risk bounded: `script-src 'self'` blocks script injection and the closed `connect-src`/`img-src` close exfiltration, leaving only same-origin cosmetic CSS effects. The CSP string, the `window-security.test.ts` exact pin, and the index.html/ocr.html meta tags are UNCHANGED; the rationale lives in the `buildCsp` header + security-model.md. Both outcomes were pre-authorized (D-D); this is the evidence-backed one. |
| **F-40** | LOW | P9 `29427a4` | **fixed (comment) + re-capture REGISTERED** — the b9585-provenance captured fixtures (GPU `--list-devices`, vision SSE sample) were never re-captured after the runtime pin bumped to b9849, and the gpu.test.ts comment still called b9585 "the pinned build" with a false "fails in CI if upstream changes the shape" guarantee (which only holds if captures track the pin): comment corrected to the honest scope — the frozen b9585 capture reddens only if the PARSER regresses against the b9585 shape (M-A5 is observation-triggered). NO fixture bytes or assertions moved: the full b9849 re-capture (with the byte-pinned `gpu.test.ts` freeMb and `vision-sse.test.ts` split-UTF-8 assertions moving) is consolidated-smoke-checklist item **(h)** (BUILD_STATE §5 item 7, TS-3). |
| **F-41** | LOW | P9 `29427a4` | **fixed (bounded slice + standing ratchet)** — ~230 `as never` casts in test stubs eroded exactly the typed PreloadApi contract `stubApi` was built to enforce: the five heaviest stub-cast files converted to typed builders + `Partial<PreloadApi>`-typed api bags (fileTranslateSession 28→0, ImagesScreen 23→0, TranslateScreen 21→0, AppLock 12→0, translateSession 9→0 real casts; deliberately-partial/contract-violating payloads use a NARROW named `as unknown as <Type>` at exactly that value — the HomeScreenPoll pattern — never a blanket `} as never)`), so method-name/return-shape renames redden typecheck again. **New one-way ratchet** `tests/unit/as-never-ratchet.test.ts`: the comment-stripped `as never` count under tests/ may only decrease (**baseline 110**); TEETH red-green (planted cast → 111 > 110 red → removed). Top-5 re-derived by live grep with one ledgered exclusion: settings-write-gate's 15 are deliberate invalid-VALUE casts (`null as never`, `42 as never`) testing the write-gate's runtime rejection — the audit's own "triage separately" class, not stub-payload erasure. The rest is fix-when-touched (CONTRIBUTING's new typed-stub-payloads bullet; doctasksStore at 8 is the next candidate). No assertion changed by any conversion. |

**Queue + new findings (§Q/NF, all closed):** **Q-1** (found P3 — the #59 warning-copy half)
**resolved P7 `ed69824`**: the scanned-PDF/OCR dead-ends `docs.scan.ocrMissing` +
`main.task.needsOcr` gained an actionable EN+DE remedy hint (re-run `prepare-drive
--with-assets`, or `fetch-runtime --family ocr` — offline-safe, names local scripts, no URLs);
`main.ingest.imageNeedsOcr` deliberately left byte-stable (persist-canonical English — changing
it would strand legacy German rows without a legacy matcher; its remedy is covered by the
user-guide/troubleshooting cross-refs added in the same commit). **Q-2** (found P4 — the
real-server error-frame smoke owed) **resolved-by-registration P9**: folded into the ONE
consolidated smoke-session checklist as item (i), below. **NF-1** (found P8, **fixed
`7b90210`**): `tests/integration/vision-security.test.ts` was an UNLISTED pinning test in
F-12's blast radius — it asserted the `.enc` sidecar after the JOB STATE flipped, which the
async port races; moved to wait on the streamed `imgDone` event (the renderer's real signal —
the sanctioned lockstep move). The §Q table closed EMPTY at Phase 9.

**Independent-review outcomes (Phases 4–6, per the kickoff execution model; each review ran
over the phase diff before acceptance):** **P4 — ACCEPT** (2 ledger nits, fixed `0404ee6`;
the reviewer also corrected the skills-flows characterization — the categorizer's pre-fix
behavior was "degraded-but-complete", not "already failed visibly", and the post-fix
fail-the-turn change is ledgered as deliberate). **P5 — FIX-UP NEEDED**: 1 should-fix (the
F-13 416-catch-path settle protection, `f59359c`, red-verified) + 2 honesty corrections applied
docs-only (F-33's window "narrowed to the ≤2 s kill grace", not "closed"; the F-13
model-policy/data-contracts resume-sentence drift). **P6 — ACCEPT** + one nit resolved by
ADDING the stronger test (the F-24 extend-branch coverage, `748fc06`, mutation-red-proven).
Other recorded deviations, all §N-a (small, in-phase, ledgered): P1's third stale-comment copy
(F-37) and deferred optional hardening; P2's q5 comment-parity line; P6's third BOM pin (F-10)
and the F-15 typecheck fixup; P8's membership-table cache-key addition (F-29); P9's re-derived
top-5 (F-41); and the Phase-0 baseline correction (4,216 → **4,217**/49 true master-HEAD gate).

**Clean verdicts worth preserving** (from the report's executive summary + Appendix B; the full
ten-dimension inventory survives in the frozen report — pointer below):

- **First full audit of this repo with no Critical/High survivor.** The four July rounds + the
  post-launch tester waves left the core (vault/crypto, offline enforcement, RAG pipeline, IPC
  contracts, i18n, test suite, CI/packaging) demonstrably solid; the residual defect mass had
  moved OUTWARD into four clusters — sidecar parity, shell-script/DIY-drive parity,
  model-catalog data hygiene after the fast Qwen promotion, docs lagging promotions — and this
  wave closed all four.
- **Security:** an entire dedicated pass (vault, network guard, download chain, skills sandbox,
  manifest→spawn trust chain, SQL/FTS injection surfaces, IPC validation) produced exactly
  **one Low** (F-39, itself resolved KEEP-with-rationale). The hard rules re-verified clean —
  including an EMPIRICAL re-test that Node's `new URL()` canonicalizes decimal/octal/hex/
  short-form IPv4 before the SSRF deny-list regex (the classic numeric-IP bypass stays closed).
- **Contracts:** all **111 IPC channels** symmetric between preload and main with zero orphans;
  the EN/DE catalogs in exact **1,398-key parity** with zero interpolation mismatches.
- **Tests:** the suite's own hygiene claims held under spot-checking — the TS-1 "no unjustified
  fixed sleeps" claim passed **6/6** spot checks; zero `.only`/unconditional `.skip`; assertion
  vacuousness ~5/4,056 it-blocks (all env-gated harness printers); FullSuiteGuard failed safe
  on every ambiguous invocation the finder could construct.
- **Adversarial verification dropped 3 finder reports** (per-leaf persisted citations = the
  documented §14.4 design; the design-review pointers = the closed §49 DOC-7 disposition; the
  `D:` smoke-drive default = documented posture inside TS-3) — recorded in the report's
  Appendix A for transparency.

**Residuals & watch items** (registered before the papers retire):

- **Consolidated manual-smoke checklist** (BUILD_STATE §5 item 7, TS-3 — ONE list, not
  scattered notes): item **(h)** the b9849 verbatim-capture re-take (F-40; byte-pinned
  assertions move with the fresh captures) and item **(i)** the real-server mid-stream
  error-frame smoke incl. the PARTIAL-frame case (F-02/Q-2; tiny ctx + context-shift disabled,
  `HILBERTRAUM_*`; reader rejects + friendly copy; a truncated error frame parses as keep-alive
  and ends cleanly). Both run at the next manual smoke-drive session.
- **F-24's sibling** — the approx-token UNDER-COUNT on pathological glued astral runs stays
  accepted (known-limitations L10, narrowed to that half by the F-24 fix).
- **F-33's ≤2 s kill-grace** concurrent-install window (accepted residual; recorded in
  packaging.md's engine-install record).
- **F-41 fix-when-touched** under the standing ratchet (baseline 110, may only decrease;
  doctasksStore 8 is the next conversion candidate).
- **The #59 both-halves comment** posts AFTER the wave merges to a pushed sha (the verbatim
  text is parked in the plan's §L Phase-7 entry — recoverable via the git-show pointer below);
  #59 is then closable.
- **Pending owner actions at this close-out** (BUILD_STATE §5 item 14 stays OPEN until done):
  ① confirm + delete both working papers and restore the CLAUDE.md doc-lifecycle sentence to
  "none are currently open" (coupled edits); ② open the stacked-chain PR
  (`fix/audit-2026-07-16-p9` → `master`), merge on green `ci-success`, post the #59 comment;
  ③ a version-checkpoint tag after merge (v0.1.51-style, owner-pushed per the standing policy).

**§-anchor legend.** Commits, code comments, tests, and BUILD_STATE cite this audit as
`audit 2026-07-16 F-xx` / `audit-2026-07-16 <ID>` (or the bare `F-xx` beside a qualified cite);
all 41 finding ids plus **NF-1**, **Q-1/Q-2**, and the **D-A…D-E** decisions resolve to this
section. This doc's § numbers collide across records (deliberate, never renumbered) — "§50"
means THIS section, the 2026-07-16 ledger directly following §49. **Working papers:** unlike
...
packaging.md's engine-install extraction-bound + per-family-guard clauses (F-33/F-32), the
image-understanding §6 RUNTIME-4 crash-recovery bullet (F-14) and its async history paragraph +
the §35 PERF-1 dated supersede (F-12), this doc's F-4 disposition addendum (F-25) and FE-7
amendment (F-31), `window-security.ts`'s buildCsp header + security-model.md (F-39),
benchmark.md + data-contracts (F-35), the coherence invariants in `committed-catalog.test.ts`
(F-06/F-16), the OCR parity net in `prepare-drive-default-set.test.ts` (F-05),
`as-never-ratchet.test.ts` + CONTRIBUTING's typed-stub bullet (F-41), BUILD_STATE §5 item 7's
...
## Evidence Pack / Review Mode — design record (EP-1, §1–§8)

_Formerly `docs/evidence-pack-review-mode-feature-spec.md` (the EP-1 feature spec) and
`docs/evidence-pack-implementation-plan.md` (the implementation plan, incl. its §14
append-only Handoff Log P0–P5) — both retired and deleted 2026-07-18 at wave close-out per
the CLAUDE.md doc-lifecycle rule. Full text in git history:
`git show b77295c0:docs/evidence-pack-review-mode-feature-spec.md` and
`git show b77295c0:docs/evidence-pack-implementation-plan.md` (the fullest plan state — all
deviations edited in, Handoff Log through P5; P6 is recorded in §8 below, nowhere else).
IMPLEMENTED 2026-07-18, phases P0–P6 (7 PRs #66–#72 merged + the P6 branch
`feat/ep1-p6-pdf-closeout`), each phase independently reviewed by 2 reviewers with a fix
round. The wave's dated BUILD_STATE entries live verbatim in `docs/build-log.md`
(2026-07-18, newest-first). **§ numbers below are stable — code comments cite them as
"EP-1 record §N"**; the closing §-anchor legend keeps the historical `spec §N` / `plan §N` /
`D-N` citations resolvable._

### §1 Intent + principles

Evidence Pack / Review Mode lets a professional (lawyer, consultant, HR, accountant,
researcher — spec §7's personas) **verify a document-grounded answer, record explicit
human decisions, and export a clear local report** — question, answer, per-block review
decisions, evidence register, source register, coverage, limitations, generation metadata.
The review is created from ONE persisted assistant message **entirely from stored rows**:
no model call, no re-retrieval, no network anywhere in the feature (FR-2/FR-12, enforced
in-suite by a runtime tripwire + the real offline guard). Principles as shipped:
**provenance is not proof** (a citation shows where information came from, never that the
answer is correct — the pack says so on its cover and is "not a certification",
known-limitations.md); **deterministic facts first** (snapshot, segmentation, pack model
and renderer are pure and deterministic; golden-tested); **honest evidence semantics**
(direct citations and whole-document provenance are never conflated — §4); **human
decisions are explicit** (six-value decision set, conservative bulk actions only, no "mark
all supported" pathway exists); **export is a snapshot** (localized at generation from the
pack's language option and frozen; never re-localized, never re-invented).

Three deliberate deviations from the spec's letter (plan §1, all owner-ratified): (1) PDF
ships via Electron's built-in `printToPDF` on the golden HTML template — no PDF-writing
dependency (D-1, §5); (2) source freshness for **legacy answers** (persisted before the
`Citation.documentId` enrichment) is best-effort by unique title with an honest
`identity: 'unresolved'` state — never guessed (§4); (3) **generation metadata is
synthesized at review creation** from the conversation row + live catalog + app version —
no per-message model id exists; every field optional, absent renders "Unavailable", never
invented.

### §2 Data model (as built — shapes live in data-contracts.md)

The authoritative shape reference is `docs/data-contracts.md` "Evidence Pack / Review
Mode" (kept current per phase; this record does not duplicate it). In one paragraph: four
tables — `evidence_reviews` → `evidence_review_items` → `evidence_review_links` (+
`evidence_reviews` → `evidence_exports`) — hang off `messages(id)` in one FK
`ON DELETE CASCADE` chain (the `result_tables` template; the real `deleteConversation`
drops the whole chain, proven by test), so review rows inherit encrypted-at-rest from the
workspace DB with nothing extra built. Snapshot JSON columns are nullable with tolerant
row→DTO parsers whose defaults all degrade toward the **weaker claim** (§4). Two additive
columns beyond the spec's §18.1 letter: `evidence_review_items.block_kind` (NULL = required
for the D-7 ready gate — corruption can only over-require, never exempt) and
`evidence_reviews.freshness_ack_json` — the acknowledge record whose **fingerprint is
value-bearing** (it canonicalizes the observed current value per drifted fact, so a
re-changed source/answer lapses a stale acknowledge; §6). The wave also stamped additive
...
(+ Draft/Ready chip, joined additively by ⚠ Outdated) gated by the shared
`isReviewEligible` predicate + a renderer streaming gate; `SourcesDisclosure` carries a
quiet footer action. `ScreenId 'review'` has NO nav-rail entry and is reachable only
through App's handoff slot (`openReview({reviewId}|{messageId})`, the `chatScope` idiom);
`resolveNavTarget('review')` deliberately falls to home. The workspace is two-pane (spec
§11): frozen answer snapshot (immutable `textSnapshot` markdown, per-block decision chips
as a roving-tabindex radiogroup, notes, link chips with origin labels) beside an evidence
pane (SourcesDisclosure card idiom, per-mode honesty captions, 24-card cap + stepped
reveal + filter that matches the marker AS DISPLAYED incl. DE `Q{n}`); narrow windows swap
the pane for a `Modal width='wide'` drawer with focus return. Edits flow through the
repo's first **debounced auto-save store** (`renderer/lib/reviewSession.ts`: 600 ms,
drain-before-await, failed patches re-merge under newer values, every post-await write
`openToken`-guarded; hard flush on exit AND in `App.lockNow` before `lockWorkspace()`,
store purged after). **Ready is read-only** — main refuses item/selection/link mutations
while ready (head edits stay allowed), the renderer disables the affordances, Reopen
restores. Reviewer **selections** are carved via a read-only `<textarea>` whose value IS
`textSnapshot`, so native `selectionStart/End` ARE the persisted UTF-16 offsets (exact by
construction; main refuses misaligned/surrogate-splitting offsets rather than clamping).
Back navigation returns to the ORIGINATING conversation via a one-shot `chatConversation`
handoff cleared on every chat mount. The summary modal carries counts, sources,
generation ("Unavailable" when absent), reviewer label, export history + hashes, and the
D-7-gated Mark ready / Reopen.

### §4 Honesty semantics (the load-bearing rules)

- **Citation vs whole-document provenance, never conflated.** Source `kind` maps from the
  persisted `coverage.mode`: `relevance` AND absent-coverage → `direct_excerpt`;
  `tree|capped` → `whole_document_provenance`; `extract` → `structured_record`; an
  **unknown-but-PRESENT mode → `whole_document_provenance` + null machine label + zero
  auto-links** — the weakest claim (a P1 review BLOCKER fix; the switch is
  compile-time-exhaustive so a new `CoverageMode` member reds the build). Whole-doc
  answers get **zero auto-links even with a literal `[S1]` in the text** (spec §13.3 hard
  rule); only the snapshot builder can mint `origin:'answer_marker'` — renderer link
  payloads are FORCED `origin:'reviewer'` at the IPC boundary.
...
- **Outdated = changed content only.** Positive drift (answer/coverage changed, ≥1 source
  ...
  **deletion warns but never gates** (spec §25.2/§28.7 letter). `outdated` is derived,
  never stored — it can never erase `ready` (spec §18.4).
- **The acknowledge fingerprint is value-bearing** (§6) — an acknowledge lapses when any
  drifted fact changes AGAIN or a NEW fact drifts; a stale acknowledge can never stamp a
  pack over a revision the reviewer never saw.
- **Tolerant parse defaults point away from unearned confidence**: unknown decision →
  `not_reviewed`, status → `draft`, link origin → `reviewer`, kind →
  `whole_document_provenance`, identity → `unresolved`; `evidence_exports.format` reads
  RAW (a repair-default would invent an export claim).
- **The D-7 ready gate**: every non-heading block decided (`not_applicable` counts;
  selections never gate; NULL `block_kind` = required).

### §5 Export architecture (one template, both formats)

One golden-tested HTML template is the single source of truth for BOTH formats. The pure
pack model (`evidence-pack/pack-model.ts`) normalizes the nine spec-§16.1 sections from
the stored review only (packId/generatedAt injected; deterministic — byte-identical
double-render pinned); the pure renderer (`evidence-pack/render-html.ts`) emits a
self-contained document — zero scripts/remote refs/`@font-face`, one embedded `<style>`,
`escapeHtml` exactly once after i18n, answer/items as escaped plain text (injection +
determinism), `@page` A4 print CSS, system fonts, and a semantic h1 + 8×h2 + h3 tree that
IS the PDF bookmark outline. Packs are structurally path-free (no path option exists).
The atomic tail (`export.ts`): tmp sibling → length-checked write → fsync → **sha256 of
the read-back bytes** → rename → `evidence_exports` row → audit `{reviewId, format}` only.
Pre-rename failure leaves no file and no row; a POST-rename record failure **unlinks the
just-renamed destination** (`EvidencePackRecordError`; if even the unlink fails,
`EvidencePackUnrecordedFileError` names the one residual state: file-exists-unrecorded).
The format is **extension-driven** — the user's chosen extension has the final word
(`packFormatForDestination`); build+render happen ONCE, post-dialog, with the effective
format threaded into the model, so content, extension, and export row always agree (the
pack's cover/Integrity "Format" line states what the artifact IS — exactly one rendered
line branches, golden-neutral by construction; the P6 adjudicated fix, §8).

PDF (D-1): a dedicated hidden `BrowserWindow` print harness
(`evidence-pack/print-pdf.ts`) — `SECURE_WINDOW_WEB_PREFERENCES` + **no preload at all**,
deny-all navigation/window-open, per-print window destroyed in `finally` + on
`before-quit`, 60 s step timeouts — loads the Phase-3 HTML **unchanged**, waits
`did-finish-load` AND `document.fonts.ready`, then `printToPDF({ pageSize:'A4',
preferCSSPageSize, printBackground, displayHeaderFooter, footerTemplate: packId +
...
### §6 Freshness (stored facts, no re-hashing)

`evidence-pack/freshness.ts` compares the frozen snapshot against the CURRENT workspace
from **stored facts only — no file I/O, nothing re-hashed** (spec §21.2; structurally
proven: the test documents have no files on disk): snapshotted `documentId` → row
existence; snapshotted `documentSha256` vs stored `documents.sha256`; answer text exact vs
`answer_snapshot`; coverage on a canonical semantic projection (reserialization/unknown
keys never drift). Per-source verdicts `unchanged|changed|missing|unverifiable`;
review-level `outdated` per §4. The spec-§20.1 "refresh on export" step (deliberately
skipped in P3) was resolved YES in P4: the verdict is computed ONCE pre-dialog, refusal
(`EvidencePackOutdatedError`) fires before any dialog, and the verdict is INJECTED into
the pure model — the pack renders **availability at export**, the §28.6 mismatch record,
and the acknowledge stamp; deleted/unverifiable sources warn but never block. Acknowledge
writes only its own column (single-purpose bypass of the ready guard), is not audited
(no spec-§22 event exists for it), and lapses by fingerprint (§2, §4).

### §7 Test inventory (pointer)

The wave's tests are the durable acceptance record (suite 4300/49 → **4625/49** across
P0–P6). Unit: `evidence-segment`, `evidence-review-gate`, `evidence-pack-model`,
`evidence-pack-html`, `evidence-pack-print-pdf`, `citation-snippet-boundary` (enrichment
leg). Integration: `evidence-reviews`, `evidence-snapshot`, `evidence-reviews-ipc`,
`evidence-pack-export` (6 golden packs under `tests/fixtures/evidence-packs/`, regenerated
only under `UPDATE_EVIDENCE_PACK_GOLDENS=1`), `evidence-freshness`,
`evidence-review-open-perf` (spec §26: 30 sources through real IPC, open < 1 s asserted,
measured ~3 ms), `evidence-pack-pdf-smoke`. Renderer: `ReviewScreen`,
`ReviewEntryPoints`, `ReviewExport`, `ReviewSelections`, `ReviewEvidencePane`,
`ReviewLockSeam`, `reviewSession`, `CitationMarkers`, plus legs in
`GermanSmoke`/`InformationArchitecture`/`LazyScreens`/`lockPurge`/`audit-ipc`/
`ipc-lock-coverage`/`window-security`. The PDF proof is a **four-layer split**: golden
HTML input (P3 suite, byte-frozen) → fake-electron harness unit tests (D-1 option
literals, teardown, quit-mid-print) → pipeline/IPC on a fake BrowserWindow → a
**REAL-Electron smoke** (`printPdfSmokeRunner.ts` bundles the real harness into the
installed binary; SKIPS where the binary/display is absent — CI installs no Electron —
and always runs on the first-class Windows dev box). Every IPC/export flow asserts the
model-runtime tripwire untouched AND the real `installOfflineNetworkGuard` silent; the
review renderer files enforce the no-call rule structurally
(`afterEach(assertNoUnexpectedApiCalls)` + `stubReviewApi`).

### §8 Decision ladder + phase dispositions

| # | Decision | Resolution (one line) |
|---|---|---|
| D-1 | PDF implementation | **Electron `webContents.printToPDF`** on a hidden window over the golden HTML template — covers every spec-§17.1 need with zero new deps; `pdf-lib` (no layout engine), `pdfkit`/`pdfmake` (a second renderer to maintain), Puppeteer (a second Chromium) all rejected; pitfall list banked and hit as predicted (§5) |
| D-2 | Conversation deletion | Warn-and-cascade: FK CASCADE via `messages(id)`; the delete confirm names the attached-review count (`evidence:countForConversation`) |
| D-3 | Reviewer identity | Free-text label only |
| D-4 | Pack manifest | Review-record only (`evidence_exports` row + hash); no sidecar file |
| D-5 | Source context | Stored extracted text around the citation (`evidence-pack/source-context.ts`, chunk ladder, never files); no page images |
| D-6 | Review title | Conversation title default, renameable, never empty (trimmed-empty refused; localized fallback title) |
| D-7 | Ready gate | All non-heading blocks decided; `not_applicable` counts; selections exempt; NULL kind = required |
| D-8 | Export retention | Metadata + hash only; no duplicate file in the workspace |
| D-9 | Build hash in pack | App version always; build hash only under Technical details (option default OFF) |
| D-10 | Branch strategy | Per-phase PRs to master (`feat/ep1-p<N>-<slug>`), merge only on green `ci-success`; master stayed releasable between phases |

Phase dispositions (each = phase commit + review fix round by 2 independent reviewers;
full Handoff Log P0–P5 at the plan's git-show pointer; dated entries verbatim in
`docs/build-log.md`):

| Phase | Branch (PR) | Shipped / review verdict |
|---|---|---|
| open | `docs/ep1-plan-landing` (#66) | Plan + spec landed, docs-only; D-1…D-10 owner-resolved 2026-07-18 |
| P0 | `feat/ep1-p0-contracts-storage` (#67) | Contracts, 4 tables, tolerant parsers, D-7 gate, 6-site `Citation` enrichment; `ef71be2`+`277b079`; 0 blockers, 6 findings fixed. Suite 4300→4342 |
| P1 | `feat/ep1-p1-snapshot-ipc` (#68) | Segmenter, snapshot builder, 13 IPC channels; `2385bef`+`8414aa7`; **1 BLOCKER: unknown-PRESENT coverage mode fell through to `direct_excerpt` and minted answer_marker links — the honesty INVERSION; now weakest-claim + exhaustive switch**; marker extraction unified to one whole-text pass. 4342→4394 |
| P2 | `feat/ep1-p2-review-ui` (#69) | Workspace UI, auto-save store, D-2 confirm; `bffae2b`+`d75a8a9`; 0 blockers, 5 SHOULD-FIX (ready-read-only enforced main-side; openToken race matrix; lock-seam pin). 4394→4451 |
| P3 | `feat/ep1-p3-html-export` (#70) | Pack model, renderer, atomic pipeline, goldens; `fe8b2f8`+`6cb548e`; 0 blockers, 2 SHOULD-FIX (post-rename unlink semantics; hash surfaced in history). **+`0fca4ec`: a LATENT P0 defect surfaced by CI — same-millisecond `created_at` ties broke on random UUID; reads now order by `rowid`** (insertion order). 4451→4519 |
...
`plan §N` (also `EP-1 plan §N`), `D-N`, `FR-N`, `spec Risk 7`, `review FIX-N`, and
`P<N> watch-out`. EP-1 `spec §N` citations live in EP-1-authored files (`evidence-*`,
`review*`, `citation-markers`, `print-pdf`, their tests) and in the EP-1-marked regions of
shared files (the `types.ts` EP-1 block, the chat entry-point comments, the EP-1 doc
sections); any other `spec §N` is the ORIGINAL MVP spec — see the retirement record below.
Anchors were never renumbered. This legend is built from the citations actually present at
retirement; resolve them as:

| Historical anchor | Meaning | Now lives in |
|---|---|---|
| spec §1–§5, §7 | Rationale, goals, principles, personas | §1 above + `product-vision.md` (one roadmap line) |
| spec §6 (§6.2–§6.8) | Terminology (review, item, link, decision, status) | the typed shapes in `data-contracts.md` + §2 |
| spec §8, §13 (§13.1/§13.3/§13.5), §14 (§14.1/§14.3/§14.4), §15 (§15.4/§15.5) | Answer classes, linking rules, decisions, coverage/limitation states | §4 above + `rag-design.md` §16 |
| spec §9 (§9.1/§9.2/§9.4), §10, §11, §12, §23, §25 | Entry points, journey, workspace UI, item model, a11y, edge states | §3 above + `design-guidelines.md` §11.12/§11.13 + `user-guide.md` "Review an answer against its sources" |
| spec §16 (§16.1.x/§16.2), §17 (§17.1/§17.2), §20 (§20.1–§20.3), §24.3 | Pack contents, formats, export pipeline, encryption-boundary warning | §5 above + `security-model.md` "Evidence-pack export boundary" |
| spec §18 (§18.1–§18.4), §19 | Data model, IPC surface | `data-contracts.md` (tables + channel table) + §2; §18.4's ready/outdated semantics in §4/§6 |
| spec §21 (§21.2/§21.3) | Freshness | §6 above + `rag-design.md` §16.5 |
| spec §22 | Privacy/audit posture | `security-model.md` + the audit content rules (ids/counts only) |
| spec §26, §28 (§28.6/§28.7/§28.9/§28.10), §29 (§29.4/§29.5), FR-1…FR-15 (§27) | Perf, acceptance, testing strategy, functional requirements | the test files themselves (§7 above); FR-2/FR-12 = the no-model/no-network tripwires |
| spec §30, §33 | Rollout phases, open decisions | §8 above (phase table + D-1…D-10 ladder) |
| spec Risk 7 (§31) | Item keys break when rendering changes | keys against the SNAPSHOT (`blockKey`, UTF-16 selection offsets) — §3/§4 above |
| spec §34–§36 | Source alignment, DoD, product description | §1 preamble; DoD verdict in the close-out BUILD_STATE entry (2026-07-18, `docs/build-log.md`); the honesty sentence in `known-limitations.md` |
| plan §1 (§1.2/§1.3) | Verdict + the three deviations | §1 above |
| plan §4 | Decision table D-1…D-10 incl. the D-1 research record | §8 above (research facts condensed into §5) |
| plan §5–§11 (+ `.N` sub-items, e.g. §7.2, §8.3, §11 item 1) | The per-phase work-item plans P0–P6 | §8's phase table + the verbatim build-log entries; full text at the git-show pointer |
| plan §12–§13 | Transfer map, orchestration protocol | executed by this record's existence; protocol facts in §8's preamble |
| plan §14, `P<N> watch-out` | Handoff Log P0–P5 + its inter-phase watch-outs | verbatim at `git show b77295c0:docs/evidence-pack-implementation-plan.md`; condensed in §8; the dated entries in `docs/build-log.md` |
