...
- [x] Phase 8 — API consistency, doc drift & housekeeping (API-1, BL-4, DATA-3/MAINT-3, DATA-4, DOC-2/3/4, API-2) — **Low/Info** ✅ (branch `backend-audit-2026-06-27-fixes`; API-1 `requireUnlocked()` preamble on every DB-touching chat handler in `registerChatIpc.ts` via new `main.chat.locked` i18n key — matches docs/collections/doctasks, in-memory-only stop/getActiveStream stay agnostic; DATA-3/MAINT-3 DECISION as built = **cheap row-count eviction** (`analysis/summary-cache.ts` `evictSummaryCache` deletes oldest rows past `SUMMARY_CACHE_MAX_ROWS`=50 000, env `HILBERTRAUM_SUMMARY_CACHE_MAX_ROWS`, called once per `buildTree`, content-free counter `summaryCacheEvictedThisSession`); DATA-4 `ORDER BY chunk_index` on `documentApproxTokenTotal` (no behaviour change); API-2 confirmed documented residual, no code change; docs — rag-design §3 "Cap" → over-cap rejection (DOC-2), known-limitations gains summary_cache eviction (DOC-4) + redaction date-locale asymmetry (BL-4), and the prefix-less-E5 retrieval **ceiling** reframed in the existing Phase-21 bullet (DOC-3 — was not folded by Phase 5); +5 teeth-verified tests, suite 2335/39)
