...
2. **Summarize each group → one fresh level-1 node** (always a new `tree_nodes` row per position — never reuse a row by hash). The summary text comes from the **content cache** (step 4). Write the node row **and its ordered edges** (`child_is_chunk=1`) in **one per-node transaction** (`exec('BEGIN')…exec('COMMIT')`, L7) so the node is atomically durable and the commit is one fsync, not one-per-edge — the commit boundary is the node boundary is the yield boundary (H9). **[L6] The node VECTOR (`embedding_blob`) is left NULL at build time** — it is computed **lazily by the consumer** (Phase 4 compare-align, or optional semi-global retrieval), under the H5 staleness discipline. Phase 1 ships zero node embeds; nothing reads a node vector until Phase 4, so paying ~250 sidecar embeds/doc up front would be wasted work if Phase 4 slips. (`tree_nodes.embedding_blob`/`summary_cache.embedding_blob` are already nullable — no schema change.)
...
- **Builder yield point** (synchronous, immediately after committing node N and *before* starting node N+1's `generate`): if a pause has been requested, persist `tree_status='building'`, mark the task resumable, **release the slot**, and return. Because this block has no `await`, no chat IPC can interleave inside it.
- **Chat path during a yielding build:** the chat guard does **not** proceed and does **not** refuse. It (1) sets `pauseRequested` synchronously, (2) **awaits a `slotReleased` handoff** the builder resolves at its next yield point, and only then (3) registers `inFlightStreams` + calls `chatStream`. Chat thus **waits** for the builder to relinquish (worst case ≈ one node), rather than racing it. A non-yielding task (summary/translate/compare/ocr) still refuses chat with `DOC_TASK_BUSY_MESSAGE` — the guard branches on the running task's *kind*.
- **Result:** the builder and chat never hold the slot simultaneously, because there is **one** slot with a synchronous claim and an explicit handoff — not two booleans read independently. (Note: the *tree* build does no embedding — node vectors are deferred to Phase 4 [L6] — so a yielding tree build cedes a genuinely idle chat slot. The Phase-4 lazy node-embed and the Phase-3 extract pass run on the CPU-only embedder/chat sidecars respectively; where one of those overlaps a ceded chat slot, "≈ one node" is a *scheduling* bound, not a zero-contention guarantee on weak hardware.)
- **[M9] Cancel on lock/quit.** Doc tasks are deliberately **not** in `inFlightStreams`, so `Lock now`/quit ([`registerWorkspaceIpc.ts:223`](../apps/desktop/src/main/ipc/registerWorkspaceIpc.ts) aborts only `inFlightStreams`) does **not** stop a build — for the short existing tasks that window is tiny, but a multi-minute build would keep calling `chatStream`/`getDb()` while the vault tears down (thrash, then a messy fail). The build must subscribe to the same lock/suspend signal and **abort its task controller** before `ctx.runtime.stop()`/`embedder.suspend()`, leaving `tree_status='building'` for `reconcileStuckTrees`.
...
**No Critical/High/Medium/Low items remain open.**
