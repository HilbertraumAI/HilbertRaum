...
> **Second-pass audit remediation (2026-06-15).** An independent second pass verified the above
> against the code and found issues the first pass missed:
> - **C4 (NEW, Critical) — the C1 honesty fix is incomplete for the EXISTING corpus.** C1 only
>   guards the *upload* path; a document indexed **before** Phase 1 ships was silently truncated at
>   1000 chunks and is still `indexed`. Building a tree on it (a manual "Build deep index" click — and
>   a big legacy doc is exactly what a user clicks it on) would report "100% of chunks" over a
>   *truncated* chunk set — resurrecting the exact dishonesty C1 killed. Fix: a per-doc
>   `fully_chunked` marker set only by the post-C1 pipeline; deep-index/100%-coverage are gated on it,
>   and a legacy doc is re-indexed first (which fully chunks it, or fails over-cap). (§3.5, §4.1, §6.)
> - **H9 (reopen H3, High) — the yielding-build handshake is under-specified and mis-sized.** As
>   written it admits a window where the builder and a chat answer both call `chatStream` (two model
>   jobs — a hard-constraint break). The existing chat↔task exclusion is two *independent* boolean
>   guards in two layers (`hasActiveTask()` throws at `chat-stream.ts:39`; `isChatStreaming()` throws
>   at `manager.ts:196`), race-free today only because each is a synchronous check-then-claim with no
>   interleaving `await`. "Chat pauses (not refuses) the build" reintroduces a TOCTOU at the
>   node→node boundary. Fix: a single in-process **model-slot arbiter**; chat **waits for an explicit
>   builder handoff** (does not "proceed"); re-sized from "small change". (§4.1, §5.1.)
> - **M8 — no transaction facility exists.** There are ZERO `BEGIN/COMMIT/transaction()` usages in
>   `services/` and `node:sqlite` has no `.transaction()` helper. The build **deliberately introduces**
>   an explicit `exec('BEGIN'/'COMMIT')` scoped to one writer — **one transaction per node** (row +
>   edges), so each node is atomically durable (L7) and finalize is a **single `UPDATE`** (inherently
>   atomic). *Tree*-level recovery still rests on resume (discard partial tree + rebuild from the warm
>   cache). (§4.1 steps 2/5.)
> - **M9 — `Lock now`/quit does not abort a build** (doc tasks aren't in `inFlightStreams`); a
>   multi-minute build thrashes the CPU while the vault re-encrypts. The build must observe a
>   lock/suspend abort. (§4.1.)
> - **M10/M11/M12** — i18n jargon leak in the plan's own UI copy; a mock-runtime-vacuous acceptance
>   probe; model-switch-mid-build → mixed-model tree. (§5.2, §6, §7, §4.1.)

> **Third-pass audit remediation (2026-06-15).** A third pass stress-tested the second pass's *own* new
> machinery against the code and fixed three issues it introduced or left under-specified:
> - **H10 (reopen H9, High)** — the "release the slot and **return**" yield had no working home in the real
>   `DocTaskManager`: a returning build is marked `done` ([`manager.ts:352`](../apps/desktop/src/main/services/doctasks/manager.ts))
>   and never re-enqueued (only a *startup* sweep would resume it), so the deep index stalls the first time
>   the user chats until an app restart. Fixed: the builder **parks on `await arbiter.reacquire()`** (stays
>   the running task, resumes in-session), and `assertChatStreamReady` becomes `async`. (§4.1, §5.1, §6, §7.)
> - **H11 (NEW, High)** — the per-node `BEGIN/COMMIT` had **no `ROLLBACK`**; a thrown insert leaves the one
>   shared `DatabaseSync` mid-transaction and poisons the next writer (the concurrent import loop, finalize, or
>   chat). Mandated `try { BEGIN … COMMIT } catch { ROLLBACK; rethrow }`. (§4.1 step 2, §7.)
> - **M13 (NEW, Medium)** — the over-cap throw must precede the destructive `DELETE FROM chunks`, else the C4
>   legacy-re-index flow turns a searchable-but-truncated doc into a `failed`/zero-chunk one. (§3.5.)
> - **M9 (strengthened)** — no signal exists to "subscribe" to; specified an explicit `abortActiveBuild()` in
>   `lockWorkspace`/`will-quit`. (§4.1.)
...
|---|---|---|
| Over-cap docs | silently truncated at 1000 chunks (still `indexed`) | **[C1]** Reject at upload with a friendly "too large" error; centralize the cap as `MAX_CHUNKS_PER_DOCUMENT` **[C2]**. **[C4]** Gate deep-index/100%-coverage on a `fully_chunked` marker so legacy (pre-C1, maybe truncated) docs can't claim full coverage — they re-index first. |
...
ensureColumn(db, 'documents', 'fully_chunked', 'fully_chunked TEXT')  -- [C4] set by the post-C1 processDocument on every successful index; NULL = legacy (maybe truncated) ⇒ must re-index before deep-index / 100% coverage
```
All three DDLs satisfy the `ensureColumn` grammar (`/^[A-Za-z0-9_ ]+$/`, [`db.ts:155`](../apps/desktop/src/main/services/db.ts)) — letters/digits/underscore/space only, no DEFAULT/NOT NULL.
...
- Append the `tree_nodes` / `tree_edges` / `summary_cache` / `extraction_records` `CREATE TABLE IF NOT EXISTS` blocks to `SCHEMA`; add the `ensureColumn` lines (`tree_status`, `tree_meta_json`, `fully_chunked` [C4], `extract_status`) in `openDatabase`. Add a `reconcileStuckTrees` call (mirror of `reconcileStuckDocuments`) for `'building'` rows. **FK order is a non-issue:** SQLite resolves FK *target tables* at DML time, not at `CREATE TABLE` time, so the new tables may reference `documents`/`chunks`/`tree_nodes` regardless of textual order in `SCHEMA`.
...
- **Reject over-cap at upload [C1].** `chunkSegments` (or a cheap pre-count helper `wouldExceedChunkCap(segments)`) must report **overflow** instead of silently returning the first `MAX_CHUNKS_PER_DOCUMENT` windows. In `processDocument`'s chunking step (the single chunk-replacement block, [`index.ts:578-585`](../apps/desktop/src/main/services/ingestion/index.ts) — runs on BOTH first-index and re-index): if the document would exceed the cap, set `status='failed'` with a new friendly, persist-canonical English message `main.ingest.tooManyChunks` ("This document is too large to fully index (over N sections). Split it and import the parts."), display-mapped at render — the existing `main.ingest.fileTooLarge` path at `index.ts:470/534` is the exact precedent (throw a persist-canonical message; the catch at `index.ts:616-619` sets `failed`). Because over-cap docs never reach `indexed`, **every NEWLY indexed document is fully chunked** → the tree's chunk coverage IS document coverage (kills the silent-truncation dishonesty). *This is a deliberate behavior change* (today such docs index partially); call it out in `known-limitations.md` and BUILD_STATE. A failed doc is never filed (`fileFromPendingDestination` runs on indexing *success* only) — no import-job/membership contract breaks.
- **[M13 — throw the over-cap error BEFORE the destructive chunk replacement.]** `processDocument` replaces chunks with `DELETE FROM embeddings … ; DELETE FROM chunks …` at [`index.ts:582-585`](../apps/desktop/src/main/services/ingestion/index.ts) *before* the fresh inserts. The cap check must run **after `chunkSegments`/`wouldExceedChunkCap` but before that DELETE** — i.e. the throw fires while the prior chunks/embeddings are still intact. Otherwise a **re-index** of an over-cap document (most importantly the C4 flow, which deliberately re-indexes every `fully_chunked IS NULL` legacy doc before deep-indexing it) would delete the existing chunks, *then* throw, leaving the doc `failed` with **zero** chunks — converting a previously searchable (if silently truncated) document into a completely unsearchable one. On a first index there are no prior chunks so the ordering is moot; the ordering matters only on re-index, but C4 makes re-index of large legacy docs the common case. Call this out in `known-limitations.md`: a legacy doc that is genuinely over-cap and gets re-indexed for deep-indexing ends up `failed`/unsearchable and must be split — the gate fails **closed**, never silently proceeding on a truncated set.
- **[C4 — legacy truncated docs must not get a false "100%".]** C1 only fixes the *upload* path. A document indexed **before** this change was silently truncated and is still `status='indexed'` on the user's drive; nothing re-chunks it automatically. If a tree is built over its stored (truncated) chunks, "100% of chunks" would again ≠ "100% of the document" — the C1 bug, resurrected for the legacy corpus, on exactly the large docs a user clicks "Build deep index" on. Fix: a per-doc marker `ensureColumn(db, 'documents', 'fully_chunked', 'fully_chunked TEXT')` (NULL sentinel; the `ensureColumn` DDL grammar forbids DEFAULT/NOT NULL) that the **post-C1 `processDocument` sets on every successful index** (every success is now fully chunked). **Gate:** tree build, deep-index, and any "100%" coverage claim are allowed **only** when `fully_chunked` is set. A doc with `fully_chunked IS NULL` (legacy) that the user wants to deeply index is **re-indexed first** — which either fully chunks a now-under-cap doc (sets the marker) or fails over-cap with the actionable message. This makes "the tree's 100% == the document" a true invariant for *every* served tree, not just freshly imported ones.
...
- **Rows per fully-built doc:** ~`ceil(chunks / branching)` level-1 nodes + the higher levels ≈ **chunks/4** node rows (≈250 for a 1000-chunk doc), one node vector each **once Phase 4 has embedded them** (384×4 B ≈ 1.5 KB → ~0.4 MB of vectors; NULL until then — L6), plus `summary_cache` entries (deduped across docs) and N `extraction_records`. Bounded by `MAX_CHUNKS_PER_DOCUMENT`, so per-doc storage is bounded.
...
2. **Summarize each group → one fresh level-1 node** (always a new `tree_nodes` row per position — never reuse a row by hash). The summary text comes from the **content cache** (step 4). Write the node row **and its ordered edges** (`child_is_chunk=1`) in **one per-node transaction** (`exec('BEGIN')…exec('COMMIT')`, L7) so the node is atomically durable and the commit is one fsync, not one-per-edge — the commit boundary is the node boundary is the yield boundary (H9). **[H11 — the transaction MUST be `try { BEGIN; inserts; COMMIT } catch { ROLLBACK; rethrow }`, never a bare `BEGIN…COMMIT`.]** The `generate`/embed calls happen **before** `BEGIN` (the transaction body is the synchronous inserts only — no `await` inside, per H9), but an insert can still *throw* synchronously: a `SQLITE_CONSTRAINT` (e.g. a NOT-NULL `summary_text`) does **not** auto-roll-back, and on a USB drive `SQLITE_IOERR`/`SQLITE_FULL` are realistic. Because the repo shares **one** `DatabaseSync` connection across chat, tasks, **and the import loop** ([`registerDocsIpc.ts:264`](../apps/desktop/src/main/ipc/registerDocsIpc.ts) runs `processDocument` with **no** `hasActiveTask()` guard — ingestion writes concurrently with a build), a `BEGIN` left open by a thrown insert would put the **very next writer on that connection** (the concurrent import's chunk/embedding write, a `setStatus`, the finalize `UPDATE`, or the chat answer the build later cedes to) *inside* the build's dangling transaction — or fail its next `BEGIN` with SQLite's "cannot start a transaction within a transaction". The explicit `ROLLBACK` on the catch is what keeps a single failed node from poisoning the shared connection; the failed build then unwinds to `tree_status='building'` for resume (step 4). This is the one place the otherwise-synchronous transaction can corrupt unrelated writers, so it is a hard requirement, not a nicety. **[L6] The node VECTOR (`embedding_blob`) is left NULL at build time** — it is computed **lazily by the consumer** (Phase 4 compare-align, or optional semi-global retrieval), under the H5 staleness discipline. Phase 1 ships zero node embeds; nothing reads a node vector until Phase 4, so paying ~250 sidecar embeds/doc up front would be wasted work if Phase 4 slips. (`tree_nodes.embedding_blob`/`summary_cache.embedding_blob` are already nullable — no schema change.)
...
5. **Finalize [M8].** All node/edge rows are already committed (step 2, one transaction per node). Finalize is therefore a **single `UPDATE documents SET tree_status='ready', tree_meta_json=? WHERE id=?`** — one statement, inherently atomic, so a reader never sees `ready` with a half-written `tree_meta`. **No multi-statement *finalize* transaction is needed:** the per-node `exec('BEGIN'/'COMMIT')` of step 2 (L7) already makes each node atomically durable, so the only states a reader/resume can observe are "node fully present" or "node absent" — never a half-node. This **deliberately introduces explicit `BEGIN/COMMIT`** to the repo (it has **zero** today, and `node:sqlite`'s `DatabaseSync` has no `.transaction()` helper — only manual `exec`), scoped to exactly this writer. Even so, *tree*-level durability still rests on resume (step 4): a crash **between** two nodes leaves a partial *tree*, which resume **discards and rebuilds from the warm cache** (0 chat calls for cached groups) rather than half-wiring parent pointers.

**Model-call cost (cold):** O(n) at the build, paid once. For 1000 leaf chunks, branching ≈5: ~200 (L1) + ~40 (L2) + ~8 (L3) + ~2 (L4) ≈ **~250 summary calls**. **Embedding calls: zero at build time [L6]** — node vectors are computed lazily by the Phase-4 consumer (~250/doc, on the separate embedder sidecar, once, when compare first needs them; cached thereafter). With a warm `summary_cache` (re-index, or repeated boilerplate), most summary calls are skipped too. **Commit cost:** one fsync per node (~250), not one per edge [L7]. **Query-time whole-doc summary cost:** read the root = **0 or 1 model call** (Tier 1; higher tiers cost more — see §4.5).

**[H3/H9] Yielding background build — must not freeze chat, must never run two model jobs.** A ~250-call build cannot run as a monolithic blocking task (DocTaskManager runs one task at a time and chat refuses while a task is active — that would lock chat out for the whole build). Instead the build is a **cooperative, per-node** job. **[H9 — the handshake, specified precisely]** This is *not* "a small change to the chat↔task mutual-exclusion": today that exclusion is **two independent boolean guards in two layers** — the chat guard throws `DOC_TASK_BUSY_MESSAGE` synchronously when `hasActiveTask()` ([`chat-stream.ts:39`](../apps/desktop/src/main/ipc/chat-stream.ts)), the task guard throws when `isChatStreaming()` ([`manager.ts:196`](../apps/desktop/src/main/services/doctasks/manager.ts)). They are race-free **only because each is a synchronous check-then-claim with no interleaving `await`** (Node is single-threaded, so an IPC can't interleave between a guard's check and its registry write). Turning "chat refuses" into "chat pauses the build" **reintroduces a TOCTOU**: if the chat IPC registered `inFlightStreams` and called `chatStream` while the builder's continuation (after committing node N) had already read "no chat in flight" and proceeded to node N+1's `chatStream`, **both** call `chatStream` on the one `llama-server` — a two-job break of the hard constraint. The fix is a **single in-process model-slot arbiter** (one owner of "who holds the chat runtime"), with these rules:
- The builder commits **one node at a time** (one transaction per node, §4.1 steps 2/5 — each node atomically durable; a crash *between* nodes leaves a partial tree that resume discards and rebuilds — resumable, cancellable).
- **Builder yield point** (synchronous, immediately after committing node N and *before* starting node N+1's `generate`): if a pause has been requested, persist `tree_status='building'`, then **`await arbiter.reacquire(jobId)`** — a Promise the arbiter resolves once chat releases the slot. **[H10 — the build does NOT `return` here.]** An earlier revision said "release the slot and return"; that is wrong against the real state machine: `DocTaskManager.run()` treats a normal return as success and sets `state='done'` ([`manager.ts:352`](../apps/desktop/src/main/services/doctasks/manager.ts)) while `pump()`'s `finally` clears `runningId` and shifts the **next queued** task ([`manager.ts:318-321`](../apps/desktop/src/main/services/doctasks/manager.ts)) — a yielded build is no longer in the queue (it was `shift()`ed out when it started), so **nothing re-enqueues it**. A "return at the yield point" would therefore mark an *incomplete* tree `done` and leave it stalled until the **startup** `reconcileStuckTrees` sweep — i.e. the deep index silently stops making progress the first time the user chats, until the app is restarted. The build must instead **stay the running task** (its `run()` stays pending on the `await`), so `runningId` keeps pointing at it, `pump()` runs nothing else, and when chat's stream ends the arbiter resolves `reacquire` and the build continues from node N+1 **in-process** (no re-enqueue, no startup dependency, no re-run of `startDocTask`'s guards — so no deadlock from the chat-in-flight guard at `manager.ts:196`). The synchronous block up to the `await` has no interleaving point, so no chat IPC can race the persist-then-park.
- **Chat path during a yielding build:** the chat guard does **not** proceed and does **not** refuse. **[H10]** This requires `assertChatStreamReady` ([`chat-stream.ts:24`](../apps/desktop/src/main/ipc/chat-stream.ts)) — today a **synchronous** throw-guard called at both [`registerChatIpc.ts:160`](../apps/desktop/src/main/ipc/registerChatIpc.ts) and [`registerRagIpc.ts:54`](../apps/desktop/src/main/ipc/registerRagIpc.ts) — to become **`async`** (both call sites already run inside `async` IPC handlers and `await` it): when the running task is a `tree`/`extract` build it (1) sets `pauseRequested` on the arbiter synchronously, (2) **`await`s the `slotReleased` handoff** the builder resolves at its next yield point, and only then (3) returns so the caller's `withChatStream` registers `inFlightStreams` + calls `chatStream`. When chat's stream finishes (its `withChatStream` `finally`), it calls `arbiter.release()`, which resolves the builder's parked `reacquire`. Chat thus **waits** for the builder to relinquish (worst case ≈ one node), rather than racing it. A non-yielding task (summary/translate/compare/ocr) still refuses chat with `DOC_TASK_BUSY_MESSAGE` — the guard branches on the running task's *kind*.
- **The arbiter is the single slot owner [H10].** One in-process object holds three primitives: `pauseRequested` (a flag the builder reads at its sync yield point), `reacquire(jobId)` (the Promise the *builder* parks on), and `release()` (called from chat's `withChatStream` `finally`, the lock/quit abort below [M9], a model-switch [M12], and build cancel). `release()` and the abort/cancel paths must **reject `reacquire`** (not just resolve it) when the slot is being torn down (cancel, lock, quit, model-switch) so the parked builder unwinds into its `cancelled`/`failed` handler instead of resuming into a stopped runtime; the builder's `await arbiter.reacquire()` is wrapped so an abort rejection lands in the existing `run()` catch (abort ⇒ `cancelled`). The two legacy booleans (`hasActiveTask()` at `chat-stream.ts:39`, `isChatStreaming()` at `manager.ts:196`) stay only for the **non-yielding** kinds; for a yielding build the arbiter — not those booleans — decides the handoff.
- **Result:** the builder and chat never hold the slot simultaneously, because there is **one** slot with a synchronous claim and an explicit, awaited handoff — not two booleans read independently — and a paused build resumes **in the same session** the instant chat finishes, never waiting on a restart. (Note: the *tree* build does no embedding — node vectors are deferred to Phase 4 [L6] — so a yielding tree build cedes a genuinely idle chat slot. The Phase-4 lazy node-embed and the Phase-3 extract pass run on the CPU-only embedder/chat sidecars respectively; where one of those overlaps a ceded chat slot, "≈ one node" is a *scheduling* bound, not a zero-contention guarantee on weak hardware.)
- **[M9] Cancel on lock/quit.** Doc tasks are deliberately **not** in `inFlightStreams`, so `Lock now`/quit ([`registerWorkspaceIpc.ts:216-245`](../apps/desktop/src/main/ipc/registerWorkspaceIpc.ts), whose `lockWorkspace` aborts only `inFlightStreams` at line 223 then stops the sidecars) does **not** stop a build — for the short existing tasks that window is tiny, but a multi-minute build would keep calling `chatStream`/`getDb()` while the vault tears down (thrash, then a messy fail). **There is no existing event a build can "subscribe" to** — so `lockWorkspace` (and the `will-quit` handler) must gain an **explicit** call to a new `ctx.docTasks?.abortActiveBuild()` (aborts the running build's task controller AND `reject`s the arbiter's parked `reacquire`, per [H10]) placed **before** the `Promise.allSettled([ctx.runtime.stop(), ctx.embedder.suspend(), …])` teardown — mirroring the `inFlightStreams` abort at line 223. This leaves `tree_status='building'` for `reconcileStuckTrees`. (A non-build task is left as-is — those are short and already fail friendly on the runtime stop.)
- **[M12] Model switch mid-build.** A build's nodes are summarized by one chat model; the `summary_cache` is keyed `(content_hash, model_id)` so a switch mid-build won't reuse the old model's summaries, but resuming under a new model would yield a **mixed-model tree**. Pin the build to `tree_meta.modelId`; if the active chat model changed at resume time, **restart** (don't resume) — the warm cache keeps the restart cheap for whichever groups that model has already summarized.
...
### 5.1 IPC
- **New deep-index task kind.** Extend `DocTaskKind` with `'tree'` (and `'extract'`) via `DocTaskManager` so progress polling, cancellation, and the busy guard come for free — but as the **yielding** class (§4.1, H3/H9): the chat guard branches on the running task's *kind* — for a tree/extract build it sets `pauseRequested` and **awaits the builder's `slotReleased` handoff** before claiming the slot (it does NOT proceed concurrently and does NOT refuse); for a blocking task it still returns `DOC_TASK_BUSY_MESSAGE`. The single model-slot arbiter (§4.1, H9) owns this handshake — not the two independent boolean guards. `DocTaskStatus.progress.stepsTotal` = planned node/extract count; `stepsDone` advances per committed node (resumable display).
...
### 5.2 Renderer
- **Coverage meter** (Phase 2 — the honesty differentiator): a component rendered with summary/answer results. **[M10 — i18n: no internal jargon in UI copy.]** The repo enforces a forbidden-UI-words policy (bucket / vector / scope_json / FTS / collection_id / membership / embedding — honoured throughout the doc-org wave) and a §3.5 informal-„du" German rule. The plan's *internal* vocabulary — `chunk`, `node`, `tree`, `deep index`, `coverage` — **must not leak into user copy**. So the meter reads, e.g. **"Covers the whole document (deeply indexed)"** vs **"Based on the most relevant passages — not the whole document"** (NOT "100% of 213 chunks", NOT "3-level tree"); the section count, if shown, uses the same user word as `tooManyChunks` ("sections"), never "chunks". "Deeply indexed" is the user-facing name for a ready tree. Every new string ships **EN + DE with type-enforced key parity** (the `Record<keyof typeof en, string>` rule) and the German copy is flagged for the standing **D-L7** informal-„du" review. Attaches in `PreviewModal` (replacing/augmenting the `summary.truncated` banner) and in the chat answer surface (near `SourcesDisclosure`).
...
Each phase is an **incrementally** shippable release in order (Phases 2/3/4 build on Phase 1's tree; there is no *backward* dependency — verified: Phase 1's `runSummary` reuses the existing `summary_json` shape and needs no `CoverageInfo` — L6), tests green + docs + BUILD_STATE per the per-phase ritual.

### Phase 1 — Cap honesty + ingestion-time summary tree (fixes the truncation bug)
- **Cap [C1/C2/C4]:** export `MAX_CHUNKS_PER_DOCUMENT`; make `processDocument` **reject** over-cap docs (`main.ingest.tooManyChunks`) instead of silent truncation, and **set the `fully_chunked` marker on every successful index** [C4]; note the behavior change in `known-limitations.md`.
...
- `tree-build.ts` as the **yielding** background build [H3/H9] (one transaction per node [L7]; single model-slot **arbiter** so chat and a node never call `chatStream` concurrently; aborts on lock/suspend [M9]); `tree` DocTask kind (Q1) auto-enqueued (size-gated, Q4) when a chat runtime is up, else `pending` + manual "Build deep index"; **precondition `fully_chunked` is set [C4]** (else re-index first); **node vectors left NULL — embedded lazily in Phase 4 [L6]**; skip generated docs [M6]; pin to one chat `modelId`, restart on model change [M12].
...
- **Acceptance:**
  ...
  - For a **near-cap fixture** (chunks just below `MAX_CHUNKS_PER_DOCUMENT`, exceeding context AND the old 12-window cap), once `tree_status='ready'` a **whole-document (root/Tier-1) summary is produced with ≤1 query-time model call and reports 100% coverage** (only because `fully_chunked` is set — C4). **[M11 — make the "no beginning-only truncation" probe testable with the MOCK runtime.]** The mock runtime does not actually summarize, so "the planted last-chunk fact appears in the root summary *text*" is **vacuous** under `npm test`. Assert the **structural** invariant instead: the last chunk is a **covered leaf reachable from the root** via the `tree_edges` provenance walk (coverage = leaf count = chunk count), so no tail is dropped. The *semantic* version (a real model surfaces a last-chunk fact in the root prose) belongs in a **manual/PAID smoke** with the real runtime, not the mock integration suite.
...
- **Integration (mock runtime + mock embedder, `npm test` from `apps/desktop`):**
  - over-cap upload fails; near-cap builds a tree whose root covers 100% and the **last chunk is a leaf reachable from the root** (the structural no-truncation probe — M11; the semantic "fact in the prose" check is a manual/PAID smoke, not the mock suite); a legacy doc with `fully_chunked IS NULL` is **re-indexed before** any tree/100% claim [C4];
  ...
  - compare swap-A/B mirror in single-pass and both-trees cases; embedder-mismatch guard fires [H5]. **[M11]** The mock embedder is hash-based/deterministic, so cosine node-alignment is meaningful only for the *structure*: plant identical sections (→ cosine 1.0, mutual-best-match pairs) and clearly-different ones (→ ~orthogonal) and assert the alignment + Only-A/Only-B **mirror** when A/B swap; do **not** assert summary/diff *content* (the mock doesn't summarize);
  - encrypted-workspace round-trip (tree nodes, `summary_cache`, extraction records persist and read back inside the vault).
- **Concurrency:** strict single model job (builder + chat never call `chatStream` concurrently); a chat request pauses (not refuses) a yielding build and **the build resumes in the same session once the chat stream ends** (reaches `ready` with no restart — H10); cancel/lock/quit mid-build **rejects** the parked `reacquire` so the build unwinds to a resumable `tree_status='building'`, never a hung `await` (H10/M9); a chat sent while a *non-yielding* task runs still gets `DOC_TASK_BUSY_MESSAGE`.
- **Per-node durability [H11]:** inject a node/edge insert that throws (e.g. a stubbed `exec` that fails on the edge insert) and assert the build issues a `ROLLBACK`, the shared connection is **not** left mid-transaction (a following independent write — e.g. a `setStatus` or a concurrent import insert — succeeds, no "cannot start a transaction within a transaction"), and the tree is left resumable.
...
**Fixed in the first revision:** C1 (upload-cap rejection), C2 (single `MAX_CHUNKS_PER_DOCUMENT`), C3 (content cache vs node identity), H1/H2 (re-index teardown + warm cache rebuild), H3 (yielding background build), H4/H5 (node-vector search + staleness, incl. the chat-hit/stale-embedding case in §3.1), H6 (collection-scope v1 behavior), H7 ("list every" honesty + closed-vocab mapping), H8 (symmetric-compare applicability). Medium M1/M2/M3/M4/M5/M6/M7 addressed in the relevant sections. Low items L1 (lease-free build), L2 (breadth≠fidelity wording, §4.5), L4 (sizing, §3.6), L5 (reverse edge index, §3.1).

**Fixed in the second-pass revision (2026-06-15):**
- **C4 (NEW, Critical)** — legacy silently-truncated docs + a tree build would resurrect the C1 honesty bug. Per-doc `fully_chunked` marker gates deep-index/100%-coverage; legacy docs re-index first. (§3.2, §3.5, §4.1, §6, §7.)
- **H9 (reopen H3, High)** — the yielding-build handshake was under-specified and mis-sized; as written it admits a concurrent two-`chatStream` window. Specified a single model-slot **arbiter** with a synchronous claim + an explicit chat-waits-for-handoff (not "proceeds"). (§4.1, §5.1.)
- **M8** — no transaction facility exists in the repo / `node:sqlite`; finalize is a single `UPDATE`, durability is tree-granular (discard+rebuild on resume). (§4.1 step 5.)
- **M9** — `Lock now`/quit doesn't abort a build (tasks aren't in `inFlightStreams`); the build must abort on the lock/suspend signal. (§4.1.)
- **M10** — the plan's own UI copy leaked internal jargon (chunk/node/tree/deep-index/coverage); user-facing wording + EN/DE parity + D-L7 flag specified. (§5.2.)
- **M11** — the "last-chunk fact in the root summary" probe is vacuous under the mock runtime; reframed to the structural coverage invariant, with the semantic check moved to a manual/PAID smoke; mock-embedder cosine-align caveat added. (§6, §7.)
- **M12** — model-switch mid-build → mixed-model tree; pin to one `modelId`, restart (not resume) on change. (§4.1.)
- **L6 (phasing + premature work)** — "independently shippable" reworded to **incrementally** shippable in order (verified no *backward* dependency: Phase 1's `runSummary` reuses the existing `summary_json` shape, needs no `CoverageInfo`). Node-vector embedding **deferred to Phase 4** (its only consumer) — Phase 1 stores nodes with a NULL vector, so it ships zero node embeds. (§3.1, §3.6, §4.1 step 2, §6 Phases 1/4.)
- **L7 (commit cost)** — each node's row + edges now write in **one `exec('BEGIN'/'COMMIT')` per node** (the commit boundary = node boundary = yield boundary), so the cold build does ~250 fsyncs (one/node), not one-per-edge — and each node is atomically durable, completing M8. (§4.1 steps 2/5.)

**Fixed in the third-pass revision (2026-06-15):** an independent third pass stress-tested the second pass's own new machinery (the arbiter and the per-node transaction) against the code and found three issues both prior passes missed:
- **H10 (reopen H9, High) — the yield handshake had no working home in the real state machine.** As written ("persist, **release the slot, and return**") a yielded build would (a) be marked `state='done'` by `DocTaskManager.run()` ([`manager.ts:352`](../apps/desktop/src/main/services/doctasks/manager.ts)) even though the tree is incomplete, and (b) never re-enqueue — `pump()`'s `finally` shifts the *next queued* task and the build was already `shift()`ed out of the queue, so the only resume path left was the **startup** `reconcileStuckTrees` sweep. Net effect: the deep index would silently stall the first time the user chats and not resume until an app restart — defeating the whole "background build" premise. Fixed: the builder **parks on `await arbiter.reacquire()` instead of returning** (stays the running task, resumes in-session when chat's stream ends), the single arbiter owns `pauseRequested`/`reacquire`/`release` and **rejects** the parked Promise on cancel/lock/quit/model-switch, and `assertChatStreamReady` ([`chat-stream.ts:24`](../apps/desktop/src/main/ipc/chat-stream.ts)) is made **`async`** so chat genuinely awaits the handoff (both call sites already await it). (§4.1, §5.1.)
- **H11 (new, High) — the per-node `BEGIN/COMMIT` had no `ROLLBACK`, so one failed insert poisons the shared connection.** The repo shares **one** `DatabaseSync` across chat, tasks, **and the import loop** (which runs `processDocument` with no `hasActiveTask()` guard — [`registerDocsIpc.ts:264`](../apps/desktop/src/main/ipc/registerDocsIpc.ts) — so ingestion writes concurrently with a build). A `SQLITE_CONSTRAINT` (which does **not** auto-roll-back) or a USB `SQLITE_IOERR`/`SQLITE_FULL` inside the node transaction would leave `BEGIN` open; the next writer on that connection then runs inside the dangling transaction or fails its next `BEGIN`. Fixed: the transaction is mandated as `try { BEGIN; …; COMMIT } catch { ROLLBACK; rethrow }`. (§4.1 step 2.)
- **M13 (new, Medium) — over-cap rejection must precede the destructive chunk DELETE.** The cap check has to throw *before* `processDocument`'s `DELETE FROM chunks` ([`index.ts:582-585`](../apps/desktop/src/main/services/ingestion/index.ts)), or a re-index of an over-cap doc (the C4 legacy-re-index flow) deletes the existing chunks then fails, turning a searchable-but-truncated doc into a `failed`/zero-chunk one. The gate must fail **closed** and preserve the prior index. (§3.5.)
- **M9 (strengthened)** — there is no existing signal a build can subscribe to for lock/quit; specified an explicit `ctx.docTasks?.abortActiveBuild()` call in `lockWorkspace`/`will-quit` before the sidecar teardown. (§4.1.)
- **Re-verified and still sound:** C4 `fully_chunked` coverage is honest because there is exactly **one** `setStatus(…, 'indexed')` site ([`index.ts:614`](../apps/desktop/src/main/services/ingestion/index.ts)) and every path (import loop, `reindexDocument`, OCR re-ingest, `materializeDocument`) funnels through `processDocument`, so a marker set there covers all `indexed` paths; the per-node transaction body is genuinely synchronous (generate/embed happen *outside* `BEGIN`); tree writes touch no FTS-triggered table; H1/H2 teardown (`DELETE FROM tree_nodes` → edges cascade via `parent_id`; `extraction_records` cascade via `chunk_id` under `PRAGMA foreign_keys=ON`) holds. M11/M12 unchanged.

**No Critical/High/Medium/Low items remain open** (third pass: H10/H11/M13 fixed in-doc above; no new Critical found — the H11 connection-poisoning hazard is Critical-class *if triggered* but is gated behind a thrown insert and now closed by the mandated `ROLLBACK`).

### Assumptions (flagging, not silently adopting)
- Level 0 = the existing stored **chunks** (overlapping ~80 tokens); the overlap is harmless for summarization. After §3.5, the chunk set is the **whole** document for any doc indexed by the post-C1 pipeline (over-cap docs are rejected, not truncated) — and the `fully_chunked` marker [C4] is what *proves* that for a given doc, so a tree/100%-coverage is served only when it is set (legacy docs re-index first). Chunk coverage == document coverage **exactly when `fully_chunked` is set**.
- Node embeddings reuse the **active E5 embedder** and the same Float32 blob encoding; node vectors live in `tree_nodes` (+ `summary_cache`), **not** the chunk-PK `embeddings` table, are searched by a dedicated node-cosine helper, and are kept out of chunk retrieval/citations (M2) so they don't dilute citation-grade chunk hits. **They are produced lazily by their first (and only) consumer, Phase 4 [L6]** — Phase 1 stores nodes with a NULL vector.
