...
>   `services/` and `node:sqlite` has no `.transaction()` helper. "Atomic finalize (one transaction)"
>   / "each node durable immediately" are reworded: durability is at **tree** granularity (resume =
>   discard partial + rebuild from the warm cache), and finalize is a **single `UPDATE`** (inherently
>   atomic). (§4.1.)
...
5. **Finalize [M8].** All node/edge rows are already committed (step 2/3, per-node). Finalize is therefore a **single `UPDATE documents SET tree_status='ready', tree_meta_json=? WHERE id=?`** — one statement, inherently atomic, so a reader never sees `ready` with a half-written `tree_meta`. **No multi-statement transaction is needed or available:** the repo has **zero** `BEGIN/COMMIT/transaction()` usages and `node:sqlite`'s `DatabaseSync` exposes no `.transaction()` helper (only manual `exec('BEGIN'/'COMMIT')`). Durability is at **tree granularity**, not node granularity: each node is a `tree_nodes` insert + N `tree_edges` inserts in autocommit, so a crash mid-node can leave a partial node — which is *fine* because resume (step 4) **discards the partial tree and rebuilds from the warm cache** (0 chat calls for cached groups). If per-node atomicity is ever wanted, wrap each node's `INSERT`s in an explicit `exec('BEGIN')…exec('COMMIT')` — but note that is a **new pattern for this repo**, not a reuse.
...
- The builder commits **one node at a time** (durable at *tree* granularity, §4.1 step 5 — resumable, cancellable).
...
- **Result:** the builder and chat never hold the slot simultaneously, because there is **one** slot with a synchronous claim and an explicit handoff — not two booleans read independently. (Note: even after the chat slot is ceded, an in-flight node **embed** on the CPU-only embedder sidecar still steals CPU, so "≈ one node" is a *scheduling* bound, not a zero-contention guarantee on weak hardware.)
...
- `tree-build.ts` as the **yielding** background build [H3/H9] (per-node commit; single model-slot **arbiter** so chat and a node never call `chatStream` concurrently; aborts on lock/suspend [M9]); `tree` DocTask kind (Q1) auto-enqueued (size-gated, Q4) when a chat runtime is up, else `pending` + manual "Build deep index"; **precondition `fully_chunked` is set [C4]** (else re-index first); node embeddings under the active embedder [H5]; skip generated docs [M6]; pin to one chat `modelId`, restart on model change [M12].
...
**Open Low (note only — not fixed):**
- **L6 (phasing)** — "independently shippable" overstates it: Phases 2/3/4 build on Phase 1's tree (verified there is no *backward* dependency — Phase 1's `runSummary` reuses the existing `summary_json` shape and needs no `CoverageInfo`). They are **incrementally** shippable in order. Separately, Phase 1 embeds ~250 nodes/doc whose **only** consumer is Phase 4 compare-align (and optional semi-global) — consider deferring node-vector embedding to Phase 4 to avoid sidecar work nothing reads if later phases slip.
- **L7 (sizing)** — a cold build does ~250 autocommit inserts; on a slow USB drive under WAL that is ~250 fsyncs. Acceptable (the build is already minutes of model calls), but batching a level's inserts behind one `exec('BEGIN'/'COMMIT')` would cut the fsync count if it ever shows up in profiling.
