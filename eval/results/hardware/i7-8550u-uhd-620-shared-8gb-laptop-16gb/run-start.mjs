...
// ADDED ON THIS MACHINE. Node's global `fetch` (undici) enforces a 300 s headers timeout that
// cannot be raised without an undici Agent, and this box is the first one slow enough to exceed
// it: the 9B decodes at ~2 tok/s on these 4 cores, so one non-streaming 512-token /completion —
// prefill included — runs well past five minutes and the request dies with
// `UND_ERR_HEADERS_TIMEOUT` before the server ever answers. Worse, `cache_prompt: false` means
// each of the three retries redoes the whole prefill, so the harness burns ~20 minutes and then
// throws WITHOUT writing the JSON or redacting the log. The first 9B start on this machine failed
// exactly that way and is kept as `leg0-9b-cpu-headers-timeout`. `node:http` applies no such cap,
// so the long /completion goes through it; /health and /tokenize are fast and stay on fetch.
const postJsonNoTimeout = (url, body) =>
  new Promise((resolve, reject) => {
    ...
    req.on('error', reject)
    req.setTimeout(0) // no inactivity cap: a slow decode is silence on the socket, not a failure
    req.end(data)
  })
const postJsonRetry = async (url, body, tries = 3) => {
  for (let i = 1; ; i++) {
    try {
      return await postJsonNoTimeout(url, body)
    } catch (e) {
      const code = e?.cause?.code ?? e?.code ?? String(e)
      requestErrors.push({ at: new Date().toISOString(), url: '/' + url.split('/').slice(3).join('/'), try: i, code })
      console.log('[request-error] ' + url + ' try ' + i + ': ' + code)
...
async function main() {
  ...
  const t1 = Date.now()
  // `--ignore-eos` (added on the Ryzen machine): Gemma 4 E2B ends its turn after ONE token on this
  ...
  // The transport is `postJsonRetry` (node:http) rather than `fetchClose` on this machine — see
  // the note at its definition; the body is byte-identical to the Ryzen copy's.
  const body = await postJsonRetry(base + '/completion', {
    prompt, n_predict: 512, temperature: 0, cache_prompt: false, stream: false, ignore_eos: ignoreEos
  })
  clearInterval(poll)
  samples.push({ t: Date.now(), ...smi() })
