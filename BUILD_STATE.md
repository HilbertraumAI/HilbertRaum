...
_2026-07-23 — **Full-audit 2026-07-23 remediation wave OPENED (branch `fix/audit-2026-07-23-remediation`, P0): a whole-repo eight-persona audit of `master` `bbf26add` (v0.1.55) confirmed 26 findings (0 Critical / 0 High / 7 Medium / 19 Low) + 4 design observations; the working papers are committed verbatim by this wave-open commit and retired at close-out.**_
Wave scope: AUD-01 (documents "Answer without it" undo silently CASCADE-deletes an evidence review — the only data-loss item), AUD-02/AUD-03 (the "Lock now" teardown window admits fresh content-surface starts / a post-lock `llama-server` auto-start; one shared lock-in-progress latch fixes both), AUD-04 (Translate adopt clobbers a held terminal result / hijacks a Documents-row translation), AUD-05 + AUD-24 (PowerShell provisioning scripts: `Write-Error` under `$ErrorActionPreference='Stop'` makes the tolerant continue paths dead code; mismatch redo re-downloads without deleting), AUD-06 + AUD-18 + AUD-20 (hygiene NUL/BOM nets skip `launchers/` and all shell/launcher extensions; the packaged-CSP guard degrades to a green skip; script-drift covers only the `.ps1` matrix), AUD-07 + AUD-21/22/23 (docs still present packaged OCR as "verify at release" though it is a verified app-killing crash; four broken `docs/docs/…` links; two catalog omissions), AUD-08…AUD-17 (evidence-pack/review correctness, a11y/i18n, and the O(N)-per-open review-chip IPC fan-out class), AUD-19/25/26 (exhaustiveness tooth, dead exports, CI Node/npm below the engines floor), and DV-1/DV-2 (native `<select>`s bypass the token system; the model picker leads with un-runnable models when no recommendation exists). Verified-and-dismissed, NOT in this wave: a refuted rank-0 RAM-sampling candidate, and an engine-download tar-child-on-quit sighting that is a live extension of §5 item 9 (recorded for a future downloads-teardown wave). Convention: ONE branch, one commit per phase, single PR at close (the OCR-R/DEP-1 audit-wave convention). Baseline at open: typecheck ✓, repo-hygiene 12/12, `tests/unit` 1656/102 files; the full suite runs once at the verification phase per the weak-box discipline.
**Phase log** (one line per phase — the retention budget is tight, so this wave keeps ONE dated entry instead of one per phase): P0 wave open — branch created, working papers committed verbatim, baseline recorded. · P1 **AUD-01** — the documents "Answer without it" undo could silently CASCADE-delete an answer's whole evidence review (head, per-block decisions + notes, links, export history), and neither restore leg carried it back; now REFUSED at the one shared regenerate choke point both channels pass through, with the affordance disabled-and-explained in the transcript. RED→GREEN on four legs (successful re-answer, failure-restore, plain-chat channel, Stop-before-first-token); fresh-context adversarial verifier signed off (only two `DELETE FROM messages` exist in `src/`, the other is the warn-and-count conversation delete; 400-transcript fuzz found no target-row divergence; mutation-check reddens the data assertions). Durable record: the EP-1 record's post-wave amendment in `docs/architecture.md`. · P4 **AUD-05 + AUD-24** — under `$ErrorActionPreference = 'Stop'` a `Write-Error` is script-TERMINATING, which made the PowerShell provisioning scripts' deliberately-tolerant paths dead code: the OCR loop aborted at the first failed language file instead of attempting the rest, every `Write-Error …; exit 2` exited 1, and a child's error propagated out through `prepare-drive.ps1`'s `&` call and killed it before its own "a whisper miss is a warning, not a failure" branch — so on mac/linux, where the whisper family deterministically has no prebuilt build, `-WithAssets` provisioning aborted before the OCR step on EVERY run. All tolerant/config paths now use `Write-Host -ForegroundColor Red` + an explicit `exit` (the pattern the same scripts already used), child invocations are try/catch-wrapped, and the documented exit-2 codes fire again; `verify-models.ps1` carried the same dead-`exit 2` bug at an unlisted site (it would have killed the drive build before its NOT SELLABLE verdict printed). AUD-24: a checksum-mismatch redo resumed onto the corrupt file (unsatisfiable range → HTTP 416, a guaranteed no-op) — now deletes first, but ONLY when the bytes on disk already reach the manifest's `size_bytes`, so a part-downloaded multi-GB weight still resumes across runs as documented. Verified on the real host against a local range-aware server (complete-but-wrong → full refetch in one run; short partial → `Range: bytes=1000-` → 206, tail only). · P3 **AUD-04** — `adoptActiveFileTranslation` guarded on `busy`, which is false in every TERMINAL state, so a Translate-screen mount wiped a COMPLETED translation's held output/gaps/truncated/resultDocumentId with a fresh "Translating…" frame; and because it never consulted the global doc-task store (unlike `guardStart`), plain navigation pulled a **Documents-row** translation into the panel — Stop then cancelled that foreign task and its output loaded as if it were a file translation. Guard is now "this store is idle" on BOTH the entry check and the post-await re-check (a rejected drop lands `failed` synchronously, so the busy-only re-check would have replaced the banner the user just triggered), plus a foreign-task check against the global store. Three RED cases, each isolating one component so no single change makes all three pass; the genuine post-reload recovery keeps a new explicit pin. Residual recorded in `known-limitations.md`: after a real reload the global store is empty too, so a Documents-started translation is still adopted — benign (the user's own task, real progress/Stop/result). · P2 **AUD-02 + AUD-03** — "Lock now" runs a multi-second AWAITED teardown before `lock()` nulls the DB, and every admission guard was a bare `isUnlocked()` (literally "the DB handle is non-null"), so a doc task / translate / image analyze / import landing in that window was admitted and — because the suspends are deliberately non-latching — lazily RESPAWNED the multi-GB sidecar the teardown had just killed, outliving the lock; separately, a model auto-start parked minutes in `computeInstallState` hashing a multi-GB weight spawned `llama-server` after the lock completed. Fixed with one `beginLock()`/`isLocking()` latch armed as the handler's first act, a shared `workspaceAdmitsWork()` predicate behind all 15 IPC guards + 3 service-level guards (each keeping its own localized copy), and a post-hash unlocked/not-locking + **unlock-epoch** re-check so a lock-and-re-unlock during the hash refuses too. Fresh-context adversarial verification enumerated all 128 `ipcMain.handle` registrations and found no missed content-bearing surface, and found two defects that were then fixed: the failed-lock disarm had a **tautological** test (deleting the disarm left 65 tests green — an ENOSPC lock would have left the workspace open and permanently latched), and a throw anywhere in the teardown could strand that same state, so the disarm is now **structural** (`beginLock()` → `try { … } catch { if (isUnlocked()) cancelLock(); throw }`, one disarm point). The QUIT teardown had the same window and now arms the same latch: refuted for translate/embedder/OCR/chat (quit latches permanently), but CONFIRMED for **vision** (a fresh ~4.6 GB `llama-server` orphaning at `app.exit(0)`) and for **import** (a plaintext transient stranded between write and shred). `architecture.md`'s "sidecars only ever start post-unlock" claim is now true and says what enforces it, naming the remaining `forceRestart` seam as a gap. · P5 **AUD-06 + AUD-18 + AUD-20** (test-only) — the NUL/BOM nets' shared extension filter excluded `.sh`/`.ps1`/`.cmd`/`.command` and neither net walked `launchers/` at all, so the byte-0-sensitive artifacts copied onto every commercial drive were unguarded: one BOM'd save (PowerShell 5.1 writes BOM'd UTF-8 by default on this dev box) and the mac/linux launchers silently stop executing for a buyer, through a green gate. Filter widened + `launchers/` added to both roots; every tracked script/launcher in the repo is now inside a net (verified by exclusion). The packaged-CSP guard's `describe.skipIf(!built)` degraded to a green skip of BOTH real security assertions on any output-layout drift — CI positive control added, plus a page-set pin closing the half the finding missed (an ADDED renderer page left `built` true and its baked CSP never inspected). `script-drift` gained the `.sh` runtime matrix, and the guards handed over by the provisioning phase: no `Write-Error` in the provisioning `.ps1`, the mismatch delete stays nested inside its size guard, and both call sites still pass the size argument (that guard fails OPEN, so a dropped argument would silently disable it). Also a new latch pinning every bare `isUnlocked()` under `src/main/` against a 12-entry allowlist, so the wave's `workspaceAdmitsWork()` conversion cannot silently drift back. Ten teeth checks, each planted and removed. · P8 **AUD-07 + AUD-21/22/23** (docs) — three surfaces still framed packaged-app OCR as "wired, verify at release" although it was measured on 2026-07-19 to be **broken and app-killing** (the tesseract.js worker's hoisted deps stay inside `app.asar` and cannot resolve from `app.asar.unpacked`; `ocrAvailable` reports true right up to the uncaught exception). All corrected by dated **superseding notes** on the dated records and rewritten present-tense guidance — plus two stale surfaces the finding had not named (the release ritual still required an OCR run; `troubleshooting.md` had nothing for a user whose app vanishes) and the durable ledger that §5 item 15(b) points at, which carried the same wrong instruction. The DEFECT is untouched and still live — the fix bundle stays registered at item 16(b). Also: four `docs/docs/…` links left by the 2026-07-12 move, fixed and now guarded by a `docs/*.md` link check (181 links across 19 files, 0 broken); the `translation/` role directory added to the manifest README; and the two missing downloadable chat models added to the README table with honest verdicts (the 2B FAILED its bar), which forced correcting the table's own "Min RAM is each model's hard floor" prose. · P1b **mid-wave finding** — `result_tables.message_id` carries the same `ON DELETE CASCADE` as the review chain, and `restoreMessage` replayed only the `messages` row, so on the two legs designed to lose nothing an answer's structured table (the "Export CSV" artifact) came back permanently gone and the derived `hasResultTable` silently read false. The snapshot now captures and replays those rows; a SUCCESSFUL regenerate still drops the table, which is correct. All 24 FKs enumerated: exactly two reference `messages(id)` and there is no third. · P6 **AUD-12/13/14/15** (EP-1 perf) — opening a documents conversation issued ONE IPC round-trip per candidate message, serially, each doing a full item-row load plus a freshness recompute (40 round-trips measured on a 40-answer conversation); the three bulk review actions fanned out into N writes each paying a redundant ready-guard head re-read, head stamp and item re-read, and **a failure part-way through left the review half-swept**; `evidence_reviews.conversation_id` had no index although it exists solely to serve two conversation-scoped reads; and the pack-export tail ran multi-MB write + fsync + full read-back on the Electron MAIN thread. Now: two new channels (`evidence:summariesForConversation`, `evidence:bulkAction`, both behind the wave's `workspaceAdmitsWork()` predicate) giving one round-trip per conversation open and one transaction per bulk action, an additive `idx_evidence_reviews_conversation`, and an async `fs.promises` export tail whose atomic tmp→fsync→hash-off-disk→rename contract is byte-for-byte unchanged. Atomicity proven by an injected SQLite trigger firing on the sweep's last statement (the genuine part-way case); the async port proven by instrumented `node:fs`/`node:fs/promises` recording zero sync calls and the exact `open→write→sync→close→readFile→rename` order, with `close` before `rm` on both failure paths. · P7 **AUD-08/16/17/09/10/11** (EP-1 polish) — the source-context modal rendered the chunker's deliberate ~80-token overlap twice (a byte-exact ~450-char run); a PDF export's plaintext print-source cleanup failure was swallowed by a catch whose body was only a comment, in a file that imported no logger, leaving unlogged decrypted residue; **two concurrent same-destination exports could swap pack content** — and the audit's provenance-swap was only half of it: the shared `${dest}.tmp` also made the loser fail outright with ENOENT, so the old code could DESTROY a legitimate concurrent export, not merely mislabel one. Both transients are now named per export through one shared token helper (so the two seams cannot drift apart), the cleanup retries and logs ids-only, and the de-overlap mirrors the summarization path. Plus three a11y/i18n corrections: the export disclosure kept `aria-expanded="true"` on a control the freshness gate had just disabled, the narrow-mode evidence drawer re-opened with a focus trap on the next Windows snap, and the reveal control said "N more sections" for SOURCE cards in both EN and DE. Four existing `${dest}.tmp` assertions went VACUOUS under the rename and were replaced with directory scans that have teeth under any name. · P9a **AUD-19/25/26** — the freshness drift fingerprint hand-enumerated `CoverageInfo`'s fields, so a future coverage field would be silently excluded from drift detection and an export could proceed without the outdated/acknowledge gate firing for it; now both a `satisfies Record<keyof CoverageInfo, unknown>` constraint and a keys-parity test, with the **persisted fingerprint proven byte-identical** (captured pre-edit, hard-coded, then changed — a different canonical string would lapse every acknowledgement already stored in a workspace). Three genuinely-dead exports pruned of 39 candidates (the documented `nodeVectorSearch` fallback correctly retained; ~34 live-but-over-exported values left alone as reachable API). CI now runs Ubuntu/Windows × Node 22.x/24.x with `corepack enable`: the declared `engines.npm >= 11` floor and the `npm@11.6.2` pin had never been exercised anywhere (Node 22 bundles npm 10.9), and the Node major the release legs full-suite-test under was never gated by CI. · P9b **DV-1/DV-2/DV-3** (design, orchestrator eyeball in both themes) — the Models context-size and Translate language-bar `<select>`s carried no class at all, so they rendered UA defaults (Arial, `radius 0`, UA border/background) — worst in dark, where the control was a light-grey box glaring against a near-black card. A shared `.select` treatment now puts all four picker-style selects (incl. the Documents translate modal, whose selects are the Translate bar's twin, and the Diagnostics filter) on `--font-sans`/`--radius-sm`/`--border-strong`/`--surface`/`--text`; measured after-values equal the tokens exactly in both themes, and height 19→30px finally clears the 24px target minimum. The model picker led with un-runnable models: on a fresh 16 GB workspace three of the first four cards carried a RAM warning. **The plan's rule ("runnable-first only when no recommendation exists") proved nearly inert** — the recommender keys off machine RAM, not a benchmark result, so a fresh workspace DOES have a recommendation; the rule was widened to unconditional runnable-first (installed-first still primary, `recommendModelIdByRam` untouched, display order only) and the ★ card can only be lifted, never demoted (observed 21→15). First RAM-warned card moved 1→16 of 23. Plus 9 ASCII quote closers fixed in the German catalog. Eyeball FAILED at loop 1 on DV-2 and passed at loop 2.
