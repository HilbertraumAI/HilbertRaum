...
_2026-07-18 — **EP-1 Phase 6 stage A DONE (branch `feat/ep1-p6-pdf-closeout`, plan §11 item 1): PDF export via `printToPDF` (D-1) — same pipeline, same atomic tail, one template; close-out (item 2) is stage B on this branch.**_
As-built record: `docs/data-contracts.md` EP-1 section (pipeline/harness/P6-tests blocks; header now "Phases 0–6") + `known-limitations.md` (PDF-accessibility honesty entry) + `user-guide.md` (format choice) + design-guidelines §9 row **"exported PDF heading order" RESOLVED** (best-effort tagged PDF, never PDF/UA — the one open §11.13 a11y item, closed). Shipped: (1) **print harness** `evidence-pack/print-pdf.ts` — per-print hidden `BrowserWindow`, `SECURE_WINDOW_WEB_PREFERENCES` spread + **no preload at all** (wiring-pinned as the THIRD call site in `window-security.test.ts`), deny-all window-open/navigation; `loadFile` (=did-finish-load) → `document.fonts.ready` → `printToPDF` with the FULL D-1 set — A4, `preferCSSPageSize` (template `@page` authoritative), `printBackground`, footer = escaped packId + `pageNumber`/`totalPages` (system fonts + inline styles ONLY) + empty-span `headerTemplate` (suppresses Chromium's default date/title header), `generateDocumentOutline`, `generateTaggedPDF` — **installed Electron 37.10.3 types support every option (verified, no casts)**; destroy in `finally` + `before-quit` hook (quit mid-print ⇒ clean rejection), 60 s step timeouts, concurrent-safe (own window, no channels). Print source = transient `${dest}.print.tmp.html` SIBLING (user-sanctioned dir, never OS temp; `.html` load-bearing for file:// MIME), removed in the same `finally`. (2) **pipeline**: `renderPdf` is a REQUIRED injected dep (IPC wires the harness; electron-free module stays testable); freshness verdict + refusal STAY before any dialog/window work and packId/generatedAt mint PRE-dialog (P4 TOCTOU posture); build+render run ONCE, AFTER the dialog, with the EFFECTIVE format threaded into the model (`EvidencePackModel.format` — review FIX-1: the cover/Integrity "Format" line states what the artifact IS, `packExport.meta.formatValuePdf` EN+DE; exactly ONE line branches, pinned byte-level; cancel renders nothing); print after the dialog (no hidden window across the dialog wait); `writePackFileAtomic` now takes string|Buffer — identical tail, row `format:'pdf'`, format NEVER in `options_json`. Format contract: wire `EvidencePackExportRequest = Partial<EvidencePackOptions> & {format?}`; `resolvePackExportFormat` (literal 'pdf' else 'html'); dialog OFFERS both filters (requested first); **the chosen extension has the final word** (`packFormatForDestination` — content/extension/record always agree); `suggestedPackFileName(title, format)`. (3) **renderer**: export-panel format radio (HTML default; `review.export.format*` — 4 new P6 keys per language incl. `packExport.meta.formatValuePdf`); history `format` display stays raw passthrough. (4) **tests +30**: fake-electron harness unit ×9 (D-1 option literals, no-@font-face/escape pins, posture, teardown incl. quit-mid-print + timeout), pipeline P6 ×6 (incl. cancel-under-PDF: no render/no print — FIX-3) + helpers +3 new (the pre-existing `suggestedPackFileName` test extended — FIX-5b), render format-line branch +2 (byte-level one-line-swap pin + DE), IPC ×2 (real harness on fake BrowserWindow; audit `{reviewId, format:'pdf'}`), panel +1, window-security +1, and **REAL-Electron smoke ×6** (`evidence-pack-pdf-smoke.test.ts`: esbuild-bundles the REAL harness, spawns the installed binary — isolated `--user-data-dir`, runner keeps a `window-all-closed` no-op or Electron's default quit races the run; SKIPS where binary/display absent — CI installs no Electron — always runs on the Windows dev box): EN+DE golden prints → pdfjs page counts, CURRENT-catalog sentinels (umlauts/ß), packId + `1/{totalPages}` footer, **outline depth exactly 3 with catalog titles, `/MarkInfo Marked` true**, kill-after-load ⇒ rejection/no output, every Chromium request `file://` + real offline guard silent. HTML goldens byte-identical WITHOUT regeneration (the format-line branch is golden-neutral by construction — the swap pin proves it; PDF bytes nondeterministic ⇒ never goldened). Suite **4625/49** (was 4595/49; +30) + typecheck + build green. NOT in this stage (stage B, same branch): design record, spec/plan retirement, CHANGELOG, build-log moves. Budget: this file sits ~298.3 kB/1917 lines of the enforced **300 KiB (307,200 B)/2000** (repo-hygiene pins the byte form) — ~8.9 kB + 83 lines for stage B, which retires the wave to build-log.
**Review fix round (2 independent reviewers → 1 adjudicated SHOULD-FIX + NITs, all applied):** FIX-1 the PDF self-described as "Self-contained HTML · pack schema v1" on cover AND Integrity — the sections a verifier reads next to the hash note — while bytes/extension/row said pdf; fixed by rendering ONCE post-dialog with the effective format in the model (shape above; the hidden window still receives the render output UNCHANGED — the §14 rule governs post-processing, which remains none; goldens stayed green un-regenerated as the neutrality proof). FIX-2 the print-source `writeFileSync` moved INSIDE the harness try — an ENOSPC partial write is decrypted pack content and is now covered by the same finally force-remove. FIX-3 cancel-under-PDF pinned (dialog null ⇒ no render, no print, no file, no row). FIX-4 the smoke runner header now states the kill leg's honest scope: `app.emit('before-quit')` fires the HOOK, not Electron's real quit sequence — proves hook-destroys-window + rejection + no output; the finally-vs-real-teardown race is out of scope (invariant holds regardless: the write sits after the rejected await). FIX-5 (a) `window-security.ts` header re-worded for THREE call sites incl. the deliberately preload-free print window ("restoring" a preload is the failure mode); (b) count breakdowns corrected here + data-contracts (helpers = 3 new + 1 extended); (c) the malformed-format IPC leg now asserts `offlineViolations` beside the runtime tripwire; (d) print-pdf concurrency comment names the same-destination `.print.tmp.html` collision (loser fails cleanly — no file, no row); (e) budget framing corrected to the enforced 300 KiB byte form (above).

_2026-07-18 — **EP-1 Phase 5 DONE (branch `feat/ep1-p5-polish`, plan §10): workflow polish — reviewer text selections, evidence-pane filter + stepped reveal (no virtualization, measured), German native pass over ALL EP-1 keys (draft flags gone), recorded §9 a11y audit, spec-§26 perf tripwire, back-to-originating-conversation.**_
As-built record: `docs/design-guidelines.md` **§11.13** (patterns, the offset-mapping strategy, the FULL recorded a11y checklist + perf numbers) + §7 EP-1 German glossary + `docs/user-guide.md` review-section additions. Shipped: (1) **selection UI** (service existed since P0) — "Review a passage separately" per block swaps in a READ-ONLY `<textarea>` whose value IS `textSnapshot`, so native `selectionStart/End` ARE the UTF-16 offsets (exact by construction; mapping through `AssistantMarkdown`'s rendered DOM was rejected — markdown dropped, `[S1]`→`[Q1]`, KaTeX rewrite; main refuses misaligned offsets, never clamps); refused ⇒ quiet `role=status` retry hint; added ⇒ toast, composer stays mounted; selection items render as tagged plain-text quotes + Remove; while READY the composer is HIDDEN and Remove renders visible-DISABLED (P4 watch-out honored); store `addReviewSelection`/`deleteReviewSelection` (immediate IPC, `openToken`-guarded, refusal ≠ save error). (2) **EvidencePane**: filter (title/snippet/section/marker/page, shown past the 24-cap) + cap-sized stepped reveal + "{shown} of {total}" status + "Linking evidence for review item N" context line (spec §23 pane↔item tie). **`@tanstack/react-virtual` NOT added** — measured first (plan §10): 30-source open = create 11.8 ms / open 3.0 ms, DOM never exceeds the revealed batch. (3) **Back to chat → originating conversation** (P2 debt): `onBackToConversation(detail.conversationId)` → App `chatConversation` handoff (chatScope idiom, cleared by every plain chat navigation) → ChatScreen `initialConversationId` mount re-attach (stream/skill-run idiom, `activeIdRef`-guarded; deleted id degrades to chat home). (4) **German native pass (D-L7)** over `review.*` 152 / `packExport.*` 96 / `main.evidenceReviews.*` 6 (EN=DE, incl. 15 new P5 keys): du-form, legal-adjacent register, §7 glossary enforced (Quellenverweis unified, „Prüfstand“ mistranslation fixed, Anlegen/Erstellen split, ellipsis spacing); ENTWURFSSTAND flags REMOVED; ONE golden regenerated (`german.html`, single-line disclaimer diff, hand-reviewed). (5) **§9 a11y audit RECORDED in §11.13** — 2 real fixes (`.msg-action` min-height 24, `.chip-remove` 24×24, both app-wide), everything else PASS or pass-with-rationale (aria-controls deliberately not stamped on non-interactive rows); keyboard path for selections = native textarea Shift+arrows (2.5.7). (6) **Perf test** `evidence-review-open-perf.test.ts`: 30-source review through real IPC, runtime tripwire ZERO touches, real offline guard silent, no fixed sleeps, open < 1000 ms asserted (spec's own budget as an order-of-magnitude guard). Bulk heading-N/A verified still working (P2 test green — NOT re-implemented). Tests +16 (`ReviewSelections` ×7, `ReviewEvidencePane` ×5, `ChatBackToConversation` ×3, perf ×1); no schema/IPC/data-contract changes; ratchet 110 untouched. Suite **4587/49** (was 4571/49) + typecheck + build green. NOT in this phase (per plan): PDF + close-out doc moves (P6).
**Review fix round (2 independent reviewers → 1 BLOCKER + 3 SHOULD-FIX + NITs, all applied):** FIX-1 (BLOCKER) the back-handoff slot survived `askSelectedDocuments` (the one chat mount OUTSIDE `navigate()`) — review→back→Documents→"Ask these documents" resurrected the OLD conversation and stomped the just-set documents mode; the path now clears the slot (only navigate/askSelectedDocuments mount chat — openReview/lockNow verified non-chat), pinned by an App-LEVEL multi-screen test (mutation-verified red without the one-line fix). FIX-2 the source filter now also matches the DISPLAYED marker via `formatCitationLabel` — a German reviewer typing the "Q3" the card shows matches (raw "S3" still does); DE-leg test. FIX-3 the spec-§23 pane↔item tie is now genuinely programmatic: the linking line is the region's `aria-describedby` DESCRIPTION (same region component in aside AND drawer; name stays "Evidence"); §11.13 + catalog comments state exactly that; described/absent both pinned. FIX-4 key-count precision: **15** new P5 keys per language (6 `review.evidence.*` + 8 `review.selection.*` + `review.item.selectionTag`); 16 was the TEST delta. FIX-5 batch: (a) refusal + filter-no-match live regions now mount EMPTY and fill on state change (`:empty` collapse; appear-with-content is missed by some SRs); the shown-count line de-live'd (changes only on the user's own click); (b) +tests: reveal-reset-on-filter-change, section/page matcher facets; (c) DE selection-surface leg — the textarea carries the RAW `[S1]` snapshot while the rendered block shows `[Q1]` (the EN fixture couldn't see the transform); (d) P2-pattern deferred-promise token races for `addReviewSelection`/`deleteReviewSelection` (post-purge ×2 + post-switch); (e) this entry's "affordances hidden while ready" precision-fixed (composer hidden, Remove visible-disabled); (f) de.ts glossary header synced with §7 and the „belegt“ rule corrected — the NOUN „Beleg“ is banned, the verb „belegt (nicht)“ is the sanctioned form (labels AND disclaimers); (g) §11.13's no-virtualization basis restated as the MEASUREMENT (3.0 ms open, DOM bounded) — `@tanstack/react-virtual` pre-exists for DocumentsScreen, so "no new dependency" was not the argument. Tests +8 (ReviewSelections 7→8, ReviewEvidencePane 5→8, ChatBackToConversation 3→4, reviewSession 24→27); suite **4595/49** + typecheck + build green.

_2026-07-18 — **EP-1 Phase 4 DONE (branch `feat/ep1-p4-freshness`, plan §9): the freshness engine, Outdated lifecycle + acknowledge, export refresh step, and source-in-context — spec §21/§15.4–15.5/§25 states from STORED facts only; historical reviews stay understandable after source change or deletion (spec §30 Phase-3 gate).**_
As-built record: `docs/data-contracts.md` "Evidence Pack / Review Mode" Phase-4 block (types, engine, IPC rows, pack/renderer additions) + `docs/rag-design.md` **§16.5** (the freshness design over §16.2's identity split) + `docs/known-limitations.md` (3 rewritten/new freshness bullets). Shipped: (1) **freshness engine** `evidence-pack/freshness.ts` — `computeEvidenceReviewFreshness(db, reviewId)` compares the frozen snapshot against the CURRENT workspace from STORED facts ONLY (spec §21.2: snapshotted `documentId` → row existence; snapshotted `documentSha256` vs the stored `documents.sha256` — **never re-hashed, no file I/O; pinned structurally: the test documents have no files on disk at all**; `messages.content` vs `answer_snapshot` exact; coverage on a fixed semantic projection — `nodeIds`/unknown extras excluded as plumbing). Per-source verdicts `unchanged|changed|missing|unverifiable`; **unresolved identity → 'unverifiable', NEVER 'changed'** (the binding P3 watch-out, adversarially tested under mutation AND deletion of same-titled docs); absent hash on either side → 'unverifiable' (unknown is not drift). **`outdated` = POSITIVE drift only** (answer/coverage changed ∨ ≥1 source changed); **deletion does NOT flip it** — spec §25.2/§28.7 letter: a deleted source is an unavailability warning (§15.4 copy, excerpt retained, export proceeds), the §28.6 acknowledge gate is reserved for CHANGED content. Derived, never stored ⇒ can never erase `ready` (pinned). (2) **acknowledge lifecycle** — additive nullable column `evidence_reviews.freshness_ack_json` (`ensureColumn`) storing `{acknowledgedAt, fingerprint}`; the fingerprint canonicalizes every non-unchanged fact, so a LATER drift change lapses the acknowledge (new warning, honest); acknowledging writes ONLY its own column — status/completed_at/**updated_at** byte-identical (spec §18.4) — and is deliberately NOT subject to the ready-state write-guard (lifecycle metadata, not a decision edit; works on ready reviews, tested); malformed record reads as never-acknowledged (never unlocks the gate); NO audit event (not a spec-§22 type). (3) **IPC** — `evidence:refreshState` REAL (the P1 stub replaced; `EvidenceReviewFreshness` widened ADDITIVELY: optional `answerState`/`coverageState`/`sources[]`/`acknowledgedAt`) + 2 new channels (**17 total**): `evidence:acknowledgeFreshness`, `evidence:sourceContext` — both `requireUnlocked` (auto-covered by `ipc-lock-coverage`); **read overlay at the IPC boundary**: `evidence:get`/`evidence:getForMessage` responses carry the REAL computed `outdated` (the chat chip is truthful with zero new renderer wiring); service reads + write-path returns keep constant-false (renderer freshness UI reads the store's refresh result exclusively). (4) **export refresh step — the spec-§20.1 decision: YES, export refreshes.** `exportEvidencePackToFile` computes the verdict up front (stored-fact compare = trivially cheap), REFUSES an outdated-unacknowledged review BEFORE any dialog (`EvidencePackOutdatedError` → localized `main.evidenceReviews.exportOutdated`; renderer gates the button too — hint + inline acknowledge in the summary), and INJECTS the verdict into `buildEvidencePackModel(detail, options, meta, freshness?)` — 4th OPTIONAL param, the model stays PURE (freshness facts arrive as input like packId/generatedAt; determinism kept, goldens inject fixed verdicts). Pack records the re-check per §28.6/§28.7/§16.1.7: cover outdated warning + acknowledge stamp, coverage-section mismatch records (`answerChangedNow`/`coverageChangedNow`/`sourcesChangedNow`/`sourcesMissingNow` — missing counts NEW deletions only), per-card §15.5/§15.4 warnings, `freshnessChecked` note (the P3 fixed `freshnessNote` now renders ONLY when no verdict was injected), and the source-register availability column becomes **"Availability at export"** (available/changed-since-review/missing/cannot-verify from the new `EvidencePackSource.currentState`). Goldens: the 5 P3 fixtures REGENERATED under `UPDATE_EVIDENCE_PACK_GOLDENS=1` (hand-reviewed: exactly 2 surfaces per pack — the note swap + the column; missing-source cells honest Missing/Cannot-be-verified) + a **6th golden `outdated-acknowledged`** pinning the §28.6 rendering. (5) **source-in-context (D-5)** `evidence-pack/source-context.ts` — `getEvidenceSourceContext(db, reviewId, sourceKey)`: renderer names review + source KEY only; main resolves the SNAPSHOTTED documentId through the review's own snapshot and reads the **stored extraction (`chunks` table)** — never the source file. Ladder: snapshotted `sourceChunkId` (verified to belong to the snapshotted document — a foreign/hostile chunk id can never leak another document's text, adversarially tested) → `instr` containment search for the persisted snippet (the `truncateSnippet` '…'-prefix shape handled) → honest `located:false` (persisted excerpt shown as recorded, never approximated). Context = located chunk ± 1 neighbor, ≤1200 chars/side, surrogate-safe boundaries; `hashState` match/mismatch/unknown = the same stored-hash compare. Null for unknown review/key AND unresolved sources. (6) **renderer** — store gains `freshness` state + `refreshReviewFreshness()` (**fired on every successful open** — fire-and-forget so the ≤1 s open never waits; `openToken`-guarded incl. the purge race, pinned) + `acknowledgeReviewFreshness()`; ReviewScreen: Outdated banner (drift facts by exact key + the §21.3 keep/acknowledge/ask-again options + acknowledge button ↔ acknowledged stamp; role=status) + an **ADDITIONAL `⚠ Outdated` chip** (Draft/Ready never replaced — spec §18.4); EvidencePane cards gain freshness badges (changed / NEWLY-missing §15.4 / resolved-cannot-verify; **unresolved keeps only its identity badge**) + **Open source in context** (resolved sources only) → new `SourceContextModal` (highlighted `<mark>` excerpt + hash-state line + stored-text note; missing → §15.4 copy + the persisted excerpt); ReviewSummaryView: changed/missing-now count lines + the export gate; MessageActions/Transcript: the `.msg-review-chip` row gains the Outdated chip (text+glyph) from the summary's computed overlay. (7) **i18n** +42 keys EN+DE each (9 `review.outdated.*`, 3 `review.source.*`, 11 `review.sourceContext.*`, 4 `review.summary.*Now`, `review.status.outdated`, 13 `packExport.*`, `main.evidenceReviews.exportOutdated`; DE stays ENTWURFSSTAND for the P6 native pass). Tests **+45**: new `tests/integration/evidence-freshness.test.ts` ×22 (engine matrix, acknowledge persist/no-op/lapse/malformed/stamps-untouched/ready, export gate + §28.6/§28.7 packs, IPC legs + real overlay + boundary guards, source-context ladder incl. the leak guard — REAL offline guard across all, no doc files exist ⇒ no re-hash possible), export goldens 15→16, `evidence-pack-html` 26→31, `ReviewScreen` 14→21 (refresh-ON-OPEN spy, banner/chip/badges, acknowledge, missing-no-banner, unresolved-badge-only, context modal ×2), `ReviewExport` 9→12 (gate disabled/hint/acknowledge-unlocks), `reviewSession` 19→24 (refresh-on-open, failed-open-no-refresh, post-purge token guard, acknowledge merge/failure), `ReviewEntryPoints` 13→14 (Outdated JOINS Ready), `GermanSmoke` +1 (DE overlay); every review renderer file now stubs the refresh structurally via new `stubReviewApi` (`tests/helpers/evidenceReview.ts`) under the existing afterEach tripwire. Suite **4564/49** (was 4519/49) + typecheck + build green. NOT in this phase (per plan): selection UI (P5), PDF (P6), P3 pipeline semantics untouched (the refresh step is additive-before-render; atomic tail byte-identical).
**Review fix round (2 independent reviewers, 0 blockers, 2 SHOULD-FIX + 7 items, all applied; deletion-split reading, unaudited acknowledge, chip staleness and golden regeneration independently ENDORSED):** FIX-1 the fingerprint's central claim ("any later drift change lapses the acknowledge") was FALSE for re-changes — state-literal fingerprints (`src:S1=changed`) read a SECOND change of the same fact as "the same drift" (doc sha aa→ff → acknowledge → ff→ee kept the gate open and stamped the pack "acknowledged" over a revision the reviewer never saw; same hole for a second answer edit and coverage; changed→recovered→changed-again resurrected the dormant record). Fingerprints now carry the OBSERVED CURRENT VALUE per non-unchanged fact — the current stored `sha256` for changed sources, a 16-hex digest of the current answer text / canonical coverage for those — while missing/unverifiable keep state literals (no value exists; a NEW deletion still lapses by ADDING a fact); hashes only, inside the encrypted row. Internals restructured (one `computeFreshness` pass returns verdict + fingerprint; `freshnessFingerprint` un-exported), module header + data-contracts re-worded to the now-true claim; pinned by 4 new tests (re-change lapse ff→ee; second answer edit; changed→recovered→changed-again no-resurrect; the stored fingerprint's canonical value-bearing shape via `parseFreshnessAck`). FIX-2 `lockPurge.test.ts` still used plain `stubApi` around its review-session open (warn-noise unmocked refresh; falsified this entry's "every review renderer file" claim) — swapped to `stubReviewApi`; the claim is now true. FIX-3 mixed drift pinned end-to-end: one source CHANGED and another DELETED simultaneously — gate fires from the changed one, §15.4 warning + retained excerpt + `sourcesMissingNow` for the deleted one, both register cells correct, AND a post-acknowledge NEW deletion lapses the acknowledge (the added fact) with the banner re-demand. FIX-4 coverage canonicalization pinned in the false-positive direction (reserialized key order + unknown extra keys + `nodeIds` ⇒ `unchanged`, never outdated). FIX-5 the banner's drift list now includes NEWLY-missing sources (reuses the existing `review.summary.sourcesMissingNow` EN+DE pair — identical copy, no duplicate keys; creation-missing stays off the list) so a deletion-lapsed re-demand names its cause; renderer test. FIX-6 the `EvidencePackOutdatedError` → `main.evidenceReviews.exportOutdated` IPC mapping pinned over the wire (localized rejection, no row/no audit event, and the SAME call proceeds post-acknowledge). FIX-7 precision: this entry's key count corrected 41→**42** (the breakdown always summed to 42); data-contracts' evidence-pack-html P4 count ×6→×5; the export test's "five golden classes" headers now say six (5 spec §29.5 + P4 outdated-acknowledged); `evidence-reviews.ts`'s `rowToReview` + module-header `outdated` comments de-staled ("no freshness engine exists until Phase 4" → the real posture: service constant-false, real overlay at the IPC read boundary, write-returns deliberately false). **Accepted-posture records (no code change):** (a) the export verdict is computed BEFORE the native save dialog — a source changing while the dialog sits open is stamped at the pre-dialog check (verdict + `generatedAt` are both pre-dialog; matches the spec §20.1 pipeline order, and the pack's "re-checked when this pack was generated" wording is accurate); (b) a source key ABSENT from an injected verdict renders `unverifiable` in the pack while the evidence pane shows no badge for it — deliberate asymmetry: both are non-claims (the pack must fill its mandatory §16.1.7 cell; the pane simply states nothing). Tests +7 (evidence-freshness 22→28, ReviewScreen 21→22); suite **4571/49** + typecheck + build green.

_2026-07-18 — **EP-1 Phase 3 DONE (branch `feat/ep1-p3-html-export`, plan §8): evidence-pack export — deterministic, atomic, hashed, self-contained HTML; the 15th `evidence:*` channel + the `evidence_pack_exported` emitter land together with their pipeline.**_
As-built record: `docs/data-contracts.md` "Evidence Pack / Review Mode" Phase-3 block (types, model, renderer/print contract, pipeline, store) + `docs/security-model.md` "Evidence-pack export boundary" + `docs/user-guide.md` §7 export paragraph + `docs/known-limitations.md` "Evidence review & evidence packs". Shipped: (1) **pure pack model** (`evidence-pack/pack-model.ts`) — nine §16.1 sections normalized from the STORED review only; §16.2 option flags (`EvidencePackOptions` + shared `EVIDENCE_PACK_OPTION_DEFAULTS`: mandated content ON, technical details OFF, **no path flag — the snapshot carries no paths, structurally stronger than "default off"**); boundary resolver drops non-boolean/unknown keys; absent/malformed → null → "Unavailable" (never invented); honesty section REUSES `evidencePaneMode` (MOVED to `shared/evidence-review.ts`, re-exported from EvidencePane — unknown-PRESENT modes stay whole-doc/WEAK, pinned against `sourceKindForMode`); `identity:'unresolved'` ("could not be verified") stays DISTINCT from resolved-but-missing. (2) **pure print-aware renderer** (`evidence-pack/render-html.ts`) — new `escapeHtml` (xmlEscape shape + quotes; every content string escaped once, after i18n interpolation); ONE fixed template: zero scripts/remote refs, one embedded `<style>`, `@page` A4, `break-inside: avoid` on cards/warnings/rows, semantic h1→h2→h3 (= the Phase-6 `generateDocumentOutline` bookmark tree), system fonts only, grayscale warnings (border+⚠+text), index-derived anchors; answer/items as ESCAPED PLAIN TEXT (deliberately no main-side markdown→HTML — injection + determinism); `[S{n}]` markers localize via the SHARED regexes (DE `[Q{n}]`, code stays literal); `formatPackTimestamp` = locale-independent `YYYY-MM-DD HH:MM UTC`; EN/DE localized AT GENERATION from the pack's language option and frozen (98 new keys EN+DE each at phase close — 83 `packExport.*` + 13 `review.export.*` + `review.status.lastExported` + `main.dialog.exportEvidencePack` — + reused review/chat keys; DE under the ENTWURFSSTAND flag). (3) **atomic pipeline** (`evidence-pack/export.ts` + the IPC handler): load → resolve → build → render → save dialog (§24.3 encryption-boundary warning shown in the renderer panel BEFORE the dialog on every platform + macOS `message`) → tmp sibling → fsync → sha256 of the READ-BACK on-disk bytes → rename → `evidence_exports` row (bare name, `EVIDENCE_PACK_SCHEMA_VERSION` 1, RESOLVED options into `options_json`); failure/cancel ⇒ NO file, NO tmp, NO row (spec §20.3/§28.9); UTF-8 WITHOUT BOM (`<meta charset>` is the contract; `bomFor` untouched). **15th channel `evidence:export`** + preload `exportEvidencePack(reviewId, Partial<EvidencePackOptions>)` + the `evidence_pack_exported` emitter (`{reviewId, format}` ONLY — path/name/title never audited, path-sentinel-swept). Export works on draft AND ready reviews (the FIX-1 write-guard covers item mutations only — pinned service-, IPC- and renderer-side). (4) **summary UI** — `ReviewSummaryView` actions row gains *Create evidence pack* → INLINE options panel (checkboxes at shared defaults + warning; no nested modal); status line gains `review.status.lastExported` (display-only, enum unchanged); export history renders real rows; store `exportReviewPack(options)` FLUSHES pending edits first (failed flush refuses — the pack renders persisted data), merges the returned record into `detail.exports` newest-first under the `openToken` guard (P2 watch-out honored); outcomes exported/cancelled(silent)/failed(inline copy). Tests +60 net: unit `evidence-pack-model` ×13 (boundary, §16.1 normalization, mapping-reuse pin, option matrix) + `evidence-pack-html` ×24 (escapeHtml, §29.4 injection incl. hostile markdown/`javascript:`, §17.2 self-containment, D-1 print contract, single-h1/8-h2 hierarchy, EN/DE freezing + code-literal markers, determinism ×3); integration `evidence-pack-export` ×13 — the five §29.5 **golden packs** (`tests/fixtures/evidence-packs/`: relevance-ready / whole-doc / partial-coverage / missing-source / German; normalized timestamps+ids; regen via `UPDATE_EVIDENCE_PACK_GOLDENS=1`), atomicity (failing write AND failing rename → no file/tmp/row, review usable), cancel/unknown-id, no-BOM+umlauts, recorded-hash==file-bytes, history newest-first + "Previous export" in the next pack, REAL offline guard silent across every test; `evidence-reviews-ipc` +3 (ready-review export over the wire + detail.exports readback, cancel → no row/no audit, boundary guards + hostile options → resolved defaults persisted; runtime tripwire); `audit-ipc` export leg (path-sentinel destination; metadata exactly `{reviewId, format}`; `evidence_pack_exported` joins the expected-types sweep); renderer `ReviewExport.test.tsx` ×7 (panel defaults incl. technical-OFF, payload shape + language freezing, history merge + Last exported, cancel keeps panel/no error, failure inline alert, flush-before-export call order, ready-review export) — `exportEvidencePack` added to stub sets under the structural afterEach tripwire. Suite **4511/49** (was 4451/49) + typecheck + build green. NOT in this phase (per plan): PDF (P6 — the template IS the P6 input), freshness (P4 — packs state "not re-verified" explicitly), selection UI (P5).
**Review fix round (2 independent reviewers, 0 blockers, 2 SHOULD-FIX + 7 NIT, all applied):** FIX-1 the POST-rename failure region — "any failure leaves no file and no row" was false on two unhandled paths (`recordEvidenceExport` throwing after a successful rename; the review deleted in another window during the open dialog, whose null return was misreported as user-cancel): both now UNLINK the moments-old destination (an unrecorded pack would make its own printed "hash is recorded" integrity note false — worse than no file; rename-over-existing had already replaced the old file, so keeping ours restores nothing) and throw distinct named errors (`EvidencePackRecordError` / `EvidencePackUnrecordedFileError` when even the unlink fails — file exists WITHOUT a record, said explicitly), localized at the IPC layer (`main.evidenceReviews.exportNotRecorded`/`.exportFileNotRecorded` EN+DE); claims re-scoped in export.ts/ipc.ts/preload/data-contracts/security-model; pinned by 3 new tests (trigger-injected INSERT failure + deleted-mid-dialog at the service, the localized surface + no-file/no-row/no-audit-event over IPC). FIX-2 the SHA-256 promise made true — user-guide + the pack's integrity note point at Export history, which showed name·format·date only: rows now render the truncated mono hash + a copy-FULL-hash action (`copyToClipboard` idiom, toast on success; keys `review.export.copyHash`/`.hashCopied` EN+DE), pinned by a copy-carries-the-full-hash test. FIX-3 the flush-failure export refusal was claimed but unpinned (deleting the guard passed the suite) — now a test proves a failed auto-save flush refuses the export (IPC never fires); a token change landing AFTER a successful export now returns 'exported' (state untouched) instead of a false 'failed' — the export is real. FIX-4 `formatPackTimestamp` renders ZONE-LESS input verbatim (host-local parsing was TZ-dependent; stamping UTC would invent a zone never recorded) + unit cases. FIX-5 `writePackFileAtomic` verifies `writeSync` wrote the full buffer (a short write would have hashed a self-consistently-wrong truncated file). FIX-6 injection suite extended: hostile source key/machineLabel/documentSha256/raw coverage mode through the technical surfaces + the structural remote-ref sweep now runs over the HOSTILE render too. FIX-7 pack items number by the CARRIED ordinal, not the render index — with unreviewed items excluded, pack "Item 3" stays workspace item 3 (holes are honest; the excluded-count notice accounts for them); goldens unchanged (default options exclude nothing). FIX-8 precision: the entry's "~60 packExport keys" corrected to the counted 98 new keys at phase close; fix round adds 4 (2 `review.export.*` + 2 `main.evidenceReviews.export*`) → **102 new keys EN+DE each**, parity intact. FIX-9 the new DE copy follows the existing du-form register under the standing ENTWURFSSTAND flag (P6 native pass). Tests +7 (evidence-pack-html 24→26, evidence-pack-export 13→15, evidence-reviews-ipc 12→13, ReviewExport 7→9); suite **4518/49** + typecheck + build green.

_2026-07-18 — **EP-1 Phase 2 DONE (branch `feat/ep1-p2-review-ui`, plan §7): the review workspace UI — open from chat, review items, decide/note with debounced auto-save, link/unlink evidence, summary, mark ready, reopen after restart; zero model/network activity end to end.**_
As-built record: `docs/design-guidelines.md` §11.12 (rollout patterns + the one spec deviation) + `docs/user-guide.md` §7 "Review an answer against its sources" + the data-contracts IPC table (now 14 channels). Shipped: (1) **navigation** — `ScreenId` +`'review'` with NO nav-rail entry; `resolveNavTarget('review')` deliberately falls to home (the screen is meaningless without App's handoff slot); `App.tsx` lazy `ReviewScreen` chunk + `openReview(reviewId | messageId)` handoff state (the `chatScope` idiom); back = the screen's own "‹ Back to chat" → `navigate('chat')`. (2) **Entry points (spec §9)** — `MessageActions` gains *Review evidence / Continue review* (last in row) + a text+glyph Draft/Ready chip, gated per message by the SHARED `isReviewEligible` (+ the conversation mode for documents-answers-without-citations) with streaming covered by the row's `actionsDisabled`; `SourcesDisclosure` gains the quiet `.sources-review` footer action; ChatScreen fetches `getEvidenceReviewForMessage` per eligible message (failure reads as "none") and hands off `{reviewId}` when a review exists else `{messageId}` (main's create is idempotent either way). (3) **`ReviewScreen`** two-pane per spec §11: answer pane = frozen `textSnapshot` markdown via `AssistantMarkdown` + `localizeServerCopy` (markers localize, text never edits), collapsible question, per-item honesty note ("No direct source marker" / "Derived through whole-document analysis" — whole-doc wording asserted BY EXACT KEY, provenance never worded as citations), 6-value decision chips as a roving-tabindex radiogroup (`review/DecisionControl.tsx`), note textarea, link chips with origin labels ("Cited by the answer" vs "Reviewer linked") + unlink; evidence pane (`review/EvidencePane.tsx`) = SourcesDisclosure card idiom + per-mode honesty caption + the spec §24.3 disclaimer + the shared `PROVENANCE_CARD_CAP` (now exported) reveal + link/relation controls (relation editing on REVIEWER links only — rewriting an `answer_marker` link through `setEvidenceLink` would flip its origin and destroy the citation claim); narrow (<980px, one matchMedia subscription) swaps the pane for a `Modal width='wide'` drawer per item with built-in focus-return. (4) **Auto-save precedent** — `renderer/lib/reviewSession.ts` module store (doctasks idiom) + the repo's first debounced flush (600 ms; optimistic read-your-writes; LOSS-FREE under rapid edits — drain-before-await + failed patches re-merge UNDER newer values; null results = stale-handle error state, never a throw); hard flush on screen exit AND in `App.lockNow` BEFORE `lockWorkspace()` (the vault must still be writable), then `purgeSessionStores()` drops the resident review content (lockPurge.ts). Renderer `computeReadyGate` mirrors D-7 for live progress; the authoritative gate comes back from `markEvidenceReviewReady` ({review, gate}; ineligible renders as "N of M decided" guidance, never a failure). (5) **Summary + gating** — summary Modal (status+completedAt, decision counts, sources incl. unresolved/missing-at-creation counts, CoverageMeter + output-truncation note, generation details with absent = "Unavailable", reviewer label D-3 + general note, export history when non-empty) with D-7-gated *Mark review ready* / *Reopen review*; **Phase 3 mounts the export action in this summary's actions row** (anchor comment in `ReviewSummaryView.tsx`). Conservative bulk menu ONLY (headings→N/A, undecided→follow-up, clear-all behind ConfirmDialog) — **no "mark all supported" exists, pinned by test + no store pathway can bulk-write a supported verdict**. (6) **D-2** — the conversation-delete confirm names the attached-review count via NEW channel `evidence:countForConversation` (+preload `countEvidenceReviewsForConversation`, requireUnlocked, ids-only, malformed→0 — the P0 service function had no channel; recorded as the phase's sanctioned deviation in data-contracts) with an honest generic fallback line when the count can't be read. (7) **i18n** — full `review.*` EN+DE set (93 keys exactly: 92 shipped with the phase — spec §24 actions/decisions/captions incl. the review disclaimer, D-2 warning, summary/autosave/bulk copy — + `review.readonlyHint` from the fix round); DE flagged DRAFT for the Phase-6 native pass (comment in de.ts). Deviation from spec §11.2 (recorded in §11.12): no per-screen "Local · Offline" header duplicate — the rail-foot indicator is THE single ambient signal (design-guidelines §12.1 #2). Deferred per plan: export UI (P3), freshness/outdated UI beyond the chip placeholder (P4), selection-creation UI (P5), header overflow menu (rename shipped inline; duplicate/delete/technical-details later). Tests (phase + fix round, +57 net): `reviewSession.test.ts` ×19 (debounce/merge/loss-free/fail-remerge/null-refusal/gate-mirror/bulk-guard/purge + the FIX-2 race matrix: post-purge and post-switch resolutions never write, failed-write re-merge is token-guarded, reopen flushes first, retry reconciles), `ReviewEntryPoints.test.tsx` ×13 (visibility matrix eligible/ineligible/streaming/plain-chat/no-handoff + Continue/chips + sources-footer incl. its streaming gate + D-2 count/zero/generic legs), `ReviewScreen.test.tsx` ×14 (create→decide→note→summary→ready with flush-before-gate order pinned; reopen; not-found; flush-on-exit; whole-doc exact-key wording ×2; answer_marker "Cited by the answer" render; `evidencePaneMode` weak-degrade pin; ready read-only ×2; D-7 gating; bulk absence; genuinely keyboard-only walk §28.10 through link + mark-ready; drawer focus-return), `ReviewLockSeam.test.tsx` ×2 (App-level flush→lockWorkspace→purge ORDER pinned by spy call-order; failed flush never blocks the lock), `lockPurge` covers the review store (all-four-stores purge), `evidence-review-gate` +2 (renderer `computeReadyGate` ≡ main `deriveReadyGate` matrix + exhaustive single-item sweep), `GermanSmoke` +1, `InformationArchitecture` +1 (review→home), `LazyScreens` +1 (full App handoff: chat → Review evidence → suspense fallback → workspace → back), `evidence-reviews` +1 (ready refuses all five item-level service mutations, head edits allowed, reopen restores), `evidence-reviews-ipc` +3 (restart survives DB close/reopen; vault lock refuses sampled channels → unlock intact — the EVERY-channel guarantee is `ipc-lock-coverage`, which auto-covers the 14th channel; count isolation/malformed legs). The zero-model/zero-network renderer gate is STRUCTURAL: file-wide `afterEach(assertNoUnexpectedApiCalls)` in `ReviewScreen.test.tsx` AND `ReviewEntryPoints.test.tsx` (fresh `stubApi` per test), not a per-test opt-in. Suite **4451/49** (was 4394/49) + typecheck + build green.
**Review fix round (2 independent reviewers, 0 blockers, 8 findings — 5 SHOULD-FIX + 3 NIT, all applied):** FIX-1 ready-state editability — a `ready` review was fully editable without Reopen, able to persist the spec-§18.4-invalid `ready`+undecided state (P3 would have exported a Ready pack with zero decisions): the five item-level service mutations now REFUSE (null/false) while ready (head edits stay allowed; recorded in data-contracts), the store mirrors the guard, and the renderer disables decisions/notes/links, hides bulk, and shows the `review.readonlyHint` line until Reopen. FIX-2 store races — every post-await state/pending write is now `openToken`-guarded (a link/flush resolution landing after `purgeReviewSession` could re-insert decrypted content post-lock; after a review switch it could spread a stale detail → footer crash), `flushReviewSession` loops until NO flush is in flight, `reopenReview` flushes first (a <600 ms-old rename was visually reverted), and the save-error retry with nothing pending now RECONCILES against the DB via `getEvidenceReview` (the stale-handle dead-end kept showing a never-persisted value). FIX-3 the lock seam was correct but unpinned (deleting the purge or reordering the flush passed the suite) — now pinned by `ReviewLockSeam.test.tsx` + the lockPurge four-store leg. FIX-4 the renderer gate mirror had no equivalence pin and the test fixtures derived gates FROM the mirror (circular) — equivalence matrix added, fixtures now use MAIN's `deriveReadyGate`. FIX-5 the "every flow" tripwire claim was true for 3 of 22 tests — made structural (afterEach in both files) and the claim sites corrected. FIX-6 the sources-footer entry ignored the streaming gate `MessageActions` honors — threaded + tested. FIX-7 keyboard walk made genuinely keyboard-only end-to-end (Tab/arrows only, through item 2, evidence linking, and Mark ready), `answer_marker` links now have a positive "Cited by the answer" render test (was absence-only), `evidencePaneMode`'s unknown-PRESENT→whole_doc weak-degrade pinned. FIX-8 precision — exact key count (93), the integration lock-leg title no longer overstates its 3-channel sample, file split corrected to 10 new + 23 modified. Reviewer-verified clean (no action needed): bulk conservatism (adversarial), snapshot immutability, drawer focus-return, DecisionControl a11y idiom, honesty wording exact-key pins, D-2 fallback over-warns-never-silences, deviations 2/3 sound, store snapshot stability, genuine German (92=92 at review time). Two recorded observations for later phases: back-to-chat lands on the chat home (P5 UX-polish candidate); the consumed handoff slot is retained but ids-only and unreachable (`resolveNavTarget` never yields review; lock resets to home).

_2026-07-18 — **EP-1 Phase 1 DONE (branch `feat/ep1-p1-snapshot-ipc`, plan §6): snapshot engine + the full IPC surface — `createEvidenceReview(messageId)` builds a complete, honest, deterministic draft review from persisted data only; 13 `evidence:*` channels + preload; model runtime provably never touched.**_
As-built record: `docs/data-contracts.md` "Evidence Pack / Review Mode" (Phase-1 block: the channel table + module contracts) + new `docs/rag-design.md` §16 (snapshot read-model + honesty mapping over §8/§14.4/§14.9/§14.10). Shipped: (1) **shared marker source** `shared/citation-markers.ts` — `CITE_CODE_SPLIT_RE`/`CITE_MARKER_RE` MOVED out of `displayMap.ts` (which now imports them; zero display change) + `extractCitationMarkers`, so display rewrite and review marker extraction can never drift (parity pinned against the real `localizeServerCopy`); (2) **pure segmenter** `main/services/evidence-pack/segment.ts` — deterministic line scanner (paragraph/list_item/heading/fence/table/blockquote; fences swallow unclosed to EOF; nested list children attach to their top-level item; ≥2-pipe-line tables are ONE unit), `blockKey = b{ordinal}-{kind}-{sha256/12}` stable against the snapshot (spec Risk 7); (3) **snapshot builder** `evidence-pack/snapshot.ts` — sources resolved honestly (`documentId` pins identity, row-gone ⇒ resolved+missing; legacy title match only when UNIQUE, else `identity:'unresolved'`; never guessed), `kind` from `coverage.mode` (relevance→direct_excerpt, tree|capped→whole_document_provenance, extract→structured_record; NO stamp → direct_excerpt = the pre-D72 renderer fallback while `answerMode:'unknown'` records what was stamped), all-optional generation snapshot from conversation row + injected `appVersion`/catalog display name (electron-free, deterministic under test), items with `block_kind` on EVERY row + headings defaulted `not_applicable`, auto-links (`origin:'answer_marker'`) for direct-excerpt answers ONLY — whole-doc answers get ZERO even with a literal `[S1]` in the text (spec §13.3 hard rule, tested); source keys written BEFORE links (the P0 watch-out); failure deletes the half-built review; (4) **IPC**: 13 `evidence:*` channels (spec §19 minus `exportEvidencePack` — pipeline is Phase 3; plus `evidence:reopen` — spec §18.4 needs it) in `registerEvidenceReviewsIpc.ts` + same-named preload methods; every handler `requireUnlocked` (`main.evidenceReviews.locked` EN+DE; auto-enforced — the module is enumerated in `ipc-lock-coverage.test.ts`); payload guards DROP malformed fields (never coerce); create is IDEMPOTENT (existing review returned; checks `getEvidenceReviewForMessage` first per the P0 watch-out; fallback title `main.evidenceReviews.defaultTitle` when a conversation title trims empty — persist-canonical EN, in `DISPLAY_MAP_KEYS`); **renderer `setEvidenceLink` payloads are FORCED `origin:'reviewer'`** — only the snapshot builder mints 'answer_marker' (a compromised renderer can't fake "cited by the answer"); `refreshEvidenceReviewState` stubbed `{reviewId, outdated:false}` (Phase-4 seam); first audit emitters `evidence_review_created/_ready/_deleted` (ids/counts only, sentinel-swept); (5) pure shared `isReviewEligible(message, conversation?)` (`shared/evidence-review.ts`, spec §9.1). `parseCitations` now exported from `chat.ts` (the `parseCoverage` precedent). Hard rules PROVEN in-suite: `evidence-reviews-ipc.test.ts` runs every flow against a runtime TRIPWIRE (zero touches asserted) with the REAL `installOfflineNetworkGuard` active (zero violations) — plan §6's mandatory no-model/no-network assertions. Exit gate met: create→read→update→delete for all four answer classes (relevance / whole-doc tree / extract / legacy no-citation) entirely from persisted data. Review fix round (2 independent reviewers, 1 blocker + 3 should-fix + 1 nit, all applied): **FIX-1** a PRESENT-but-unrecognized `coverage.mode` (a workspace written by a NEWER app — `parseCoverage` accepts any string) used to fall through the switch `default` into `direct_excerpt` and mint `answer_marker` links — now maps to the WEAKEST claim (`whole_document_provenance`, no labels, zero links) with a compile-time-exhaustive switch (a new `CoverageMode` member reds the build; rag-design §16.1 gained the row); **FIX-2** creation is now ONE transaction around head+items+links (new exported NON-transactional core `insertEvidenceReviewItems`; the old catch's unguarded `deleteEvidenceReview` cleanup could mask the original error AND leave a half-built review the idempotent create would adopt forever — pinned by an injected SQLite-trigger failure test: all four tables empty, original error surfaces); **FIX-3** markers are now extracted ONCE over the whole snapshot with offsets (`extractCitationMarkerOffsets`) and assigned to blocks by range — per-block extraction diverged from the whole-message display split when a code region spans block boundaries (mid-line ``` swallowing to EOF over-claimed a link the UI renders as code; a region closing mid-line dropped a link the UI renders as citation; both repros pinned two-sided, unit + end-to-end); **FIX-4** tripwire coverage made match the "every flow" claim (real `deleteSelection` success + a documentId-resolved source under the tripwires; the guard/refusal tests now assert `runtimeTouched`/`offlineViolations` too); **FIX-5** `markEvidenceReviewReady` on an already-ready review is a NO-OP (original `completed_at` kept, no re-audit; service returns additive `becameReady`, the handler audits on it and strips it from the wire). Tests +52 net (unit `evidence-segment` ×26; integration `evidence-snapshot` ×19; `evidence-reviews-ipc` ×6 — every test in the file asserts the runtime tripwire + real offline guard; +1 lock-coverage module row; audit-ipc gains the 5 review sentinels + 3 event legs inside existing tests). Suite **4394/49** (was 4342/49) + typecheck green. NOT in this phase (per plan): export pipeline (P3), renderer/UI (P2), real freshness (P4), delete-confirm copy (P2).

_2026-07-18 — **EP-1 Phase 0 DONE (branch `feat/ep1-p0-contracts-storage`, plan §5): the entire persisted layer + service CRUD for Evidence Pack / Review Mode — contracts, 4 tables, tolerant parsers, D-7 gate, additive `Citation.documentId`/`chunkId` — no IPC, no UI, zero behavioral diffs.**_
As-built record in `docs/data-contracts.md` "Evidence Pack / Review Mode (EP-1 Phase 0 live)". Shipped: (1) shared types (`ReviewDecision`, `EvidenceReviewStatus` 'draft'|'ready' with `outdated` DERIVED-only per spec §18.4, `EvidenceSourceSnapshot` + the plan-§1.2 `identity: 'resolved'|'unresolved'` honest legacy state, all-optional `EvidenceGenerationSnapshot`, item/link/summary/detail/export shapes, IPC patch/input types); (2) `db.ts` SCHEMA + `evidence_reviews`/`evidence_review_items`/`evidence_review_links`/`evidence_exports`, FK chain `messages → reviews → items → links` (+ `reviews → exports`) all `ON DELETE CASCADE` (the `result_tables` template — the REAL `deleteConversation` drops the chain, proven by test), snapshot JSON columns nullable, one-review-per-message service-enforced (no UNIQUE), plus one deliberate column beyond spec §18.1: nullable `evidence_review_items.block_kind` so the D-7 heading exemption is exact (NULL = required — corruption can't exempt a block); (3) `main/services/evidence-reviews.ts` CRUD + tolerant row→DTO parsers (safe defaults point AWAY from unearned confidence: unknown decision→'not_reviewed', status→'draft', link origin→'reviewer', source kind→'whole_document_provenance', identity→'unresolved') + pure `deriveReadyGate` (D-7: non-heading blocks required, 'not_applicable' counts as decided, selections never gate); (4) additive `Citation.documentId?`/`chunkId?` stamped at the SIX constructors that exist today, each pinned by a test (enumerated deliberately — nothing enforces the stamp on a future constructor): relevance retrieval / `retrieveWholeDocument` / `answerWholeDocFromChunks` reps / compare-diff `buildDiffResult` (all `rag/index.ts`), `documentLeafProvenance` (`analysis/coverage.ts`, the whole-doc-tree path), and `chunksToCitations` (`skills/analysis/common.ts`, the bank/invoice deterministic-answer path — `loadCitationChunks` SELECTs `id`, the projection takes `documentId`; review fix round); `isCitation` extended tolerantly, legacy rows parse byte-identically (pinned by test); (5) audit literals `evidence_review_created/_ready/_deleted` + `evidence_pack_exported` (types only, no emitter until Phase 1) — the exhaustive Diagnostics label record forced the 4 `diag.audit.*` EN+DE keys along (behaviorally inert; the one deviation from the phase brief's "no i18n/renderer changes", recorded for the Handoff Log). Deliberately NOT in this phase (plan): snapshot builder, segmenter, IPC/preload, export pipeline, freshness engine (`outdated` always false until Phase 4). Review fix round (2 independent reviewers, same branch): skill-analysis citation path enriched (the sixth site above — the initial "all sites" claim had missed it); `createEvidenceSelection` REFUSES surrogate-splitting offsets (F-15 class — a misaligned cut would persist a lone surrogate; refuse-never-clamp); `createEvidenceReview` refuses a trimmed-empty title (D-6 never-unnamed holds from birth, title stored trimmed); `updateEvidenceReviewItem` normalizes a patched decision ON WRITE (an unknown literal never enters storage); `evidence_exports.format` passes through RAW on read (a repair-default toward 'html' would invent an export claim — the honesty-rule tiebreak), DTO `format: string`, writes stay typed. Tests +42 total (unit `evidence-review-gate` ×18: D-7 matrix + pure snapshot parsers; integration `evidence-reviews` ×23: schema fresh+reopen, hostile-string round-trips of every table, malformed-JSON tolerance, cascade via real `deleteConversation`, service status derivation, all-six-site enrichment + legacy byte-parity, surrogate/title/decision guards; +1 enrichment leg in `citation-snippet-boundary`). Suite **4342/49** (was 4300/49) + typecheck green.

_2026-07-18 — **EP-1 Evidence Pack / Review Mode wave OPENED: implementation plan + feature spec landed (docs-only PR); execution = orchestrator + one sub-agent per phase per plan §13.**_
[`docs/evidence-pack-implementation-plan.md`](docs/evidence-pack-implementation-plan.md) is the single source of truth while the wave is open: phases §5–§11 (P0 contracts/storage → P6 PDF export + close-out), all §4 decisions D-1…D-10 owner-resolved 2026-07-18 (D-1: PDF via Electron `printToPDF`, research record in the D-1 row; D-10: per-phase PRs to master), handoff protocol §13, append-only §14 Handoff Log (one entry per phase close). The spec (`docs/evidence-pack-review-mode-feature-spec.md`) is reference only — where they disagree the plan wins; its §1 records the three deliberate deviations (printToPDF over a PDF-writing dependency; legacy answers' source identity resolved best-effort by title, honest "unresolved" state; generation metadata synthesized at review creation, absent = "Unavailable"). Both files retire at close-out per the doc-lifecycle rule (plan §11 item 2, transfer map §12); CLAUDE.md's open-plan parenthetical updated to name the plan. Zero code changes in this PR.
