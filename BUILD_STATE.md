...
_2026-07-19 — **DEP-1 P4 — electron 37.10.3 → 39.8.10 (`^39.8.5` floor; Chromium 138→142, Node 22.21→22.22.1, SQLite 3.50.4→3.51.2): all 4 HIGH + 6 med + 3 low electron alerts cleared, `npm audit` 0 vulns (plan §4 P4).**_
Bumped `electron` via `npx npm@11.6.2` (lockfile pins 39.8.10, registry+integrity, `^39.8.5` floor); **also `electron-builder.yml electronVersion` 37.10.3 → 39.8.10** — the pin that selects the SHIPPED runtime (the first package built E37 until fixed; NO test guards this sync — follow-up candidate). §2.1 ledger, all on the real E39 binary: **(1)** suite **4678/50/4728, 334 files** exact parity; node:sqlite/FTS5 + webUtils legs run in-suite, none skipped. **(2)** `npm run dev` boots clean (mock embedder, offline); packaged chat roundtrip (mock) + RAG ask (citations=1) via Playwright `_electron`. **(3) packaged win:** BE-2 CSP re-take — the `onHeadersReceived` header ATTACHES+ENFORCES on `file://` in BOTH windows (base-uri + ocr `blob:` blocked, `originalPolicy`==`buildCsp(false)`), both baked metas localhost-free; Evidence-Pack PDF export → valid 145 KB `%PDF…%%EOF` (D-1 options accepted); OCR raster→tesseract **done 3/3, 2 pages, text recognized** on the E39 binary (dev — PACKAGED OCR blocked by a PRE-EXISTING, version-independent tesseract.js `asarUnpack` gap: hoisted regenerator-runtime/is-url not unpacked; NOT a bump regression — follow-up). **(4)** node:sqlite in E39 Node 22.22.1 / SQLite 3.51.2: FTS5 `snippet`/`highlight`/`bm25` all work. **(5)** `Uint8Array.prototype.toHex` is NATIVE on Chromium 142; the repo has NO polyfill (OCR uses the legacy pdfjs build + same-realm copy) → nothing to guard. **(6)** electron-builder 26.15.2 (no electron peer/cap; current 26.x line) packaged E39 (empirical). Typecheck ✓; reproducibility proven under npm 11.6.2. Living version docs refreshed (architecture/security-model/packaging/data-contracts + db.ts/print-pdf.ts/ocr comments); dated ledgers/research-gates stay historical. Alerts auto-resolve at merge: #1–18 / #23–40 (all electron).

_2026-07-19 — **DEP-1 P3 — vite 5.4.21 → 6.4.3 + electron-vite 2.3.0 → 3.1.0, esbuild 0.21.5 → 0.25.12 everywhere: HIGH #42/#20 + mediums #43/#21, #38/#16, #22 cleared (plan §4 P3).**_
Both bumped in `apps/desktop/package.json` (`^6.4.3` / `^3.1.0`, caret kept) via `npx npm@11.6.2`; vite **6.4.3** is a single deduped copy (`@vitejs/plugin-react` 4.7.0 — no bump —, `electron-vite` 3.1.0, `vitest` 3.2.6 / `vite-node` 3.2.4 all resolve onto it, no nested vite 5). esbuild **0.25.12** is the ONLY resolved copy (vite 6 + electron-vite 3.1.0 deduped) — cluster 6 cleared with NO `overrides`. **ZERO config migrations required:** the vite-6 plugin API is back-compatible here (`transformIndexHtml {order:'pre',handler}` + `configResolved` stable; no sass / json.stringify / import.meta.glob / Environment-API surface used) — the `hilbertraum:csp-meta` transform survives untouched (build-path CSP proof green) and the "CJS build of Vite's Node API is deprecated" warning is GONE. Gates (final tree, after the csp-meta interlude fix `4bf3249b`): typecheck ✓, build ✓ (vite 6.4.3), csp build-output tooth **4 pass/1 skip** + both baked prod metas localhost-free, full suite **4678 pass / 50 skip (4728), 334 files** (= the prior 4675/50/4725, 333 files + the interlude's +3 tests / +1 file), `npm run dev` launches + HMR verified (`[vite] (client) hmr update /App.tsx`; the session's inherited `ELECTRON_RUN_AS_NODE` was cleared for the run — an agent-shell artifact, not a repo condition), `npm run preview:build` ✓. Reproducibility proven under npm 11.6.2 (`ci` + `install --package-lock-only` → byte-identical lockfile). No product `src/` change; no living doc names vite 5 / electron-vite 2 (grep: only the uncommitted plan file). Alerts to auto-resolve at merge: HIGH #42/#20 (vite `server.fs.deny` bypass), #43/#21, #38/#16 (vite mediums), #22 (esbuild dev-server CORS).

_2026-07-19 — **DEP-1 P3 interlude — `csp-meta` dev-serve false "no CSP meta tag" throw FIXED (guard on tag ABSENCE, not the `replaced === html` no-op): `npm run dev` renderer no longer 500s. Latent since OCR-R P5 (14259f68).**_
Mechanism: dev serve rewrites the meta with `buildMetaCsp(true, page)`, which BY DESIGN equals the checked-in dev policy verbatim (window-security keeps the meta ≡ the dev policy), so the correct rewrite is a no-op (`replaced === html`) → the old guard read that as a missing tag and threw. Fix (`electron.vite.config.ts`, guard line only): test `!cspRegex.test(html)` before replacing; the regex + replacement expression are unchanged (regex is non-global → `.test()` then `.replace()` reuse is lastIndex-safe). Security-neutral — build path byte-identical (csp tooth 4 pass/1 skip + both baked prod metas localhost-free, re-verified post-fix). RED→GREEN: new `tests/unit/csp-meta-plugin.test.ts` (+3 tests; the index/ocr dev-rewrite legs were RED pre-fix, the no-meta-throws tooth stays). Discovered at the DEP-1 P3 dev-launch gate — the first live dev serve since 14259f68 (agent shells inherit `ELECTRON_RUN_AS_NODE`, which exited electron before it requested the page and masked this).

_2026-07-19 — **DEP-1 P2 — vitest 2.1.9 → 3.2.6 + @vitest/coverage-v8 3.2.6: CRITICAL CVE-2026-47429 (Vitest UI-server arbitrary file read/execute) cleared (plan §4 P2).**_
Both bumped in `apps/desktop/package.json` to `^3.2.6` (caret kept) via `npx npm@11.6.2`; the whole `@vitest/*` cluster + `vite-node` (→3.2.4) moved to 3.2.x (ride-alongs: `@vitest/pretty-format` 3.2.7 via vitest's own caret dep, tinyrainbow 1.2.0→2.0.0; tinypool unchanged 1.1.1). `vitest.config.ts` UNCHANGED — `pool:'forks'` + deterministic-collection posture preserved. **Zero migrations needed:** the 2→3 breaking surfaces this suite actually touches — the custom `Reporter.onFinished`/`File.name` full-suite guard, one `mockReset()` on a plain `vi.fn()`, `useFakeTimers` across the renderer tests — are all v3-safe, proven by green typecheck + exact-parity suite (no config/helper/product edits; product `src/` zero diff). Gates: typecheck ✓, build ✓ (build-before-test), full suite **4675 pass / 50 skip (4725), 333 files (312 pass / 21 skip)** = exact baseline parity; `npm audit` shows no CRITICAL left (only electron→P4, esbuild→P3). Reproducibility proven under npm 11.6.2 (`ci` + `install --package-lock-only` → byte-identical lockfile). **Coverage:** full-width `npm run test:coverage` STARVED this 16 GB (~4 GB free) machine twice (attempt 1: 5 timing-class failures — Argon2id/KDF + 50k-chunk-corpus tests — plus a `[vitest-worker] Timeout calling "onTaskUpdate"` RPC error @493s; attempt 2: 2 timing-class failures; a maxWorkers=4 try then died mid-run OOM) — the repo's documented parallel-fork starvation class under V8-instrumentation overhead, NOT a vitest-3 regression (the plain suite was all-green at parity). Coverage verified GREEN at exact parity via `npx vitest run --coverage --maxWorkers=2` (from apps/desktop): **4675 pass / 50 skip (4725), 333 files (312/21), 511.79s, v8 report produced**. Script + CI left unchanged (CI runs `npm test`, not coverage). Honest caveat: NO recorded full-width-coverage baseline exists at the current 4725-test size (coverage added TS-8 at ~4024 tests), so this capacity constraint cannot be attributed to vitest 3-vs-2 either way without a counterfactual deliberately skipped (cost/benefit). Owner follow-up candidate (close-out): cap `test:coverage` parallelism or document a RAM floor for full-width coverage.

_2026-07-19 — **DEP-1 P1 — form-data 4.0.6 + undici 6.27.0 (node-gyp) / 7.28.0 (jsdom): HIGH #44/#46/#49 + 7 medium/low cleared (plan §4 P1).**_
`npx npm@11.6.2 update form-data undici` moved all three targets on the first attempt (form-data 4.0.5→4.0.6, undici 6.26.0→6.27.0, undici 7.27.2→7.28.0) — no `--include-workspace-root`, no root `overrides` needed. Lockfile-only diff (`package-lock.json`, 3 stanzas, npmjs.org + integrity hashes, zero ride-alongs); reproducibility proven under npm 11.6.2 (`ci` + `install --package-lock-only` → zero further diff). Gates: typecheck green, build green, suite **4675 pass / 50 skip (4725)** at exact baseline parity. Flake note for later phases: the first `npm test` run crashed via a Tinypool "Worker exited unexpectedly" fork-pool crash (no test assertion failed; matches this repo's documented CPU/memory-starvation full-suite flake history — CONTRIBUTING.md, build-log.md); the immediate rerun was clean at full parity — treat a bare fork-pool crash as retry-worthy, not an automatic regression.

_2026-07-19 — **DEP-1 dependency-remediation wave OPEN (branch `fix/dependabot-2026-07`): 48 open Dependabot alerts = 6 clusters (vitest CRITICAL, highs in electron/vite/form-data/undici, esbuild ride-along), cleared in 5 sequential phases, one commit each, single PR at close.**_
Wave-open facts: baseline `master` @ `11e0a4fa` (squash-merge of #76; content-identical to the plan's `145766ff`). Baseline gates measured on this machine: suite **4675 pass / 50 skip** (the plan's 4664/50 predates the +11 skills-parity tests of #76), typecheck green, `npm run build` green — the CSP build-output test needs build-before-test order (a stale dev build in `out/` fails its 4 prod-meta legs; CI's order already guarantees this). Lockfile hazard found + fixed in the wave-open commit: local npm is 10.9.3 vs the pinned `packageManager` npm@11.6.2, so ALL lockfile-writing ops this wave run through `npx npm@11.6.2`; the committed lockfile was no longer canonical under 11.6.2 (missing root `engines.npm` + apps/desktop `license` metadata — package.json edits post-dating the last regen), synced (+3/−1, zero dependency moves). Registry pre-verification: every plan fix-version exists (vitest 3.2.6 + coverage-v8 3.2.6; vite 6.4.3 = newest 6.x, deps esbuild ^0.25.0; electron-vite **3.1.0** = newest 3.x, peers vite ^6, deps esbuild ^0.25.1; electron 39.8.10 ≥ the 39.8.5 floor; form-data 4.0.6; undici 6.27.0/7.28.0). Working paper: `docs/dependabot-remediation-plan.md` (uncommitted by design, carries the per-phase Handoff Log; deleted at close-out).
