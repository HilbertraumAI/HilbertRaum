...
23. **Models/runtime wave residuals (2026-09-07, PR for #310/#312/#313/#314/#315):** (a) #312: no per-model "cannot load" session latch — an unloadable model re-walks up to three 180 s health timeouts per start; (b) #310: no older-build fence for `files:` manifests (a `format` marker is #311's containment call) and no per-file view in `ModelInfo`/the Models screen; the shard-completeness rule covers `local_path`, not `mmproj.local_path`; (c) #314: download jobs still die with the app process (renderer reload only); (d) #315 item 5 (name shown twice) left as is.
