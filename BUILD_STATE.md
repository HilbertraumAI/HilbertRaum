...
21. **ZIM knowledge packs — follow-up register (registered at the 2026-09-04 MVP; durable
    ...
    **UNBLOCKED by D-Z22** (the defect-route traffic argument is void; scope — opt-in per ask vs on by default — and the converter keeping
    ...
    (l) **P7 residual register — CLOSED 2026-09-06.** Collision surface (D-Z16), UI nits (design-guidelines §11.15 "Follow-up wave"), T18-b/T19 (rag-design §17 "Real acceptance"), #344 (PR #348), the post-add probe (D-Z15), LaTeX (PR #361), the non-ASCII add message (PR #360), L3 (D-Z18) + the #353 ladder (PR #366) — all landed; R-1 closed by P8-2's in-app install, R-2 multipart unsupported, R-6 → L1 kept as is.
    Full text: `docs/build-log.md` "2026-09-06 — ZIM open-issues wave: item 21 (l) retired verbatim". T18-b's owner visual leg is the only piece of the register still open.
