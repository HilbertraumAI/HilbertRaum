...
23. **Models/runtime wave residuals (2026-09-07, PR #371 for #310/#312/#313/#314/#315; owner rulings 2026-09-07):** (a) #312 per-model "cannot load" session latch + the model-named notice when acceleration is off → **#372** (filed; owner: do both); (b) #310 no older-build fence for `files:` manifests — **accepted** (owner: no old versions in use; a `format` marker stays #311's call); no per-file view in `ModelInfo`/the Models screen; the shard-completeness rule covers `local_path`, not `mmproj.local_path` — owner: leave; (c) #314 download jobs die with the app process — owner: leave, dedicated issue only on request; (d) #315 item 5 (name shown twice) — **fixed in PR #371** (the panel names the model only while its row is off screen).
## 6. Open issues / risks
