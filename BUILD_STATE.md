...
_2026-07-18 — **OCR-R P5: packaged CSP truth measured + meta drift fixed (BE-2) + rasterizer wiring harness (audit test-gap #2) — ocr-audit 2026-07-18.**_
**Step A probe ACHIEVED autonomously** (packaged `npm run package:win` build, temporary main-process instrumentation logging each window's effective CSP + localhost-fetch probes to a file, then byte-reverted — SHA-verified): on Electron 37, `onHeadersReceived` DOES fire for `file://` loads in BOTH windows and the injected `buildCsp(false)` header IS ENFORCED (a fetch the then-shipped meta allowed was blocked with the header string as `originalPolicy`; env gotcha en route: `ELECTRON_RUN_AS_NODE` had to be cleared to launch the packaged exe). So the header — not the meta — is the load-bearing packaged policy; BE-2's erosion is real but second-layer. Fix (Step B): new `buildMetaCsp(isDev, page)` in window-security.ts is the SINGLE source of truth for both pages' meta tags, rewritten at build time by the new `hilbertraum:csp-meta` `transformIndexHtml` plugin (fails the build loudly if a page lacks the tag); prod metas = dev metas with ONLY the `ws://localhost:*`/`http://localhost:*` connect-src entries stripped, the ocr page keeping pdfjs's `worker-src 'self' blob:` + `img-src data: blob:` (effective packaged policy = header ∩ meta, provably unchanged — measured: a real pdfjs rasterization through the actual hidden window succeeded in the packaged build under the new CSP, pageCount 1, PNG bytes out). Checked-in HTML stays the dev policy (HMR); dev behavior byte-identical. Step C: new `ocr-rasterizer-harness.test.ts` (10 tests, fake-Electron pattern) — destroy on success/error/timeout/abort, in-use latch + throw-reset, exact 60 s step-timeout pin (`RASTER_STEP_TIMEOUT_MS` now exported, the PRINT idiom), sender-spoof rejection on all three reply channels, listener removal, and P4's `render-process-gone` fast-fail; new `csp-build-output.test.ts` (4 tests, watched RED against the pre-fix baked output: both prod metas byte-equal `buildMetaCsp(false,…)` and contain no `localhost` ever). window-security pins +5. Docs: security-model CSP section rewritten to the MEASURED two-layer mechanism (header load-bearing on file://, meta = defence-in-depth, intersection semantics); packaging.md build-time CSP generation bullet. Deferrals for close-out: packaged OCR smoke's RECOGNITION leg impossible here (no `*.traineddata.gz` on this machine; the CSP-exposed rasterizer leg IS verified) + macOS/Linux packaged smoke. Suite 4664/50 + typecheck + build green.

_2026-07-18 — **OCR-R P4: backend hardening batch (BE-5, BE-6, BE-7, BE-8, BE-9 code-half + test-gap #5 ride-along) — ocr-audit 2026-07-18.**_
BE-8 first (smallest): `parsers/image.ts` forwards `{ signal: ctx?.signal }` into `engine.recognize` — a cancelled photo import now aborts the recognition instead of waiting it out. BE-5: (a) `ensureWorker()` refuses when `this.stopped` (a recognition queued before `stop()` can no longer lazily respawn a worker past the will-quit teardown); (b) new non-latching `OcrEngine.suspend?()` (terminates the warm WASM worker, next recognition respawns — `stop()` stays quit-only per plan §8-#5: a terminal stop in the lock list would brick OCR until relaunch) added to the workspace-lock teardown `Promise.allSettled` in `registerWorkspaceIpc` — sidecar parity restored; `tesseract.ts` header comment now distinguishes the two teardowns. BE-6: `pipelinePages` gains `maxPages` (pure-pipeline enforcement, `min(declared, cap)`); the rasterizer resolves `resolveIngestionLimits().pdfMaxPages`, logs the truncation and reports the CLAMPED count via `onPageCount` so `stepsTotal`/progress/`ocr_json.pageCount` stay truthful — M-2 parity with the text parser; known-limitations bullet added. BE-7: `renderer/ocr/main.ts` calls `page.cleanup()` in a `finally` after each page's PNG encode; `rasterizer.ts` subscribes `render-process-gone` → `slot.fail(...)` (a dead hidden renderer fails the run immediately instead of burning 60 s per remaining step; listener joins the `finally` teardown) — automated gone-event teeth arrive with P5's harness, `cleanup()` memory profile stays a §8 manual-smoke deferral. BE-9 code-half: engine id derived `` `tesseract.js-${version}` `` via `createRequire('tesseract.js/package.json')` (packaged-safe idiom; byte-identical 'tesseract.js-7.0.0' today; `resolveVersion?` injection seam for RED provability). Ride-along (test-gap #5): `encrypted-documents.test.ts` pins the `<id>.parse-ocr.pdf` transient name to the crash-sweep matcher. Tests +9, each behavioral fix watched RED pre-fix (BE-7's gone-event is the deliberate P5-harness exception). The F-41 `as never` ratchet caught the two new casts the tests initially copied from the file's idiom — resolved properly (typed `TesseractModule` slice), baseline NOT bumped. Suite 4647/49 + typecheck + build green.

_2026-07-18 — **OCR-R P3: docs & comments truth batch (DOC-1 High + DOC-2/3/4/5, BE-3/BE-4 comments, BE-9 docs-half) — ocr-audit 2026-07-18.**_
Docs/comment-only, suite count unchanged. DOC-1: architecture.md's Phase-38 record paragraph no longer claims "Cancel persists nothing." — replaced with the two-part GAP-7 contract (pre-persist cancel persists nothing; a cancel during the signal-less final re-ingest is deliberately ignored and completes `'done'`). DOC-2: known-limitations OCR bullet (a cancel during "Finishing…" can't stop the task; row shows "Stopping if possible…"; doc completes searchable) + user-guide "You can always cancel" softened to "most tasks" with the OCR-finishing exception. DOC-3: the "recognized text survives re-index" bullet now discloses it is PDF-only (a photo re-recognizes from scratch each re-index, seconds). DOC-4: the second ceiling named in known-limitations + architecture's timeout ¶ — fixed 60 s per-step RENDER timeout (`RASTER_STEP_TIMEOUT_MS`, not env-tunable) besides the 2-min recognition ceiling (`HILBERTRAUM_OCR_PAGE_TIMEOUT_MS`). DOC-5 docs-half: troubleshooting OCR remedy + drive-layout both say restart the app after adding OCR files (availability resolved once at startup). BE-3: `handlers/ocr.ts` backpressure comment now states the ING-5 truth (recognitions serialize; 1-deep render look-ahead). BE-4: `doctasks/context.ts` getOcrEngine comment drops "can appear mid-session" — fixed at startup, restart after installing assets; behavioral refresh deliberately deferred (§8 register). BE-9 docs-half: security-model line — pdfjs/tesseract.js pinned via lockfile only, review upstream advisories each release, offline posture has no auto-update channel. Consistency note (Documents +1-by-label vs Translate subtraction) verified already recorded in P1's Phase-38 bullet — not duplicated. Stale-phrase cross-check: "Cancel persists nothing" + "can appear mid-session" zero-hit repo-wide outside the two working papers (which quote them historically and are deleted at P6). repo-hygiene 12/12; suite 4639/49 byte-green + typecheck + build green.

_2026-07-18 — **OCR-R P2: Translate explains scanned PDFs honestly (FE-3) — ocr-audit 2026-07-18.**_
`fileTranslateSession.runImport`'s imported-but-nothing-ingested branch no longer collapses every ingest failure to `fail('unsupported')` ("try a PDF…" — nonsensical for a file that IS a PDF): new `resolveImportFailure` reads the failed row via read-only `listDocuments` (no new IPC surface, generation-guarded per the module's F8 pattern) and branches — `scanDetected` (the canonical main-derived flag, not a renderer string re-match) → new failure code `scanned` rendering `translate.file.err.scanned` (EN+DE: make it searchable first in Documents, then translate — copy consistent with P1's "on this row" wording); any other ingest failure (corrupt/encrypted/timeout) → `failWith(errorMessage)`, localized on-screen via `localizeServerCopy`; unreadable row/message → the existing generic `importFailed` frame. The genuinely-unsupported EXTENSION path (nothing imported) stays byte-identical; the `FileTranslateErrorCode` union gains `scanned`, exhaustiveness compile-enforced via the `FILE_ERR_KEY` Record. Tests +3 (scan → handoff copy + no task start, corrupt-DE → real localized reason, unsupported control byte-unchanged; 2 watched fail pre-fix, control green throughout) — closes the FE-3 slice of audit test-gap #1; the ONE pre-existing pin of the old collapse (`fileTranslateSession.test.ts` "completed === 0 is unsupported") updated to the new contract (unreadable row ⇒ `importFailed`), inside the phase's stated blast radius. Docs: user-guide Translate ¶ (scanned PDFs: make searchable first), known-limitations Document-translation bullet. Suite 4639/49 + typecheck + build green.

_2026-07-18 — **OCR-R P1: OCR initiation restored (FE-1 Critical) + D33 redo (FE-2) + admission guard (BE-1) + honest finishing/stopping display (FE-4/FE-5) — ocr-audit 2026-07-18.**_
BE-1 landed first (interlock order): `DocTaskDeps.isDocumentProcessing` predicate (late-bound via new optional `AppContext.docIngestionActive`, assigned where `registerDocsIpc`'s `processing` set lives — predicate injection, set not moved) refuses `startDocTask` for EVERY kind while a target doc is mid-import/re-index (`main.task.documentBusyIngesting`, EN+DE; absent predicate ⇒ never busy, old test deps byte-green). FE-1: the failed-row branch renders an inline primary "Make searchable (OCR)" button when `showOcr` (a detected scan is by definition a failed row and failed rows have no ⋯ overflow — the old overflow item was provably dead code and is replaced by FE-2's item); `docs.scan.ocrOffer` reworded "below"→"on this row" EN+DE. FE-2: "Read again (OCR)" (`docs.makeSearchableAgain` + redo-vs-Re-index title) in the healthy-row overflow, gated `d.ocr != null && ocrAvailable`, disabled while any task active. FE-4: busy label switches to `docs.task.ocrFinishing` at the final re-ingest step (count keeps the record's "+1"; label-switch, not the Translate subtraction — the two conventions documented in the Phase-38 record); FE-5: Cancel stays enabled, a cancel click renders the disabled "Stopping if possible…" (`docs.task.stopping`) per GAP-7. Tests +11 (9 renderer + 2 backend; 7 renderer + 2 backend watched fail pre-fix, 2 deliberate green controls) — closes audit test-gaps #1 (initiation slice) + #3. Docs: user-guide "Scanned PDFs" (button on the row, finishing wording, redo item), architecture Phase-38 record (initiation surface, admission guard, finishing-label display contract). DOC-1 (`architecture.md:1567`) deliberately left to P3. Manual dev-run smoke deferred to P5's packaged check. Suite 4636/49 + typecheck + build green.

_2026-07-18 — **OCR remediation wave "OCR-R" OPEN (P0): the 2026-07-18 OCR audit's 18 findings + 5 test gaps enter remediation.**_
Wave-open per `docs/ocr-remediation-plan-2026-07-18.md` (committed alongside `docs/ocr-audit-2026-07-18.md` on this branch as the in-flight reference; both are working papers, deleted again at P6 close-out per the doc-lifecycle rule). Owner amendment to the plan's §1 branch table: ONE branch `fix/ocr-remediation-2026-07-18`, sequential phase commits, one PR merged at wave end — not stacked per-phase branches. Phases: P1 initiation interlock (FE-1 Critical + FE-2/BE-1/FE-4/FE-5), P2 Translate handoff (FE-3), P3 docs truth (DOC-1..5, BE-3/BE-4 comments, BE-9 docs), P4 backend hardening (BE-5..BE-9 code), P5 packaged CSP (BE-2) + rasterizer harness, P6 independent verification + ledger + close-out. Cite `OCR-R P<n>` + `ocr-audit 2026-07-18 <ID>` in commits.
