...
_2026-08-15 — **Qwen3.8-27B wave MEASURED (branch `bench/qwen3.8-27b-wave`, eval + docs only): four Unsloth quants (Q4_K_M / UD-Q4_K_XL / Q5_K_M / Q6_K) through the full §2+§3/§4 protocol on the i9 rig — quality ties the reigning `qwen3.6-27b` pair inside .003 F1 with ZERO hallucinations on all four; Q6_K fully fits a 24 GB GPU at ctx 8192. NO manifests, NO rank moves — evidence only.**_
Durable record: **`model-benchmarks.md` NEW §9.4** (tables + the two method findings that matter for reproduction: per-quant cool-GPU runs because GDDR6X thermal throttle moves tg by ~25% across a back-to-back sweep, and pp512's Vulkan boost-clock ramp ⇒ warm-median method). Raw files: `eval/results/i9-9900X-qwen38-vulkan-{quality,quality-rescored,items}` (one combined 4-model harness run) + `i9-9900X-vulkan-b10430-speed.csv` (SEPARATE file — measured on release b10430, not the b9849 pin; not comparable row-to-row with the pinned CSVs). CPU leg (Q4_K_M `-ngl 0`) running at entry time; row + a follow-up commit when done. Open before any promotion: license review (private/legal), real HF-LFS hashes + manifests, §9.1 per-quant smoke, owner ratification. Context: the maintainer's public benchmark post uses this run's speed numbers (setup disclosed as i9-9900X + RTX 3090, b10430 Vulkan, llama-bench).

_2026-08-10 — **v0.1.56 release-run failure FIXED (branch `fix/sha-helper-backslash-escape`, scripts-only): the #102 win-runtime step's FIRST real tag run exposed a latent `sha256_of` bug — GNU coreutils' backslash-escape prefix broke the checksum compare on git-bash Windows paths; all three shipped shell scripts now hash from STDIN.**_
Run 31341651519 (tag v0.1.56), build-win only: `fetch-runtime.sh` downloaded the pinned b9849 Vulkan zip fine but "FAIL: archive checksum mismatch (expected ed6156…, got \ed6156…)" — the hashes IDENTICAL except a leading backslash. Root cause: the step's `--target "$RUNNER_TEMP/drive"` is `D:\_temp/drive` under git-bash, and when a filename contains a backslash, coreutils sha256sum ESCAPES it and prefixes the whole output line with `\` — so `awk '{print $1}'` yields `\<hash>` and the string compare fails on a good archive (mac/linux never see backslash paths; #102's close-out had explicitly deferred real validation to "the NEXT tag run" — this one). Fix in the shared `sha256_of()` helper of ALL THREE scripts that carry it (`fetch-runtime.sh`, `fetch-models.sh`, `verify-models.sh` — same latent class): hash from stdin (`sha256sum < "$1"`) — no filename in the output, nothing to escape; sibling `shasum -a 256` branch same. Verified on the dev box in git-bash: the old form reproduces the `\`-prefix on a backslash path, the new form is clean, and a full `fetch-runtime.sh --target` run against a backslash path downloads + VERIFIES the same pinned archive end to end. `bash -n` ×3 + hygiene green. Release path: tag v0.1.56 to be re-pointed at the fixed head after merge (tag-triggered checkouts build the TAG's tree; no release object/assets were ever published — the release job was skipped).

_2026-08-10 — **Master push-run flake FIXED (branch `fix/dictation-rel3-ci-timeout`, test-only): the dictation REL-3 wedged-child test gets an explicit 120 s timeout — the #84/#97/#101/vault-lock starved-runner class, SECOND occurrence for this test and the first to blow even the 60 s CI budget.**_
The v0.1.56 bump's master push run (31337902509) failed ONLY windows/24.x, ONLY `dictation-ipc.test.ts` REL-3 ("Test timed out in 60000ms"), on a version-fields-only diff that was PR-green minutes earlier with 3/4 legs + the sibling windows leg green and the failing leg running ~2× the ubuntu legs — the 2026-07-24 vault-lock-cipher scenario verbatim. Same test flaked at the 15 s budget in PR #100's era ("pure fork starvation"; #101 subsumed the then-proposed per-test patch). Now that a starved runner has stretched it past the raised 60 s CI budget, the test gets the vault-lock-style explicit third-arg budget at 120 s (its OWN ceiling is the injected 30 ms `maxDurationMs`; every assertion semantic — nothing loosened). The failed jobs were re-run to correct master's commit status (the vault-lock precedent). Occurrence evidence in the test's header comment.

_2026-08-09 — **#171 follow-up SHIPPED (branch `fix/one-width-everywhere`, owner decision at live review): the two-tier scheme COLLAPSED to ONE width — every screen's container is the centred 1180px `.screen` frame; the reading measure moved one level down (72ch caps on `.lead`/`.hint`/`.offline-statement`); chat's full-bleed layout is the sole exception.**_
Decision path: after #172 merged, the owner clicked through the live build and judged the remaining reading-vs-workspace frame difference (Home/Settings/chat-empty at 860 beside 1180 elsewhere) as reading like inconsistency, not intent — chosen explicitly over keeping the tiers and over uncapped full width (the 72ch prose caps preserve what the 860 tier was FOR per §1.1/§1.5/§8, without the frame inconsistency). Built: `.screen` 860 → 1180; the six per-screen `max-width` rules DELETED (single source of truth; the class names stay in markup as semantic hooks — no TSX change); 72ch on the three prose roles. Durable record: **design-guidelines §14 "Amendment 2"**; CHANGELOG's unreleased #171 bullet rewritten to the end state. Guards: `screen-width-tiers.test.ts` rewritten — the single width, a no-per-screen-max-width sweep (any `.…-screen` rule declaring max-width fails), the scrollbar-gutter pin, three 72ch prose pins. Verified: typecheck + build + full suite green; eyeball walk — every screen 1180 with identical 271/271 gutters, Settings hint lines wrap at the 72ch measure, dark spot-check clean.

_2026-08-09 — **Issue #171 FIXED (branch `fix/171-list-screen-tier`, renderer + guard test + docs): the #166 fullscreen follow-up — AI model and Skills RE-TIERED to the 1180px workspace width (they are card/row lists, the §11.6 argument); Home, Settings and the §3 chat transcript deliberately KEPT at reading width, rationale recorded.**_
The owner ratified the issue's recommended option 1: tier FIT re-examined per screen, tier VALUES untouched. Built: `models-screen` class on all three ModelsScreen render sites (error/loading/main — stable width across states) + `skills-screen` on the SkillsScreen wrapper (SkillsTab's only render site — the section's "Settings →" CSS title is historical), two §14-cited CSS rules; the guard test's workspace list grows to six containers (sweep floor 5 → 7). NOT done, deliberately: Home/Settings widening (860px already yields ~105–115 chars/line, above the 45–90 readability ideal — the gutters are the recorded price of §1.1/§1.5/§8), any §3 transcript change, and fluid `clamp()` tiers (re-opens the width-drift §14 closed — rejected in the amendment). Durable record: **design-guidelines §14 "Amendment (2026-08-09, issue #171)"**; CHANGELOG "Changed". Verified: guard + Models/Skills renderer suites green targeted, then typecheck + build green and the full suite green except two KNOWN-family local flakes on the loaded weak box (the documented `evidence-pack-export-async-fs` PDF-tail 15 s local-budget timeout, and the #169 wave's new #157 dequeue test tripping its own `waitTerminal` 10 s bound — the #97 pattern; both green solo 45/45, neither reachable by a CSS/guard-test diff — the PR run is the clean-machine proof); eyeball walk re-measured — Models/Skills at 1180 with symmetric gutters, reading screens byte-identical. PLUS the owner's screenshot catch mid-review: same-width columns still LANDED differently per screen — the ~15px scrollbar consumed content width on scrolling pages only, shifting the centred column ~8px between nav clicks — fixed with `scrollbar-gutter: stable both-edges` on `.content` (pinned in the guard test; the walk now measures IDENTICAL gutters on every screen, 271/271 workspace and 431/431 reading, scrollbar or not).

_2026-08-09 — **Automated release-notes enrichment SHIPPED (branch `ci/auto-release-notes`, CI-only + docs): every tag's draft release now appends an "Issues resolved" section plus GitHub's generated notes (PR list with authors, New Contributors, Full Changelog link) below the curated CHANGELOG block — zero manual input.**_
Two layers in `release.yml`'s release job, both computed against the latest PUBLISHED release (same baseline → the sections agree; an abandoned draft folds into the next tag): NEW `scripts/release-issues-section.sh` (compare `PREV...TAG` → each commit's associated PRs via `commits/{sha}/pulls` — robust against squash AND true merge commits, unlike subject-line "(#N)" parsing — → each PR's `closingIssuesReferences` via GraphQL, i.e. the links GitHub tracks from "Fixes #N" keywords in PR bodies) appended to NOTES.md best-effort (API failure ⇒ `::warning::`, never blocks the build), then `generate_release_notes: true` on the action-gh-release step. Job permissions gain `issues: read` + `pull-requests: read` (top-level stays `contents: read`). Validated locally against the real `v0.1.55...master` range: 25 PRs → 52 issues with titles; the PREV==TAG and no-baseline edges exit empty. Like #102 item 1, the live notes render is only provable on the next tag/`workflow_dispatch` run. Docs: packaging.md release-job bullet. Categorized notes (`.github/release.yml` by PR label) deliberately NOT added — it depends on a manual labeling habit; the flat generated list is the automated floor.
