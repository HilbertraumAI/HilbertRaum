...
Outcome only — the durable per-alert ledger is **`docs/architecture.md` "Dependabot triage — design record (wave DEP-3, PR #115)"** (dispositions, reachability verdicts, mechanism, follow-ups). Headline: pdfjs-dist 6.0.227→6.2.108 (the one manifest edit, `^6.2.108` floor; CVE-2026-16633 never reachable in-app — no annotation scripting + `script-src 'self'` OCR CSP; real-data PDF gold-set green on the new version), mermaid 11.16.1 + dompurify 3.4.13 (all six alerts unreachable — plugin never wired, chain tree-shaken + asar-negated; NEW PIN in `assistant-markdown.test.tsx` fails if the mermaid plugin is ever wired in; corrects DEP-2's "dompurify ships in the renderer" framing — graph-scope only), dev-tooling hygiene (undici 7.29.0/6.28.0, fast-uri 3.1.5, postcss 8.5.26, js-yaml 4.3.1) + all 7 brace-expansion copies (npm-audit advisory-lag, DEP-2 precedent). Lockfile collateral: nanoid only (postcss floor). THIRD-PARTY-NOTICES regenerated (pdfjs-dist the only shipped-set mover). Gates: fresh `npm ci`, typecheck, build, suite **4866/50** (+1 = the pin), documents+chat screenshot smoke, `npm audit` 0. Open: DEP-1 §5 item 4 (grouped-weekly `dependabot.yml`, owner-gated) is now three hand-rolled batches old.
