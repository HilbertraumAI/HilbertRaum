...
describe('DocumentsScreen — Summarize action (Phase 33)', () => {
  it('runs the polling flow: busy state with progress, then the summary opens in the preview', async () => {
    ...
    // Summarize now lives in the per-row "⋯" overflow (§11.6).
    ...
    // Busy state is gone; after closing the modal the overflow's action reads "Summarize again".
    expect(screen.queryByText(/Summarizing…/)).not.toBeInTheDocument()
    await user.keyboard('{Escape}')
    await user.click(await screen.findByRole('button', { name: 'More actions for contract.pdf' }))
    expect(await screen.findByRole('menuitem', { name: /summarize again/i })).toBeInTheDocument()
  })
