...
describe('retrieve() with an external arm', () => {
  ...
  it('without an arm the pipeline is byte-identical (no seam side effects)', async () => {
    const db = freshDb()
    const embedder = new MockEmbedder()
    await seedDocument(db, embedder, 'notes.txt', ['Treibhausgase entstehen in der Landwirtschaft.'])
    const a = await retrieve(db, embedder, 'Treibhausgas', SETTINGS, null, null, undefined)
    const b = await retrieve(db, embedder, 'Treibhausgas', SETTINGS)
    expect(a).toEqual(b)
  })
})
