...
// Phase 33 IPC: document tasks (wave-3 plan §6). Async with polling — the Phase-4
// import / Phase-18 download precedent: `startDocTask` validates + enqueues and returns
// a job id immediately; the renderer polls `getDocTask` to drive progress UI;
// `cancelDocTask` aborts (no jobId = the active task, for the chat screen's busy
// banner). All guards (one-at-a-time, refuse-while-chat-streams, runtime-required) live
// in the DocTaskManager itself so non-IPC callers get them too.
//
