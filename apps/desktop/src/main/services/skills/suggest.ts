 ...
 * (the user already approved them — a crafted document can never *introduce* a skill, §10.2). Returns
 * an array (v1: 0 or 1) so the contract stays future-proof.
 */
export function suggestSkillsForTurn(
  ...
  const candidates: SkillCandidate[] = listSkills(db)
    .filter((s) => s.enabled && s.unavailableAt == null)
