...
/**
 * Why OCR is or is not on offer (#232). `'missing'` = no language files on the drive; the other
 * three mirror the engine's `OcrAvailability`: `'probing'` = packaged build, startup proof still
 * running; `'unavailable'` = files present but the recognizer cannot run here; `'available'`.
 */
...
export interface AppStatus {
   ...
   * probe runs and after a worker failure). Gates the "Make searchable (OCR)" offer and photo
   * imports — availability-driven, no settings key.
   */
  ocrAvailable: boolean
  /**
   * Why `ocrAvailable` is what it is (#232) — see `OcrState`. Additive + optional so older
   * status fixtures stay valid (the `translationDevice` shape).
   */
  ocrState?: OcrState
  /**
