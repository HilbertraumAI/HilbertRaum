...
- **When translation is slow, the Translate screen now says why on the line that reports it.** It
  ...
  memory was mostly taken, usually by the chat model. The line underneath is the remedy alone — use
  a smaller chat model, or stop it before translating — instead of repeating the explanation. The
...
- **The copied Performance report no longer files this computer's recommendation under another
  ...
  machine's. That line now reads "Recommended for the next start on this computer", with the
  context size directly beneath it.
...
- **Questions find the right knowledge-pack articles more often.** The pack server's search
  ...
  of nine.
- **A typo or a rare word among otherwise-fine ones is less likely to blank a knowledge-pack
  search.** The pack server still requires every remaining word to appear in an article, so one
  misspelled or pack-rare word could empty the whole search even after the fixes above. When a
  search still finds nothing, the app now checks how common each remaining word is in that
  archive and tries once more without the one that looks most responsible.
