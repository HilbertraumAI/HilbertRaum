...
- **REL-2 (Low, strongest new) — `VisionService.stop()` defeated by a `run()` rebuilding the runtime during teardown.** `run()` does `this.runtime ??= createRuntime(status)` *after* an `await getStatus()`; a `stop()` (lock/quit) interleaving that await leaves the resumed `run()` to spawn a fresh ~4.6 GB vision sidecar **after** teardown decided everything was down. Unlike embedder/reranker (F19), `VisionService` has no orchestrator-level `tearingDown` latch. Fix: add the F19 `tearingDown` latch, set at top of `stop()`, re-checked in `run()` after the `getStatus()` await. (`vision/index.ts:111-112` vs `:187-203`.)
  - ✅ **REMEDIATED (2026-06-30, Phase 6)** — service-level `tearingDown` latch (set at top of `stop()`, cleared in `finally`; re-checked in `run()` at the top AND after the `getStatus()` await). **Sharpened repro / divergence:** the "parked in `getStatus()`" variant was ALREADY neutralized by the existing `if (signal.aborted)` check (stop() aborts every vision controller synchronously before yielding); the genuinely-uncovered window the latch closes is a **NEW `analyze()` during an in-progress teardown** (fresh, un-aborted controller) — single-neuter teeth-checked (`createCalls` 1→2). See architecture.md §37 + GPU §5.5c.
- **REL-1 (Low) — OCR worker init latch nulled mid-flight.** `terminateWorker()` nulls `this.starting` even if its init promise is still pending (`ocr/tesseract.ts:111-139` vs `:203-215`); a timeout/`stop()` not ordered by `this.chain` can transiently spawn a second WASM worker / reject an awaiting recognition. Bounded/self-healing. Fix: only null `this.starting` when it still equals the torn-down promise, or route terminate through `this.chain`.
  - ✅ **REMEDIATED (2026-06-30, Phase 6)** — `terminateWorker` now AWAITS the in-flight init (the e5/reranker teardown mirror) so the worker it spawns is the one terminated, and clears the latch only if still that same promise. **Divergence:** the literal "capture + compare" is a no-op without the await (the reachable harm is the init outliving `stop()` — a leaked worker, not a second spawn, since `recognize()` is `stopped`-gated and timeout terminates run inside the chain). Teeth: drop the await → the late-born worker is never terminated. See architecture.md §37.
- **REL-3 (Low) — `e5.embed()` doesn't re-check `stopped`/`tearingDown` between batches** (`embeddings/e5.ts:216-271`); a suspend mid-ingestion surfaces as a confusing "not started" count error rather than a clean cancel. Fix: re-check per batch, or feed the ingestion `AbortController` into `embed(opts.signal)`.
  - ✅ **REMEDIATED (2026-06-30, Phase 6)** — per-batch re-check throws the recognizable cancellation `ensureStarted` raises. **Divergence:** the load-bearing condition is `this.server !== server` (captured-server staleness) on top of `stopped`/`tearingDown` — `tearingDown` clears in teardown's `finally`, so a `suspend()` that COMPLETED between batches is caught only by the staleness signal. Teeth: drop the re-check → the next batch fetches the dead server and reddens with "not started". See architecture.md §37 / §5.5b/c.
- **REL-4 (Low) — quit-path `shutdown()` doesn't abort in-flight streams before `runtime.stop()`** (`main/index.ts:476-508`), unlike the lock path (`registerWorkspaceIpc.ts:224-229`); a partial reply in flight at quit is lost rather than persisted-as-partial. Mirror the lock ordering or document the divergence.
  - ✅ **REMEDIATED (2026-06-30, Phase 6) — option (a) (abort, mirror the lock path).** The quit teardown was EXTRACTED to `main/shutdown.ts` (`performShutdown`) and now aborts `inFlightStreams` BEFORE `runtime.stop()`, so the partial reply unwinds as an abort and persists (`ctx.db` open until `lock()` last). Teeth: drop the abort loop → the ordering test reddens. See architecture.md §37.
...
### Phase 6 — Reliability hardening (REL-2 vision latch, REL-1 OCR latch, REL-3 e5 batch, REL-4 shutdown order) — ✅ REMEDIATED 2026-06-30
> **Done on branch `audit-followup-phase6-reliability`** (unmerged; do NOT auto-merge/push). All four latent
> teardown/concurrency gaps closed, each with a DETERMINISTIC gated-exit interleave test that RED→GREENs and is
> TEETH-CHECKED (neuter the guard → race resurrects → restore). Suite **2586 passed / 39 skipped** (was 2579/39
> → **+7**: vision-teardown ×2, OCR REL-1 ×1, e5 REL-3 ×1, shutdown REL-4 ×3), typecheck + `npm run build` green.
> No IPC / schema / audit-payload change; behavior-preserving (the robust paths the audit confirmed CLEAN stay
> green). **REL-4 DECISION: option (a)** — abort in-flight streams before `runtime.stop()` (mirror the lock path),
> with the quit teardown extracted to `main/shutdown.ts` (`performShutdown`) to make the ordering unit-testable.
> **Three divergences recorded (all the mechanically-correct call):** REL-2's sharpened repro (the latch is
> load-bearing for a NEW analyze during teardown, not the audit's already-guarded `getStatus()`-park variant);
> REL-1 awaits the in-flight init (the literal capture+compare is a no-op without it); REL-3's load-bearing
> condition is captured-server staleness (`this.server !== server`), since `tearingDown` clears between batches.
> Durable record: **architecture.md §37** (per-finding ledger) + the **§5.5c F19-family** cross-ref (REL-2/REL-3).
