...
- **✅ Phase C (2026-06-30, branch `audit-2026-06-30-phaseC-reliability`):** deterministic persistence. `withChatStream` publishes a per-stream `streamSettled` promise (`ipc/inflight.ts`), resolved (never rejected) in its `finally` AFTER the run's abort-unwind → `appendMessage` completes; `lockWorkspace` + `performShutdown` call `awaitInFlightStreamsSettled()` AFTER the sidecar stop and BEFORE purge/`lock()`, so persist-before-close is the ordering, not a race (placed after the stop so a signal-ignoring generation is still unwound by the dead sidecar — no stall; `allSettled` so one stream can't block teardown). Defense-in-depth: `generateAssistantMessage` swallows the partial-persist `appendMessage` cleanly (→ empty message, `log.warn` with the conversation id only) when `opts.signal?.aborted && !db.isOpen`, while a genuine open-DB error still propagates. SUPERSEDES the §37 REL-4 "persists during the awaited `runtime.stop()` window" reliance. Teeth: drop a settle-await → lock/quit ordering test reds; drop the guard → locked-DB persist rejects. Tests: `chat-stream` ×2, `shutdown` ×1, `workspace-ipc` ×1, new `lock-stream-persistence` ×3. Durable record: architecture.md §39.
...
- **✅ Phase C (2026-06-30):** the release-on-abort is installed on BOTH paths — one `release` closure (shared `released` latch with the returned release fn, so the slot is freed EXACTLY once) plus `signal.addEventListener('abort', release, { once })`. The slow path's `waitForHandoff` still handles an abort WHILE PARKED and throws before the listener install. Teeth: neuter the listener → the parked-build resume test reds (chatHolders stuck at 1); a second test pins idempotency (a naive double-decrement would resume the build prematurely). `analysis/model-slot-arbiter.ts`; `model-slot-arbiter.test.ts` ×2; §39.
...
- **✅ Phase C (2026-06-30):** both reconcile paths now STAGE every decode into a local array (the new pure `decodeRow`) and APPLY removals + additions in one throwless `applyStaged` step, committing `byChunk` + `modelByChunk` together; a throw during staging leaves the prior maps untouched for a clean retry. This stages only the |added|+|removed| delta (NOT a clone of the resident map), so **Phase-B P2 resident-iteration and the F12 O(K) delta perf are preserved**, and the deferred-P6 `COUNT(*)` staleness path is untouched. Teeth: inject a `db.prepare` whose 2nd point-lookup throws mid-reconcile → the live map is still the pristine base at the throw (in-place mutation already shows `{id_b, id_c}`), and the next clean query equals a from-scratch build. `embeddings/resident-cache.ts`; `resident-cache.test.ts` ×1; §39.
...
- **R4 ✅ (Low, latent)** `ocr/pipeline.ts:24-41` — the catch `await`s the **same** already-settled `prevOnPage` again for the final page; harmless today, but the "drain the in-flight recognition" guarantee is wrong for the last page. Fix: null `prevOnPage` before the in-try final `await`. **✅ Phase C:** `const last = prevOnPage; prevOnPage = null; if (last) await last` — the catch now drains only a genuinely still-pending look-ahead. Teeth: final-page recognition awaited exactly once. `ocr-pipeline.test.ts` ×1; §39.
- **R5 ✅ (Low, latent)** `runtime/gpu.ts:141-160` — probe timeout `SIGKILL`s but doesn't await reap; rapid `invalidate()`+re-probe ("Try GPU again" mashing) stacks short-lived children (unref'd, OS-reaped). Fix: ignore/await an in-flight probe per binary before starting a new one. **✅ Phase C:** `invalidate()` drops only SETTLED entries; an in-flight re-probe COALESCES onto the existing promise (one child per binary). Teeth: mashing during an in-flight probe spawns 1, not 1+N. `gpu.test.ts` ×1; §39 + GPU §5.4.
- **R6 ✅ (Low, latent)** `ocr/rasterizer.ts:100-107` — `expect(channel)` overwrites a pending `waiter` without rejecting it; safe under the current single-in-flight protocol, but a duplicate frame / refactor would orphan the prior promise to its 60 s timeout. Fix: reject the existing waiter before reassigning. **✅ Phase C:** the closure is extracted to a pure, exported `RasterReplySlot` (behavior-identical) whose `expect()` rejects a still-pending prior waiter (`'superseded'`) before re-arming. Teeth: two `expect()`s without a settle → the first rejects. new `ocr-rasterizer-slot.test.ts` ×3; §39.
- **R7 ✅ (Low, latent)** `vision/runtime.ts:194-309` — `analyze()`'s `finally` `armIdleTimer()` vs a concurrent `stop()` is unsynchronized across the await; a timer armed in the window survives `stop()`'s `cancelIdleTimer()` (unref'd, self-cancels on fire). Fix: re-call `cancelIdleTimer()` after the awaits in `stop()`. **✅ Phase C — ALREADY-MITIGATED; defense-in-depth added:** the race is already closed TWICE — `armIdleTimer` returns early on BOTH `this.stopped` AND `!this.server`, both set synchronously by `stop()` before any await (triple-guarded), so a racing `analyze()` finally cannot arm a surviving timer today. The re-cancel after `stop()`'s awaits is a third, defense-in-depth backstop making stop()'s "no idle timer on return" postcondition LOCAL (the §37 REL-2 / F18 co-guarded-twin pattern). Test (g) is a property/regression guard (no idle timer survives a stop racing an analyze settle); single/dual-neuter stay green (the `!server` guard also blocks the arm), recorded transparently. `vision-runtime.test.ts` (g); §39 + GPU §5.5c.
...
- **✅ Phase D (2026-06-30):** the `if (!mountedRef.current) return` guard added before `onText`/`onError` and the `finally` `setState` — the IPC completes harmlessly, the dead component is never touched. `Dictation.test.tsx` resolves `transcribeDictation` after `unmount()` → `onText`/`onError` not called, no act-warning; teeth: drop the guard → red. arch "Renderer robustness" Phase D.
...
- **✅ Phase D (2026-06-30):** done exactly as recommended — `localizedStream = useMemo(…)` feeds both the visible bubble and `<StreamAnnouncer>`; byte-identical output, half the work. arch "Renderer robustness" Phase D.
...
- **✅ Phase D (2026-06-30):** the `.then`/`.catch` now apply `setSkillSuggestion` only when `mountedRef.current && (activeIdRef.current ?? '') === convId` (reusing the FE-1 `activeIdRef`), so a late reply never setStates a dead component or stamps a stale-conversation suggestion. arch "Renderer robustness" Phase D.
...
- **✅ Phase D (2026-06-30):** BOTH — a `busy` (= `analyzing`) prop threaded through `AnswerThread`→`TurnRow` disables "Try again" (Copy stays live); and `analyze()` now returns `'started' | 'busy' | 'noop'` so `runAnalyze` surfaces `images.err.busy` if a click still reaches the busy guard. Disabling on `analyzing` also covers the pre-`activeJobId` create window. Test: a 2nd analysis in flight → prior "Try again" `toBeDisabled()`; teeth: drop `disabled={busy}` → red. arch "Renderer robustness" Phase D.

#### F5–F8 — low-severity renderer items
- **F5 ✅ (Low) — DEFERRED (accepted).** `Composer.tsx:75-81` — auto-grow effect reads `scrollHeight`/`offsetHeight` after writing `style.height`, forcing a layout reflow per keystroke (compounds with ChatScreen's per-keystroke re-render). Acceptable for one textarea; batch via rAF if it shows. **✅ Phase D:** left as-is by design — single textarea, sub-ms imperceptible cost; an rAF batch would add a cancelable frame handle + unmount cleanup and risk a one-frame height-lag jump. Rationale recorded in arch "Renderer robustness" Phase D.
- **F6 ✅ (Low, a11y) — FALLBACK ADDED + residual documented.** `Transcript.tsx:377-426` (`StreamAnnouncer`) — only advances on sentence terminators and `stripMarkdown`s code/markup to spaces, so a code-block- or table-only answer announces nothing until completion (then near-empty). Document as an accepted limitation or add a length-based fallback boundary. **✅ Phase D:** added the length fallback — past `ANNOUNCE_SOFT_CAP` (160) with no terminator the announcer flushes to the last **word** boundary (`lastWordBoundary`), so tables/lists/run-on prose announce incrementally; teeth: neuter → region stays empty → red. **Accepted residual:** a *pure code block* strips to ~nothing (voicing code punctuation is worse than silence) → stays quiet (known-limitations Accessibility). arch "Renderer robustness" Phase D.
- **F7 ✅ (Low) — DEFERRED (accepted; self-heals).** `ChatScreen.tsx:915-939` — regenerate optimistically slices the last assistant turn before `stream(...)`; if the IPC throws *before* the backend mutates, or the user switched conversations, the local view drops an answer the DB still holds until a manual re-select. Self-heals; not data loss. **✅ Phase D:** verified the self-heal is automatic — `stream`'s `catch` calls `refreshIfVisible()` (re-reads `listMessages(convId)`) in place when the user stayed, and the `activeId`-change effect re-reads on return when they switched away, so NO manual re-select is needed. Deferring keeps the immediate optimistic feedback (known-limitations Engineering trade-offs). arch "Renderer robustness" Phase D.
- **F8 ✅ (Low, hypothesis → CONFIRMED reachable, FIXED).** `lib/visionSession.ts:202-232` — `unsubs`/`activeTurnId` are module-global; a fast image-switch-then-analyze can let a prior job's `teardownStream()` clear the new job's `unsubs`. Hard to hit (busy guard). Fix: scope per-job and verify `job.jobId === activeJobId` before patching. **✅ Phase D — reproduced and fixed:** the busy guard only covers `activeJobId`, which isn't set until AFTER the `imageAnalyze` create round-trip, so a slow round-trip + switch + re-analyze leaves two analyzes both awaiting it; the slower wires a zombie stream its late done/error tears down over the newer job. Each `analyze()` now claims a monotonic `analyzeGen` (bumped by `abortActive`/`clearVisionSession`); a superseded call cancels its orphan job main-side and bails WITHOUT wiring; per-handler `jobId === snapshot.activeJobId` checks are belt-and-braces. Store-level test (`visionSession.test.ts`): a slow A-create resolving after a switch-to-B is cancelled + never wires, B stays live; teeth: remove the gen bail → A wires, `imageCancel('jobA')` never fires → red. arch "Renderer robustness" Phase D.
...
|---|---|---|---|
| **T1** ✅ | Med | `tests/integration/vision-teardown.test.ts:37,66,72,103,108`, `vision-cancel.test.ts:61` | Real `setTimeout` `sleep(N)` gates the REL-2 interleave premise (`void this.run` is detached; nothing deterministically guarantees it parked in `getStatus()` before `stop()`). Under CPU starvation the assertion `createCalls===0` can pass **vacuously** — a regression removing the post-`getStatus()` `tearingDown` re-check could ship green. Ironically the Phase-7 TEST-1 round deleted exactly this real-timer class from a *sibling* file. Fix: deterministic park gate (the `model-slot-arbiter`/`combine-signals` pattern). **✅ Phase F:** both files de-flaked to injected gates + `while (cond) await tick()` (setImmediate drain); test (2) redesigned to hold the teardown IN FLIGHT so the post-`getStatus()` re-check is a LIVE co-guard. Teeth (verified empirically, recorded honestly): each scenario reds only on its DUAL neuter — both `tearingDown` checks are co-guards (the prior "single-neuter top-latch reds" comment was inaccurate; corrected); the F18 cancel dual-neuter still reds. arch "Test-enforcement seams" Phase-F. |
| **T2** ✅ | Med | `tests/renderer/DocumentsScreen.test.tsx:~1039-1094` | Render-count deltas on the private `__docRowRenderCounts` Map are the **sole** oracle — no `toBeChecked()`, no selection-UI assertion. A regression where the click stops toggling selection but an unrelated re-render still bumps the count passes green; and since DX-2 DEV-guards the counter, the test degenerates if DEV ever flips. Fix: pair each delta with a behavioral assertion. **✅ Phase F:** each PERF-5 case now asserts the click EFFECT first (toggle: `toBeChecked()` + siblings unchecked + toolbar `1 selected`; menu: `getByRole('menu')` + `Re-index` item + sibling titles intact), keeping the deltas as a SECONDARY perf oracle. Teeth: dropping `memo(DocRow)` reds the deltas (perf layer intact) while the behavioral layer still passes — the two are independent. arch "Test-enforcement seams" Phase-F. |
| **T3** ✅ | Med | rollback paths: `tree-build.ts:255`, `ingestion/index.ts:750/944/1549`, `doctasks/handlers/categorize.ts:111`, `analysis/node-vectors.ts:156`, `vision/history.ts:233` | Only **1 of ~12** `BEGIN…COMMIT…ROLLBACK` sites (`deleteConversation`) has an injected-failure rollback test. The rest claim rollback-on-throw in comments but no test drives it; on the single shared `DatabaseSync` connection, a left-open `BEGIN` poisons the *next* unrelated op. The ingestion path (~1000 inserts) is higher-blast-radius than the one tested. Fix: add injected-failure tests for `commitNode` + the categorize persist, mirroring `data-layer-hardening.test.ts:67-100` (assert no partial rows + a subsequent `BEGIN/COMMIT` succeeds). **✅ Phase F:** added gold-standard injected-failure rollback tests for the **categorize persist** (`doctasks-categorize.test.ts`, real `DocTaskManager`+`runCategorize`) and the **ingestion chunk-insert loop** (`ingestion.test.ts`, real `processDocument`, throw on the 2nd insert so a chunk is inserted-then-rolled-back) — each asserts no partial rows + connection-not-poisoned + clean recovery; teeth-checked (neuter the `ROLLBACK` → red). **Audit correction:** `commitNode` is NOT untested — H11 (`whole-doc-analysis.test.ts:421`) already drives the real `buildTree` with an injected edge-insert failure + the gold-standard assertions, and the extraction insert has its own (`whole-doc-extract.test.ts:221`); "1 of ~12" was ≥3. arch "Test-enforcement seams" Phase-F. |
...
| **T5** ✅ | Low | `vitest.config.ts:36-41` | The 3× timeout (15 s) papers over documented "1-2 flakes per run, a different test each time" instead of removing the wall-clock waits. Treat the flakes as a backlog (start with T1). **✅ Phase F (swept):** converted the three genuine correctness-gating fixed waits (`reranker:125` + `e5-embedder:262` `sleep(1)`→`while(!seenSignal)`; `ocr-task:210` `sleep(30)`→a polled `rasterizeReached` gate); the bulk of the others are STATE-POLL loop intervals (already deterministic). Residuals recorded (NOT converted): `chat:171`/`image-history:111` timestamp-monotonicity (need a timestamp seam), `core-model-ipc:448` absence-settle (no racing call to await), `ingestion-limits:80/254` deliberate slow-op simulations. Timeout headroom retained (it absorbs the orthogonal CPU-starvation renderer flake, not a fixed-sleep gate). arch "Test-enforcement seams" Phase-F. |
| **T6** ✅ | Low | `tests/integration/password-change.test.ts:388-399` | The doc-work/password-change race guard drives the private `changingPassword` field directly ("cannot be produced from the outside"), pinning implementation state, not the invariant. If `changePassword` becomes async and forgets to set the flag, the test still passes. At least spy the flag transitions; ideally inject a pause inside the real `changePassword`. **✅ Phase F:** the test now traps the private flag via `Object.defineProperty` to RECORD its real transitions and, at the instant the real `changePassword` flips it true mid-work, prove `beginDocumentWork()` is genuinely refused — asserts `transitions === [true, false]` (SET+CLEARED around the real Argon2id rewrap) AND the in-flight refusal. Teeth: neuter `changePassword`'s flag SET → `[false]` + no refusal → red (the field-poking test could not). Reciprocal direction (`:375`) left intact. arch "Test-enforcement seams" Phase-F. |
| **T7** ✅ | Low | `assets.ts:451-475`, `tests/integration/assets.test.ts:701-792` | The SSRF guard is literal-dotted-decimal only; decimal/octal/hex IP encodings of loopback are neither blocked nor tested nor documented as out-of-scope. Either canonicalize numeric hosts + test, or assert `assertSafeDownloadUrl` rejects `http://2130706433/` so the decision is pinned. **✅ Phase F — ALREADY-MITIGATED; pinned, NO src change:** driving the real path shows it is not bypassable — `assertSafeDownloadUrl` reads `new URL(raw).hostname`, and the WHATWG parser CANONICALIZES every numeric IPv4 spelling to dotted-decimal before the deny-regex (`new URL('https://2130706433/').hostname === '127.0.0.1'`; hex/octal + `169.254.169.254` likewise), so it already rejects them. Regression-pinned in both F15 styles (`assets.test.ts`): the real `downloadToFile`→`assertSafeDownloadUrl` rejects a redirect to each encoding `/private\/loopback/`, + the unit classifier on the canonicalized host. Decision documented (closed by URL canonicalization). arch "Test-enforcement seams" Phase-F. |
...
### Phase C — Lock/teardown reliability (R1; latent R2, R3; small R4–R7) ✅ REMEDIATED (2026-06-30, branch `audit-2026-06-30-phaseC-reliability`, stacked on the Phase-B branch)
- **✅ Outcome (BEHAVIOR-PRESERVING — no schema/IPC/audit-payload change; each guard independently revertible + teeth-checked by a deterministic interleave; content class never logged):** **R1** (the one live race) fixed via a `streamSettled` registry the lock/quit teardown awaits before closing the DB + an `appendMessage` locked-DB guard (per-finding §2 R1). **R2** (fast-path abort listener), **R3** (reconcile stage-then-commit, Phase-B P2/F12 preserved), **R4** (final-page drain), **R5** (probe coalescing), **R6** (rasterizer supersede) all fixed and RED→GREEN teeth-checked. **R7** verified ALREADY-mitigated (armIdleTimer is triple-guarded by `stopped`+`!server`); a re-cancel backstop added as defense-in-depth (property test, recorded transparently). Verification from repo root: `npm test` **2669 passed / 41 skipped** (Phase-B baseline 2653/41 → **+16**), two consecutive runs identical; `npm run typecheck` + `npm run build` green. Durable record: architecture.md **§39** (+ GPU §5.4/§5.5c cross-refs). No reliability issue outside R1–R7 surfaced. Phases F/D/E remain open; the report is NOT retired.
...
### Phase D — Renderer lifecycle & a11y guards (F1; F3, F4; small F2, F5–F8) — renderer-only ✅ REMEDIATED (2026-06-30, branch `audit-2026-06-30-phaseD-renderer`, stacked on the Phase-F branch)
...
- **✅ Outcome (renderer-only; no main-process/IPC/schema/i18n change — the one banner reuses the existing `images.err.busy` key; each fix independently revertible):** **F1** (the one real bug) fixed — `stopAndTranscribe` mountedRef-guards `onText`/`onError`/`setState` (per-finding §2 F1). **F2** memoizes `localizeServerCopy` (one compute feeds bubble + announcer). **F3** gates the suggestion debounce on `mountedRef` + `activeIdRef`-convId match. **F4** disables the per-turn "Try again" while `analyzing` AND surfaces `images.err.busy` on the busy early-return (`analyze()` → `'started'|'busy'|'noop'`). **F6** adds the `StreamAnnouncer` word-boundary length fallback (pure-code residual documented). **F8** verified REACHABLE (pre-`activeJobId` create window) and fixed with a per-call `analyzeGen` + per-handler `activeJobId` checks. **F5** (Composer reflow) and **F7** (optimistic regenerate slice) DEFERRED as accepted — F7 verified self-healing via the `catch` `refreshIfVisible` + the `activeId` effect (both in known-limitations). F1/F4/F6/F8 are teeth-checked (neuter → red → restore). **Verification from repo root:** `npm test` **2677 passed / 41 skipped** (Phase-F baseline 2673/41 → **+4**: F1+F4+F6+F8); `npm run typecheck` + `npm run build` green. Durable record: architecture.md "Renderer robustness — design record" **Phase D** subsection. Phase E remains open; the report is NOT retired.
...
### Phase F — Test-suite robustness (T1–T4; T5–T7 as backlog) — test-only ✅ REMEDIATED (2026-06-30, branch `audit-2026-06-30-phaseF-tests`, stacked on the Phase-C branch)
...
- **✅ Outcome (test-only; `git diff src/` EMPTY):** T1–T3 done + T5–T7 dispositioned (per-finding in the §4 table above); **T4 was landed in Phase A** (not redone here). T1 — vision teardown/cancel de-flaked to injected gates + state-polling, teeth recorded honestly (both `tearingDown` checks are co-guards → DUAL-neuter reds; the prior "single-neuter" comment corrected). T2 — render-count deltas paired with `toBeChecked()`/menu/toolbar behavioral assertions (perf deltas kept as a secondary oracle). T3 — added categorize-persist + ingestion-chunk-insert rollback tests (gold-standard, teeth-checked); **discovered `commitNode` (H11) + the extraction insert already have rollback tests** — the audit's "1 of ~12" was ≥3. T5 — swept; converted the 3 genuine correctness-gates (`reranker`/`e5`/`ocr-task`), recorded the residuals (timestamp-monotonicity / absence-settle / slow-op simulations), kept the timeout headroom. T6 — `changingPassword` lifecycle pinned via a property-trap on the real `changePassword` (teeth: neuter the flag SET → red). T7 — **already-mitigated** (WHATWG `new URL` canonicalizes numeric IPv4 → dotted-decimal before the deny-regex); pinned in both F15 styles, **no guard change**. **Verification from repo root:** `npm test` **2673 passed / 41 skipped** (Phase-C 2669/41 → **+4**: T3 ×2 + T7 ×2); `npm run typecheck` + `npm run build` green; **two consecutive full runs identical** (one orthogonal pre-existing CPU-starvation renderer flake observed once across four runs, in an untouched test, passing in isolation — the documented `vitest.config.ts` load flake, not a Phase-F regression). Durable record: architecture.md "Test-enforcement seams — design record" Phase-F subsection. Phases D/E remain open; the report is NOT retired.
...
6. **Phase D (renderer) — F1 is the only real renderer bug; the rest are polish.** Independent. **✅ DONE (2026-06-30, branch `audit-2026-06-30-phaseD-renderer`)** — F1/F4/F6/F8 fixed + teeth-checked, F2/F3 guarded, F5/F7 deferred as accepted; renderer-only; see §6 "Phase D" outcome.
