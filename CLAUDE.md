...
1. [`BUILD_STATE.md`](BUILD_STATE.md) — the live handoff/state file. **Always read first.** It says what is done, current decisions, and the next actions. It holds **open work only** — dated entries for the currently-open waves, and a §5 whose closed rounds are collapsed to outcome + ledger pointer + still-open residuals. **Per-section size budgets** enforce that (`repo-hygiene.test.ts`; a whole-file cap alone let §3 and §5 grow to 80% of the file before anyone noticed). Retired entries **and retired sections** live verbatim in [`docs/build-log.md`](docs/build-log.md) (grep it when an old "BUILD_STATE <date> entry" or "BUILD_STATE §3 / §8" citation needs resolving), and the shared data contracts live in [`docs/data-contracts.md`](docs/data-contracts.md). **Never renumber a §5 item** — those citations are load-bearing.
